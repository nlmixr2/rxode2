rxTest({
  ...
  test_that("rxFromSE() reproduces the recorded output byte for byte", {
    .f <- readRDS(.fixtureFile)
    .r <- .seFixtureCmp(.f$fromSE, function(x) rxode2::rxFromSE(x), "rxFromSE")
    expect_equal(.r$nbad, 0L, info = .r$msg)
  })

  test_that("rxFromSE() unknownDerivatives= modes are unchanged", {
    .f <- readRDS(.fixtureFile)
    .rf <- .seFixtureCmp(.f$fromSEforward, function(x) rxode2::rxFromSE(x, "forward"), "rxFromSE(forward)")
    expect_equal(.rf$nbad, 0L, info = .rf$msg)
    .rc <- .seFixtureCmp(.f$fromSEcentral, function(x) rxode2::rxFromSE(x, "central"), "rxFromSE(central)")
