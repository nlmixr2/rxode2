rxTest({
  ...
  test_that("past() first-order sensitivities match finite differences", {
    ...
    .s <- rxSolve(rxode2(.mstr(0.3, 1, 0.5), calcSens = c("k", "a", "b")), .ev, atol = 1e-11, rtol = 1e-11)
    .fwd <- function(k, a, b) rxSolve(rxode2(.mstr(k, a, b)), .ev, atol = 1e-11, rtol = 1e-11)$y
    .eps <- 1e-4
  ...
  test_that("past() second-order sensitivities match finite differences", {
    ...
    .pars <- c("k", "a", "b")
    .s <- rxSolve(rxode2(.mstr(0.3, 1, 0.5), calcSens = .pars, calcSens2 = .pars), .ev, atol = 1e-12, rtol = 1e-12)
    .fwd <- function(m) rxSolve(rxode2(.mstr(m[1], m[2], m[3])), .ev, atol = 1e-12, rtol = 1e-12)$y
    .base <- c(k = 0.3, a = 1, b = 0.5)
    names(.base) <- .pars
    .h <- 1e-3
    .same <- function(p) {
    ...
    for (.i in seq_along(.pars)) {
      for (.j in .i:length(.pars)) {
        ...
        if (is.null(.s[[.col]])) {
          .col <- paste0("rx__sens_y_BY_", .q, "_BY_", .p, "__")
        }
  ...
  test_that("past() without d/dt() errors cleanly and leaves the parser usable", {
    ...
    expect_output(
      try(suppressMessages(rxModelVars(.bad)), silent = TRUE),
  ...
  test_that("past() survives chunked expression optimization", {
    .filler <- paste(sprintf("v%d = %d * exp(kg * t) + sin(v0 * %d)", 1:45, 1:45, 1:45), collapse = "\n")
    .mod <- paste0(
      "a = 1\nb = 0.5\nT = 12.8\nk3 = 5\nkg = 0.4\nk4 = 0.3\nk5 = 0.1\nv0 = 2\n",
      "G(0) = a\nd/dt(G) = k3 - kg * G\n",
    ...
    .ev <- et(seq(0, 30, by = 1))
    .s1 <- rxSolve(rxode2(.mod), .ev, method = "dop853", atol = 1e-10, rtol = 1e-10, dense = TRUE)
    .s2 <- rxSolve(rxode2(.o), .ev, method = "dop853", atol = 1e-10, rtol = 1e-10, dense = TRUE)
  ...
  test_that("generated models keep the past() duration matched to delay() (#1193)", {
    ...
    .p <- c(lT = 0.2, b = 0.5)
    .ref <- rxSolve(rxode2(.genModel), .p, .ev, method = "dop853", dense = TRUE, atol = 1e-10, rtol = 1e-10)
    for (.g in list(rxode2(.genModel, calcJac = TRUE), suppressMessages(rxode2(.genModel, calcSens = "k")))) {
      ...
      .s <- rxSolve(.g, .p, .ev, method = "dop853", dense = TRUE, atol = 1e-10, rtol = 1e-10)
  ...
  test_that("an operator duration still matches its delay() in a generated model", {
    ...
    for (.g in list(rxode2(.m, calcJac = TRUE), suppressMessages(rxode2(.m, calcSens = "k")))) {
      expect_error(.rxValidatePast(.g), NA)
      expect_equal(rxSolve(.g, .ev, method = "dop853", dense = TRUE)$G, .ref$G, tolerance = 1e-8)
    }
  })
