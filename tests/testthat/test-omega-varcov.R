rxTest({
  ...
  test_that("first derivatives of Omega^-1 and log|Omega| match FD", {
    ...
    for (k in seq_len(np)) {
      ...
      fdld <- (determinant(perturb(Om, k, h), logarithm = TRUE)$modulus -
        determinant(perturb(Om, k, -h), logarithm = TRUE)$modulus) /
        (2 * h)
  ...
  test_that("second derivative of log|Omega| matches FD", {
    ...
    for (j in seq_len(np)) {
      for (k in seq_len(np)) {
        pp <- perturb(perturb(Om, j, h), k, h)
        pm <- perturb(perturb(Om, j, h), k, -h)
        mp <- perturb(perturb(Om, j, -h), k, h)
        mm <- perturb(perturb(Om, j, -h), k, -h)
        fd <- (as.numeric(determinant(pp, logarithm = TRUE)$modulus) -
          ...
          as.numeric(determinant(mm, logarithm = TRUE)$modulus)) /
          (4 * h * h)
