rxTest({
  ...
  .evObs <- function(obsT, doseT = 0, amt = 100, rate = 0) {
    dose <- data.frame(id = 1, time = doseT, amt = amt, evid = 1, cmt = 1, rate = rate, ii = 0, ss = 0)
    obs <- data.frame(id = 1, time = obsT, amt = 0, evid = 0, cmt = 1, rate = 0, ii = 0, ss = 0)
    rbind(dose, obs)
  }
  ...
  test_that("delta memo is bitwise-exact on a mixed dose/obs design", {
    mod <- .linCmtTestModel(2, 1, 0:4)
    pars <- .linCmtTestPars(2, 1)
    # mixed: bolus + infusion doses, uniform AND irregular observation gaps
    ev <- rbind(
      .evObs(c(seq(1, 12, by = 1), 13.7, 15.9, 22.31), doseT = 0),
      data.frame(id = 1, time = 24, amt = 80, evid = 1, cmt = 1, rate = 20, ii = 0, ss = 0),
      data.frame(id = 1, time = c(26, 28, 30, 31.5, 44.123), amt = 0, evid = 0, cmt = 1, rate = 0, ii = 0, ss = 0)
    )
    ev <- ev[order(ev$time), ]
  ...
  test_that("the within-row slot is exact on a fully non-uniform design", {
    ...
    mod <- .linCmtTestModel(3, 1, 0:6)
    pars <- .linCmtTestPars(3, 1)
    ev <- .evObs(cumsum(seq(0.17, 3, length.out = 40)))
    for (.phi in list(FALSE, 2L)) {
      rxode2::linCmtDeltaMemo(1L)
      rxode2::linCmtSeqStats(TRUE)
      sOn <- as.data.frame(rxode2::rxSolve(
        mod,
        pars,
        ev,
        cores = 1L,
      ...
      st <- rxode2::linCmtSeqStats(TRUE)
      rxode2::linCmtDeltaMemo(0L)
      sOff <- as.data.frame(rxode2::rxSolve(
        mod,
        pars,
        ev,
        cores = 1L,
  ...
  test_that("threaded solve is bit-identical to single-threaded with the memo", {
    ...
    s1 <- as.data.frame(rxode2::rxSolve(
      mod,
      pars,
      ev,
      cores = 1L,
    ...
    for (r in 1:5) {
      sN <- as.data.frame(rxode2::rxSolve(
        mod,
        pars,
        ev,
        cores = 2L,
