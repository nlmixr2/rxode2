  ...
  for (.cfg in .fixedCases) {
    local({
      ...
      test_that(paste("order test (fixed):", .meth, "order", .order), {
        .s <- rxode2::rxSolve(.mod, .ev, inits = c(x = 0), method = .meth, hmin = 1.0, cores = 1)
        expect_lt(abs(.s$x[1] - 1.0), .tol, label = paste(.meth, "order test error"))
      })
    })
  }
  ...
  for (.cfg in .var_cases) {
    local({
      ...
      test_that(paste("order test (adaptive):", .meth, "order", .order), {
        .s <- rxode2::rxSolve(.mod, .ev, inits = c(x = 0), method = .meth, atol = 1e10, rtol = 1e10, cores = 1)
        expect_lt(abs(.s$x[1] - 1.0), .tol, label = paste(.meth, "order test error"))
      })
    })
  }
