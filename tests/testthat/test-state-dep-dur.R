rxTest({
  ...
  test_that("state-dep dur with state=0 uses exactly dur0 (no state shift)", {
    ...
    s <- solve(mod, et, inits = c(state = 0, depot = 0), params = c(dur0 = dur0, ka = ka))
  ...
  test_that("state-dep dur shifts infusion duration by state value: dur = dur0 + state_at_start", {
    ...
    for (S0 in c(2, 5, -3)) {
      ...
      s <- solve(mod, et, inits = c(state = S0, depot = 0), params = c(dur0 = dur0, ka = ka))
      ...
      expect_equal(
        ...
        expected,
        tolerance = 1e-3,
        label = paste0("depot matches analytic solution for dur=", D, " (S0=", S0, ")")
      )
  ...
  test_that("state-dep dur: consecutive doses evaluated at their own start state", {
    ...
    s <- rxSolve(mod, et, params = c(dur0 = 5, ka = 0.4, kstate = 0.2), inits = c(depot = 0, state = 3))

    # No numerical failures
    expect_false(any(is.na(s$depot)), label = "no NAs in depot with two state-dep dur doses")

    # The trajectory must differ from a state=0 run (different duration per dose)
    s0 <- rxSolve(mod, et, params = c(dur0 = 5, ka = 0.4, kstate = 0.2), inits = c(depot = 0, state = 0))
