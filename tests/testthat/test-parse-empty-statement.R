  ...
  test_that("the bodiless if/while error says what to write instead", {
    ...
    .msg <- function(x) {
      .o <- utils::capture.output(
        try(rxNorm(x), silent = TRUE),
        type = "output"
      )
      paste(.o, collapse = "\n")
    }
    expect_match(.msg("if (a>1)"), "'if' needs a statement for its body", fixed = TRUE)
    expect_match(.msg("while (a>1)"), "'while' needs a statement for its body", fixed = TRUE)
    ## `if (c) else s` fails at the `else`; the missing body is the then-branch
    expect_match(.msg("if (a>1) else b=1"), "'if' needs a statement for its body", fixed = TRUE)
    ## an unrelated syntax error must NOT be relabelled as this one
    expect_false(grepl("needs a statement for its body", .msg("a = 1 +"), fixed = TRUE))
  })

  test_that("an empty body written as {} or ; still parses", {
    expect_equal(rxNorm("if (a>1) {}"), "if (a>1){\n}\n")
    expect_equal(rxNorm("if (a>1) ;"), "if (a>1){\n}\n")
    expect_equal(rxNorm("if (a>1) {} else {b=1}"), "if (a>1){\n}\nelse {\nb=1;\n}\n")
    expect_equal(rxNorm("if (a>1) ; else b=1"), "if (a>1){\n}\nelse {\nb=1;\n}\n")
  })
      ...
      paste(
        c(
          ...
          unlist(lapply(seq_len(n), function(i) {
            c(
              sprintf("kel%d = cl%d/v%d", i, i, i),
              sprintf("d/dt(c%d) = ka*depot - kel%d*c%d + p%d*(c%d/v%d)", i, i, i, i, i, i)
            )
          }))
        ),
        collapse = "\n"
