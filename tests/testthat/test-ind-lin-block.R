rxTest({
  ...
  .blkSolve <- function(mod, pars, ev, off = FALSE, ...) {
    ...
    .s <- withr::with_envvar(
      ...
      suppressMessages(rxSolve(mod, params = pars, events = ev, method = "indLin", ...))
    )
  ...
  test_that("the split carries the output accumulator and the sensitivities", {
    ...
    .rate <- .th[["cl"]] /
      .th[["v"]] *
      (.s$central + .s$rx__sens_central_BY_ka__ + .s$rx__sens_central_BY_cl__ + .s$rx__sens_central_BY_v__)
