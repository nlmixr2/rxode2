rxTest({
  ...
  test_that("linToOde() keeps the compartment numbers of a mixed model", {
    ...
    expect_equal(ode$stateDf[["Compartment Name"]], c("depot", "central", "ce", "peripheral1"))
  ...
  test_that("linToOde() does not reuse a model variable for the prediction", {
    f <- function() {
      ini({
        tka <- 0.45
        tcl <- 1
    ...
    ode <- suppressMessages(linToOde(f))
    expect_equal(ode$predDf$var, "rxLinCmtOde1")
    expect_error(rxode2(ode$simulationModel), NA)
  })

  test_that("linToOde() translates linCmt() in an ODE and with arguments", {
    inOde <- function() {
      ini({
        ka <- 1
        cl <- 2
      ...
      model({
        d/dt(ce) <- ke0 * (linCmt(ka, cl, v) - ce)
        ce ~ add(add.sd)
      })
    }
    ui <- suppressMessages(inOde())
    ode <- suppressMessages(linToOde(ui))
    expect_false(any(grepl("linCmt", vapply(ode$lstExpr, deparse1, character(1)))))
    et <- et(amt = 100) |> et(seq(0.5, 24, by = 0.5))
    s1 <- rxSolve(ui, et, returnType = "data.frame")
    ...
    args <- function() {
      ...
      model({
        ...
        cl <- exp(tcl)
        v <- exp(tv)
        linCmt(ka, cl, v) ~ add(add.sd) | tmp
      })
    }
    ode <- suppressMessages(linToOde(args))
    expect_false(any(grepl("linCmt", vapply(ode$lstExpr, deparse1, character(1)))))
    expect_false(ode$predDf$linCmt)
  })

  test_that("linToOde() writes the ODEs once for several linCmt() endpoints", {
    f <- function() {
      ini({
        tka <- 0.45
        tcl <- 1
    ...
    ode <- suppressMessages(linToOde(f))
    .lines <- vapply(ode$lstExpr, deparse1, character(1))
    expect_equal(sum(grepl("^d/dt\\(central\\)", .lines)), 1L)
    expect_equal(nrow(ode$predDf), 2L)
    expect_false(any(ode$predDf$linCmt))
  })
})
