test_that("model builds survive leaked compiler environment variables", {
  ...
  # and a bogus library.
  .old <- Sys.getenv(c("PKG_CPPFLAGS", "PKG_LIBS", "USE_CXX17"), unset = NA_character_)
  on.exit(
    {
      for (.v in names(.old)) {
        if (is.na(.old[[.v]])) {
  ...
  Sys.setenv(PKG_CPPFLAGS = paste0("-include '", .hdr, "'"), PKG_LIBS = "-lrxNoSuchLibrary", USE_CXX17 = "1")
  # a model text unique to this run so a cached DLL cannot mask the compile
  .txt <- sprintf("d/dt(a) = -%s*a", format(stats::runif(1, 0.1, 0.2), digits = 10))
  .m <- rxode2(.txt)
