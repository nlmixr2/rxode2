rxTest({
  ...
  test_that("a model at or below chunkLines is untouched by the default", {
    ...
    suppressMessages(expect_identical(rxOptExpr(.m, "model"), rxOptExpr(.m, "model", chunkLines = 0L)))
  })
      ...
      tryCatch(
        {
          .z <- capture.output(
            suppressMessages(
              rxOptExpr(.m, "model", chunkLines = .chunkLines)
            ),
            type = "output"
          )
          NA_character_
  ...
  test_that("delay disguise/restore round-trips and is chunk-local", {
    .chunks <- list(
      c("d/dt(x)=-k*x", "d/dt(y)=k*x-ke*y", "a=delay(x, tau)+1"), # d/dt(x) here: kept as-is
      c(
        "b=delay(x, exp(tau))*2", # split from d/dt(x): disguised
        "c=rxDelayD2(y, tau+delay(x, exp(tau)))", # nested call inside a disguised one
        "d=delay(z, tau)"
      )
    ) # no d/dt(z) anywhere: left alone
    ...
    expect_identical(
      .d$chunks[[2]][1],
      sub("delay(x, exp(tau))", names(.d$map)[match("delay(x, exp(tau))", .d$map)], .chunks[[2]][1], fixed = TRUE)
    )
    .joined <- vapply(.d$chunks, paste, character(1), collapse = "\n")
    expect_identical(
      .rxRestoreDelay(paste(.joined, collapse = "\n"), .d$map),
  ...
  test_that("a model already using the chunker's own generated names is not chunked", {
    ...
    .ode <- c("d/dt(depot)=-exp(THETA[1])*depot", "d/dt(center)=exp(THETA[1])*depot-exp(THETA[2])*center")
    for (.nm in c("rx_expr_c1_0", "rx__disg_ic__depot__", "rx__disg_mod__f__depot__")) {
      .m <- paste(c(.ode, paste0(.nm, "=exp(THETA[3])"), .pad, paste0("cp=center*", .nm)), collapse = "\n")
  ...
  test_that("only the rx_expr_ names a chunk introduces are renamed", {
    ...
    .m <- paste(c(.base, "my_rx_expr_var=exp(THETA[3])", .pad, "cp=center*my_rx_expr_var"), collapse = "\n")
  ...
  test_that(".rxRealignPastTau() only ever rewrites the duration it is sure of", {
    ...
    .two0 <- paste(
      c("d/dt(G)=-delay(G, exp(lT))-delay(G, 2*lT)", "past(G,exp(lT))=2", "past(G,2*lT)=3"),
      collapse = "\n"
    )
    .two1 <- paste(
      c(
    ...
    expect_identical(.pastOf(.r(.two1, .two0)), c("past(G,rx_expr_0)=2", "past(G,rx_expr_1)=3"))
    ...
    .mrg0 <- paste(c("d/dt(G)=-delay(G,1+1)-delay(G,2)", "past(G,1+1)=2", "past(G,2)=3"), collapse = "\n")
    .mrg1 <- paste(
      c("rx_expr_0~2", "d/dt(G)=-delay(G,rx_expr_0)-delay(G,rx_expr_0)", "past(G,1+1)=2", "past(G,2)=3"),
      collapse = "\n"
    )
    ...
    expect_identical(
      .pastOf(.r(.dot, "past(.y.1,exp(lT))=2\nd/dt(.y.1)=-delay(.y.1,exp(lT))\n")),
    ...
    .dup1 <- paste(
      c("rx_expr_0~exp(lT)", "d/dt(G)=-delay(G, rx_expr_0)-delay(G, rx_expr_0)", "past(G,exp(lT))=2"),
      collapse = "\n"
    )
    ...
    .co0 <- paste(
      c(
        "d/dt(G)=-delay(G, exp(lT))-delay(G,",
        "tau2)-delay(G, exp(lT)*1)",
        "past(G, exp(lT))=1",
        "past(G, tau2)=2",
        "past(G, exp(lT)*1)=3"
      ),
    ...
    .cm0 <- paste(c("d/dt(G)=-delay(G,exp(lT)) # delay(G,junk)", "past(G,exp(lT))=2"), collapse = "\n")
    .cm1 <- paste(
      c("rx_expr_0~exp(lT)", "d/dt(G)=-delay(G,rx_expr_0) # delay(G,junk)", "past(G,exp(lT))=2"),
      collapse = "\n"
    )
    ...
    expect_identical(
      .pastOf(.r(.cm2, "d/dt(G)=-delay(G,exp(lT)) # (see above\npast(G,exp(lT))=2")),
      "past(G,rx_expr_0)=2"
    )
    ...
    .sd0 <- paste(
      c("OCC=\"first\"", "d/dt(I)=delay(G, 1+(OCC==\"first\"))", "past(G, 1+(OCC==\"first\"))=2"),
      collapse = "\n"
    )
    .sd1 <- paste(
      c(
        "OCC=\"first\"",
        "rx_expr_0~1+(OCC==\"first\")",
        "d/dt(I)=delay(G, rx_expr_0)",
        "past(G, 1+(OCC==\"first\"))=2"
      ),
      collapse = "\n"
    )
    ...
    expect_identical(.rxCodeOnly("d/dt(G)=-delay(G,rx_expr_0)"), "d/dt(G)=-delay(G,rx_expr_0)")
  })
  ...
  test_that("the chunked path rejects a wrong duration the way the whole model does", {
    ...
    .cm <- suppressMessages(rxOptExpr(.mod("exp(lT)", "delay(G,exp(lT)) # delay(G,junk)"), "m", chunkLines = 15L))
    ...
    expect_error(
      .rxValidatePast(rxModelVars(suppressMessages(
  ...
  test_that("an unsupported lhs names itself and does not print (#1192)", {
    ...
    expect_error(..rxOptLhs(quote(foo(bar, baz))), "foo(bar, baz)", fixed = TRUE)
