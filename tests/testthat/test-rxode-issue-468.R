rxTest({
  test_that("nocb infusion rxode2 vs NONMEM; Issue RxODE#468", {
    ...
    rx_302_1 <- rxode2::rxSolve(mod_run30x, eta_302_1, pat_302_1, covsInterpolation = "nocb", addDosing = TRUE)

    # Oral to depot
    pat_301_1 <- data.frame(
      ID = 1,
      TIME = c(
        0.00000,
        1.50000,
    ...
    rx_301_1 <- rxode2::rxSolve(mod_run30x, eta_301_1, pat_301_1, covsInterpolation = "nocb", addDosing = TRUE)

    # import NONMEM output
    lst <- readRDS("test-issue-468.rds")
    ...
    simu_30x <- rbind(
      cbind(rx_301_1[, c("time", "IPRED")], run = "oral", solver = "rxode2"),
      cbind(rx_302_1[, c("time", "IPRED")], run = "IV", solver = "rxode2"),
      cbind(run301_tab[run301_tab$ID == 1, c("time", "IPRED")], run = "oral", solver = "NONMEM"),
      cbind(run302_tab[run302_tab$ID == 1, c("time", "IPRED")], run = "IV", solver = "NONMEM")
    )

    expect_equal(
      as.data.frame(rx_301_1[, c("time", "IPRED")]),
      as.data.frame(run301_tab[run301_tab$ID == 1, c("time", "IPRED")]),
      tolerance = 1e-5
    )

    expect_equal(
      as.data.frame(rx_302_1[, c("time", "IPRED")]),
