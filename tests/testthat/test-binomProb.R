rxTest({
  if (!.Call(`_rxode2_isIntel`)) {
    test_that("test binomProb()", {
      ...
      t1 <- c("2.5%" = z2.5[1], "5%" = z5[1], "50%" = m, "95%" = z5[2], "97.5%" = z2.5[2])
      ...
      expect_equal(binomProbs(x2, ciMethod = "wilsonCorrect"), setNames(rep(NA_real_, length(t1)), names(t1)))

      expect_equal(
        binomProbs(x2, onlyProbs = FALSE, ciMethod = "wilsonCorrect"),
