rxTest({
  test_that("batched multi-subject linCmt() + modeled alag() does not leak amounts between subjects (#1153)", {
    ...
    pars <- data.frame(tcl = log(3) + c(-0.5, 0, 0.5), tv = log(30), tka = log(1), tlag = log(0.7))

    # reference: each subject solved alone
    ref <- unlist(lapply(seq_len(nrow(pars)), function(i) {
      rxSolve(mod, params = pars[i, ], events = ev, returnType = "data.frame", addDosing = FALSE)$cp
    }))
    ...
    for (cr in 1:2) {
      o <- rxSolve(mod, params = pars, events = ev, cores = cr, returnType = "data.frame", addDosing = FALSE)
  ...
  test_that("nonzero linCmt() initial amounts via inits= are still honored", {
    ...
    s <- rxSolve(mod, events = ev, inits = c(central = 50), returnType = "data.frame")
