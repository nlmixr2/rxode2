rxTest({
  ...
  test_that("the shape derivatives match a high accuracy numeric derivative", {
    ...
    for (.a in c(0.5, 2, 11.1)) {
      for (.z in c(0.3, 2, 10)) {
        expect_equal(gammapDera(.a, .z), .nd(function(aa) pgamma(.z, shape = aa), .a), tolerance = 1e-6)
      }
    }
    expect_equal(ibetaDera(2.3, 4.1, 0.4), .nd(function(aa) pbeta(0.4, aa, 4.1), 2.3), tolerance = 1e-6)
    expect_equal(ibetaDerb(2.3, 4.1, 0.4), .nd(function(bb) pbeta(0.4, 2.3, bb), 4.1), tolerance = 1e-6)
    expect_equal(studentTCdfDnu(1.3, 6), .nd(function(v) pt(1.3, v), 6), tolerance = 1e-6)
  })
  ...
  test_that("the derivative table is complete for the inverse CDFs", {
    ...
    expect_equal(
      rxFromSE("Derivative(gammapInv(a, p), p)", unknownDerivatives = "error"),
      "1/gammapDer(a,gammapInv(a,p))"
    )
    for (.e in c(
      "Derivative(gammapInv(a, p), a)",
      ...
      expect_true(nzchar(do.call(rxFromSE, list(.e, unknownDerivatives = "error"))), info = .e)
    }
  })
  ...
  test_that("the eta transform differentiates exactly through symengine", {
    ...
    .chk <- function(model, var, fnum, at, tol = 1e-5) {
      ...
      expect_equal(
        .an,
        (do.call(fnum, .p) - do.call(fnum, .m)) / (2 * .h),
        tolerance = tol,
  ...
  test_that("every declarable family's quantile template is right", {
    ...
    .ref <- list(
      ...
      doubleExponential = function(u) 0.3 + 1.1 * ifelse(u < 0.5, log(2 * u), -log(2 * (1 - u))),
    ...
    for (.i in seq_len(nrow(.tab))) {
      ...
      expect_equal(.an, (.f(0.6 + 1e-6) - .f(0.6 - 1e-6)) / 2e-6, tolerance = 1e-5, info = .nm)
    }
  })
  ...
  test_that("the densities differentiate, so a second derivative stays exact", {
    ...
    for (.a in c(0.5, 2, 11.1)) {
      for (.z in c(0.3, 2, 10)) {
        expect_equal(gammapDer(.a, .z) * ((.a - 1) / .z - 1), .nd(function(zz) gammapDer(.a, zz), .z), tolerance = 1e-6)
        expect_equal(
          gammapDer(.a, .z) * (log(.z) - digamma(.a)),
          .nd(function(aa) gammapDer(aa, .z), .a),
          tolerance = 1e-6
        )
      }
    }
    expect_equal(
      ibetaDer(2.3, 4.1, 0.4) * ((2.3 - 1) / 0.4 - (4.1 - 1) / (1 - 0.4)),
      .nd(function(xx) ibetaDer(2.3, 4.1, xx), 0.4),
      tolerance = 1e-6
    )
    expect_equal(
      ibetaDer(2.3, 4.1, 0.4) * (log(0.4) - digamma(2.3) + digamma(2.3 + 4.1)),
      .nd(function(aa) ibetaDer(aa, 4.1, 0.4), 2.3),
      tolerance = 1e-6
    )
    expect_equal(
      ibetaDer(2.3, 4.1, 0.4) * (log(1 - 0.4) - digamma(4.1) + digamma(2.3 + 4.1)),
      .nd(function(bb) ibetaDer(2.3, bb, 0.4), 4.1),
      tolerance = 1e-6
    )
    expect_equal(
      -studentTDen(1.3, 6) * 7 * 1.3 / (6 + 1.3^2),
      .nd(function(xx) studentTDen(xx, 6), 1.3),
      tolerance = 1e-6
    )
  })
  ...
  test_that("the eta transform's SECOND derivative is exact too", {
    ...
    .chk <- function(model, f) {
      ...
      for (.z in c(-2.5, -0.4, 0.9, 2.1)) {
        ...
        .h <- 1e-4
        expect_equal(
          eval(parse(text = .e2), envir = .env),
          (f(.z + .h) - 2 * f(.z) + f(.z - .h)) / .h^2,
          tolerance = 1e-4,
          info = paste0(model, " at z=", .z)
        )
      }
    }
    .chk("cl=gammapInv(a, phiU(z))/b", function(z) gammapInv(11.1, phiU(z)) / 2.22)
    .chk("v=studentTInv(phiU(z), nu)", function(z) studentTInv(phiU(z), 6))
    .chk("w=ibetaInv(s1, s2, phiU(z))", function(z) ibetaInv(2.3, 4.1, phiU(z)))
  })
  ...
  test_that("a declared distribution implies its unit variance", {
    ...
    .txt <- paste(vapply(rxEtaDistExpand(.u)$lstExpr, deparse1, character(1)), collapse = "\n")
  ...
  test_that("simulation recovers the declared marginals and the copula", {
    ...
    for (.rv in c(0.09, 0.5, 1.0, 2.0)) {
      rxSetSeed(1042)
      .s <- rxSolve(.gammaMod(.rv), .ev, nSub = 4000, returnType = "data.frame", addDosing = FALSE)
      ...
      expect_gt(
        suppressWarnings(
          stats::ks.test(.s1$cl, "pgamma", shape = .a, rate = .b1)
        )$p.value,
        0.001
      )
      expect_gt(
        suppressWarnings(
          stats::ks.test(.s1$v1, "pgamma", shape = .a, rate = .b2)
        )$p.value,
        0.001
      )
      ## the correlation is a Gaussian copula, so it is exact on the
      ## normal-score scale, not on the gamma scale
      expect_equal(stats::cor(qnorm(pgamma(.s1$cl, .a, .b1)), qnorm(pgamma(.s1$v1, .a, .b2))), 0.5, tolerance = 0.05)
    }
  })
  ...
  test_that("dist() pipes on and off a model", {
    ...
    expect_equal(rxUiEtaDists(.w)$etaDist[rxUiEtaDists(.w)$name == "eta.cl"], "dexp(1/exp(lclm))")
