rxTest({
  ...
  test_that("a prior can be piped onto a covariance block", {
    ...
    expect_true(any(grepl("prior(eta.cl, eta.v) ~ invWishart(4)", deparse(.u$iniFun), fixed = TRUE)))
  })
  ...
  test_that("a piped prior is validated the same as one in the block", {
    ...
    expect_error(ini(.u, prior(tka) ~ invWishart(4)), "covariance matrix")
    ## a correlation prior needs a block of more than one
    expect_error(ini(.u, prior(eta.cl) ~ lkjCorr(2)), "more than one parameter")
  })
  ...
  test_that("a piped prior survives printing and re-parsing", {
    ...
    .again <- eval(.u$iniFun)
    expect_equal(as.data.frame(.again)$prior[c(1, 5)], c("dnorm(0, 10)", "invWishart(4)"))
  })
})
