rxTest({
  ...
  .rxS3Registered <- function(pkg, generic, class) {
    ...
    .tbl <- try(get(".__S3MethodsTable__.", envir = .defenv, inherits = FALSE), silent = TRUE)
  ...
  .rxS3Src <- paste(
    deparse(body(rxode2:::.onLoad), width.cutoff = 500L), # nolint
    collapse = "\n"
  )
  .rxS3Found <- regmatches(
    .rxS3Src,
    gregexec('\\.s3register\\("([^":]+)::([^"]+)", *"([^"]+)"\\)', .rxS3Src)
  )[[1]]
  .rxS3Cases <- if (length(.rxS3Found) == 0L) {
    list()
  } else {
    apply(.rxS3Found, 2L, function(.m) list(pkg = .m[2], generic = .m[3], class = .m[4]))
  }
  ...
  test_that("every .onLoad() registration is covered below", {
    ...
    expect_false(any(vapply(.rxS3Cases, function(.c) anyNA(unlist(.c)), logical(1))))
  })
  ...
  for (.case in .rxS3Cases) {
    test_that(sprintf("%s.%s is registered for %s", .case$generic, .case$class, .case$pkg), {
