rxTest({
  ...
  test_that("discrete adjoint of a trajectory objective matches forward sens to machine precision", {
    ...
    gFwd <- Reduce(
      `+`,
      lapply(seq_along(obsSteps), function(i) {
  ...
  test_that("discrete adjoint with an additive-bolus (F) dose jump matches forward sens to machine precision", {
    ...
    dcs <- c("ka", "cl", "v", "Fbio")
    dp <- c(ka = 1.2, cl = 3.5, v = 25, Fbio = 0.7)
  ...
  test_that(".rxAdjointExpand exposes F_X/F_p as lhs matching the reference to machine precision", {
    ...
    expect_equal(ex$ns, 2L)
    expect_equal(ex$np, 3L)
    expect_equal(ex$fxOff, 0L)
    expect_equal(ex$fpOff, 4L)
    expect_equal(ex$nlhsAdj, 10L)
    m <- rxode2::rxode2(ex$text)
    mv <- rxode2::rxModelVars(m)
    # first nlhsAdj lhs are the adjoint expansion, in the documented order
    expect_equal(
      mv$lhs[seq_len(ex$nlhsAdj)],
      c(
        "rx__adjFX_0_0__",
        "rx__adjFX_0_1__",
        "rx__adjFX_1_0__",
    ...
    sd <- as.data.frame(s)
    fx <- matrix(0, ex$ns, ex$ns)
    fp <- matrix(0, ex$ns, ex$np)
    for (i in seq_len(ex$ns)) {
      for (j in seq_len(ex$ns)) {
        fx[i, j] <- sd[[sprintf("rx__adjFX_%d_%d__", i - 1L, j - 1L)]]
      }
    }
    for (i in seq_len(ex$ns)) {
      for (pp in seq_len(ex$np)) {
  ...
  test_that("in-engine rk4s solver fills rx__sens_* columns matching FD of the RK4 solve", {
    ...
    solveBase <- function(pp) {
      as.matrix(as.data.frame(
        rxode2::rxSolve(mbase, ev, params = pp, method = "rk4", cores = 1)
      )[, c("depot", "center")])
    }
    b0 <- solveBase(p)
  ...
  test_that("steady-state (ss) bolus: forward-only adjoint primal matches base to machine precision", {
    ...
    base <- as.matrix(as.data.frame(
      rxode2::rxSolve(mbase, evSS, params = p, method = "rk4", cores = 1)
    )[, c("depot", "center")])
    mplain <- rxode2::rxode2(mText)
    fo <- as.matrix(as.data.frame(
      rxode2::rxSolve(mplain, evSS, params = p, method = "rk4s", cores = 1)
    )[, c("depot", "center")])
      ...
      for (pn in cs) {
        for (st in ex$st) {
          mx <- max(
            mx,
            max(abs(
              a[[sprintf("rx__sens_%s_BY_%s__", st, pn)]] -
  ...
  test_that("steady-state (ss): FD cross-check of the expanded rk4s adjoint sensitivities", {
    ...
    solveBase <- function(pp) {
      as.matrix(as.data.frame(
        rxode2::rxSolve(mbase, evSS, params = pp, method = "rk4", cores = 1)
      )[, c("depot", "center")])
    }
    fdmax <- 0
    for (pn in cs) {
  ...
  test_that("steady-state ss=2 (superposition, bolus): adjoint sensitivities match forward", {
    ...
    chk2 <- function(ev, adjMeth, fwdMeth, tol) {
      ...
      mx <- 0
      for (cn in scol) {
        mx <- max(mx, max(abs(a[[cn]] - f[[cn]]), na.rm = TRUE))
      }
    ...
    evA <- et(amt = 100, cmt = "depot") %>%
      et(amt = 50, cmt = "depot", ss = 2, ii = 12, time = 12) %>%
      et(c(1, 4, 8, 12, 16, 20, 24))
    chk2(evA, "rk4s", "rk4", 1e-5)
    chk2(evA, "vern98s", "vern98", 1e-5)
    # two ss=2 superposition events (t=12 and t=36)
    evB <- et(amt = 100, cmt = "depot") %>%
      et(amt = 50, cmt = "depot", ss = 2, ii = 12, time = 12) %>%
      et(amt = 50, cmt = "depot", ss = 2, ii = 12, time = 36) %>%
      et(c(4, 12, 24, 36, 48))
    ...
    evC <- et(amt = 100, cmt = "depot", ss = 1, ii = 24) %>%
      ...
      et(amt = 50, cmt = "depot", ss = 2, ii = 24, time = 36) %>%
      et(c(4, 12, 20, 24, 30, 36, 44))
    ...
    evD <- et(amt = 100, cmt = "central") %>%
      et(amt = 50, rate = 10, cmt = "central", ss = 2, ii = 12, time = 12) %>%
      et(c(1, 8, 12, 16, 24, 30))
    ...
    evE <- et(amt = 80, cmt = "central") %>%
      et(amt = 100, rate = 10, cmt = "central", ss = 2, ii = 8, time = 8) %>%
      et(c(1, 8, 12, 16, 24, 30))
    ...
    evF <- et(amt = 80, cmt = "central") %>%
      et(amt = 120, rate = 10, cmt = "central", ss = 2, ii = 12, time = 12) %>%
      et(c(1, 8, 12, 16, 24, 30))
  ...
  test_that("steady-state on the composite (AutoSwitch) path: adjoint sensitivities match forward", {
    ...
    chkC(
      et(amt = 100, cmt = "depot", ss = 1, ii = 24) %>%
        et(amt = 100, cmt = "depot", ss = 1, ii = 24, time = 24) %>%
        et(c(1, 12, 20, 24, 30, 44)),
      1e-4
    ) # interior ss1 reset
  })
  ...
  test_that("steady-state (ss): not-yet-covered cases and drivers stay guarded", {
    ...
    expect_error(
      rxode2::rxSolve(
        mMdl,
        et(amt = 100, rate = -2, cmt = "center", ss = 1, ii = 24) %>% et(c(1, 8, 16)),
        params = p,
        method = "dop853s+ros4s",
        cores = 1
      ),
      "steady-state"
    ) # composite guarded
    expect_error(rxode2::rxSolve(
      mMdl,
      et(amt = 100, rate = -2, cmt = "center", ss = 1, ii = 8) %>% et(c(1, 4, 8)),
      params = p,
      method = "rk4s",
      cores = 1
    )) # large-dur modeled -> errors
    # liblsodaadj covers single ss==1 (below) but NOT ss==2 or interior/multiple ss==1
    expect_error(
      rxode2::rxSolve(
        madj,
        et(amt = 100, cmt = "depot", ss = 2, ii = 12) %>% et(c(1, 2, 4)),
        params = p,
        method = "liblsodaadj",
        cores = 1
      ),
  ...
  test_that("modeled rate()/dur() steady-state: adjoint sensitivities match FINITE DIFFERENCES", {
    ...
    csM <- c("ka", "cl")
    pM <- c(ka = 1.2, cl = 3.5, v = 25)
    chkFD <- function(mt, ev, tol) {
      exM <- rxode2::.rxAdjointExpand(mt, csM)
      madj <- rxode2::rxode2(exM$text)
      mb <- rxode2::rxode2(mt)
      a <- as.data.frame(suppressWarnings(rxode2::rxSolve(
        madj,
        ev,
        params = pM,
        method = "rk4s",
      ...
      for (pn in csM) {
        ...
        pp[pn] <- pp[pn] + hh
        pm[pn] <- pm[pn] - hh
        sp <- as.data.frame(suppressWarnings(rxode2::rxSolve(
          mb,
          ev,
          params = pp,
          method = "rk4",
        ...
        sm <- as.data.frame(suppressWarnings(rxode2::rxSolve(
          mb,
          ev,
          params = pm,
          method = "rk4",
  ...
  test_that("nmtest dosing scenarios: adjoint solution AND gradients match forward sensitivities (ka + elimination)", {
    ...
    ncs <- c("ka", "cl")
    pp <- c(ka = 1.5, cl = 1.1, v = 20)
    nex <- rxode2::.rxAdjointExpand(mt, ncs)
    nmadj <- rxode2::rxode2(nex$text)
    ...
    for (mp in list(
      ...
      for (id in unique(d0$id)) {
        di <- d0[d0$id == id, ]
        if (any(di$evid != 0 & !(di$cmt %in% c(1, 2)))) {
          next
        }
        a <- tryCatch(
          as.data.frame(suppressWarnings(rxode2::rxSolve(
            nmadj,
            di,
            params = pp,
            method = mp[1],
        ...
        if (is.null(a)) {
          next
        } # guarded / not-yet-supported ss case
        ...
        pd <- 0
        for (st in nex$st) {
          pd <- max(pd, max(abs(a[[st]] - f[[st]]), na.rm = TRUE))
        }
        sd <- 0
        for (cn in ncols) {
          sd <- max(sd, max(abs(a[[cn]] - f[[cn]]), na.rm = TRUE))
        }
  ...
  test_that("STIFF adjoint solvers (Rosenbrock/implicit): non-ss nmtest gradients match FD", {
    ...
    ncs <- c("ka", "cl")
    pp <- c(ka = 1.5, cl = 1.1, v = 20)
    nex <- rxode2::.rxAdjointExpand(mt, ncs, stiff = TRUE)
    nmadj <- rxode2::rxode2(nex$text)
    mb <- rxode2::rxode2(mt)
    ...
    for (id in unique(d0$id)) {
      ...
      for (pn in ncs) {
        ...
        for (st in nex$st) {
          fd[[sprintf("rx__sens_%s_BY_%s__", st, pn)]] <- (sp[[st]] - sm[[st]]) / (2 * hh)
        }
      }
      for (meth in stiff) {
        a <- as.data.frame(suppressWarnings(rxode2::rxSolve(
        ...
        mx <- 0
        for (cn in scol) {
          mx <- max(mx, max(abs(a[[cn]] - fd[[cn]]), na.rm = TRUE))
        }
  ...
  test_that("STIFF adjoint solvers: steady-state sens match forward, using the ANALYTIC Jacobian", {
    ...
    chkS(
      et(amt = 100, cmt = "center") %>%
        et(amt = 50, cmt = "center", ss = 2, ii = 12, time = 12) %>%
        et(c(4, 12, 24)),
      1e-4
    ) # ss=2 superposition
  })
  ...
  test_that("in-engine rk4s F/dose-jump: Fbio + all sens columns match FD of the RK4 solve", {
    ...
    fcs <- c("ka", "cl", "v", "Fbio")
    fp <- c(ka = 1.2, cl = 3.5, v = 25, Fbio = 0.7)
    tms <- c(1, 2, 4, 8, 12, 24)
    ev <- et(amt = 100, cmt = "depot") %>% et(tms)
  ...
  test_that("discrete-adjoint event jumps (reset/replace/multiply) match FD of the RK4 solve", {
    ...
    skip_on_ci()
    cs2 <- c("ka", "cl", "v")
    p2 <- c(ka = 1.2, cl = 3.5, v = 25)
    ex <- rxode2::.rxAdjointExpand(mText, cs2)
    madj <- rxode2::rxode2(ex$text)
    mbase <- rxode2::rxode2(mText)
    solveBase <- function(pp, ev) {
      as.matrix(as.data.frame(
        rxode2::rxSolve(mbase, ev, params = pp, method = "rk4", cores = 1)
      )[, c("depot", "center")])
    }
    evs <- list(
      reset = et(amt = 100, cmt = "depot") %>%
        et(4, evid = 3) %>%
        et(amt = 100, cmt = "depot", time = 4) %>%
        et(c(1, 2, 6, 8, 12)),
      replace = et(amt = 100, cmt = "depot") %>%
        et(amt = 40, cmt = "center", evid = 5, time = 4) %>%
        et(c(1, 2, 6, 8, 12)),
      multiply = et(amt = 100, cmt = "depot") %>%
        et(amt = 0.5, cmt = "center", evid = 6, time = 4) %>%
        et(c(1, 2, 6, 8, 12))
    )
  ...
  test_that("modeled-alag transversality: d/d(lag) sensitivity matches FD (rk4s/dop853s)", {
    ...
    cs2 <- c("ka", "cl", "v", "tlag")
    p2 <- c(ka = 1.2, cl = 3.5, v = 25, tlag = 0.7)
    ex <- rxode2::.rxAdjointExpand(mText2, cs2)
    madj <- rxode2::rxode2(ex$text)
    mbase <- rxode2::rxode2(mText2)
    ev <- et(amt = 100, cmt = "depot") %>% et(c(1, 2, 4, 8, 12))
    solveBase <- function(pp, meth) {
      as.matrix(as.data.frame(rxode2::rxSolve(
        mbase,
        ev,
        params = pp,
        method = if (meth == "cvodesadj") "liblsoda" else "rk4",
        atol = 1e-11,
        rtol = 1e-11,
        cores = 1
      ))[, c("depot", "center")])
    ...
    for (meth in c("rk4s", "dop853s", "radauiia5s", "ros4s", "cvodesadj")) {
      ...
      s <- as.data.frame(rxode2::rxSolve(madjM, ev, params = p2, method = meth, atol = 1e-11, rtol = 1e-11, cores = 1))
  ...
  test_that("modeled-rate infusion dual: d/d(rate) sensitivity matches FD (rk4s/dop853s)", {
    ...
    mText2 <- "d/dt(depot)=-ka*depot\nd/dt(center)=ka*depot-(cl/v)*center\nrate(depot)=Rin"
    cs2 <- c("ka", "cl", "v", "Rin")
    p2 <- c(ka = 1.2, cl = 3.5, v = 25, Rin = 40)
    ex <- rxode2::.rxAdjointExpand(mText2, cs2)
    madj <- rxode2::rxode2(ex$text)
    mbase <- rxode2::rxode2(mText2)
  ...
  test_that("modeled-dur infusion dual: d/d(dur) sensitivity matches FD (all methods)", {
    ...
    mText2 <- "d/dt(depot)=-ka*depot\nd/dt(center)=ka*depot-(cl/v)*center\ndur(depot)=Dur"
    cs2 <- c("ka", "cl", "v", "Dur")
    p2 <- c(ka = 1.2, cl = 3.5, v = 25, Dur = 2.5)
    ex <- rxode2::.rxAdjointExpand(mText2, cs2)
    madj <- rxode2::rxode2(ex$text)
    mbase <- rxode2::rxode2(mText2)
  ...
  test_that("in-engine rk4s population solve (parallel) matches per-subject FD", {
    ...
    for (i in 1:3) {
      ...
      solveB <- function(pp) {
        as.matrix(as.data.frame(
          rxode2::rxSolve(mbase, evi, params = pp, method = "rk4", cores = 1)
        )[, c("depot", "center")])
      }
      si <- s[s$id == i, ]
      for (pn in cs2) {
  ...
  test_that("rxSolveAdjointRk4 wrapper: clean full-trajectory gradient + cache", {
    ...
    expect_true(all(vapply(
      ...
      function(pn) {
        all(vapply(
          c("depot", "center"),
          function(st) {
            sprintf("rx__sens_%s_BY_%s__", st, pn) %in% names(df)
          },
          logical(1)
        ))
      },
    ...
    expect_identical(rxode2::.rxAdjointModel(mText, cs)$model, rxode2::.rxAdjointModel(mText, cs)$model)
  })
  ...
  test_that("stiff ros4s (Rosenbrock) adjoint is exact for a linear model (dJ/dp term)", {
    ...
    expect_true(ex$stiff)
    expect_gt(ex$jpOff, 0)
    madj <- rxode2::rxode2(ex$text)
  ...
  test_that("DDE delay() adjoint converges to FD for explicit rk4s methods", {
    ...
    expect_true(exD$hasDelay)
    expect_gt(exD$fxdOff, 0)
    expect_gt(exD$tauOff, exD$fxdOff)
    madjD <- rxode2::rxode2(exD$text)
    pD <- c(k = 0.3, tau = 2)
    evD <- et(amt = 10, cmt = "central") %>% et(seq(0, 20, by = 2))
    errAt <- function(h) {
      sc <- as.data.frame(rxode2::rxSolve(madjD, evD, params = pD, method = "rk4s", hmin = h, cores = 1))
      sb <- function(pp) {
        as.data.frame(rxode2::rxSolve(madjD, evD, params = pp, method = "rk4s", hmin = h, cores = 1))$central
      }
      hh <- pD[["k"]] * 1e-6
      pp <- pD
      pm <- pD
    ...
    e3 <- errAt(0.005)
    expect_lt(e2, e1)
    expect_lt(e3, e2) # converges as h shrinks
  ...
  test_that("param-dependent delay tau(p): adjoint d/dtau matches FD incl. dose-induced jump", {
    ...
    errAt <- function(h) {
      ...
      sb <- function(pp) {
        as.data.frame(rxode2::rxSolve(madjD, ev, params = pp, method = "rk4s", hmin = h, cores = 1))$central
      }
      hh <- p[["tau"]] * 5e-4
      pp <- p
      pm <- p
    ...
    errAt <- function(method, ctl) {
      ...
      sb <- function(pp) {
        as.data.frame(do.call(
          rxode2::rxSolve,
          c(list(madjD, evD, params = pp, method = method, cores = 1), ctl)
        ))$central
      }
      hh <- pD[["k"]] * 1e-6
      pp <- pD
  ...
  test_that("adaptive methods: primal matches liblsoda and frozen-step adjoint matches FD", {
    ...
    madj <- rxode2::rxode2(ex$text)
    mbase <- rxode2::rxode2(mText)
    ref <- as.data.frame(rxode2::rxSolve(mbase, ev, params = p, method = "liblsoda", atol = 1e-11, rtol = 1e-11))
    # per-method (tolerance, primal-tol) -- lower-order pairs need looser primal
    cfg <- list(
      dop5s = c(1e-11, 1e-5),
      dop853s = c(1e-11, 1e-5),
      ck54s = c(1e-11, 1e-5),
      bs32s = c(1e-8, 1e-4),
      rk43s = c(1e-9, 1e-4),
      vern65s = c(1e-11, 1e-5),
    ...
    for (meth in names(cfg)) {
      tl <- cfg[[meth]][1]
      ptol <- cfg[[meth]][2]
      sd <- as.data.frame(rxode2::rxSolve(madj, ev, params = p, method = meth, atol = tl, rtol = tl, cores = 1))
  ...
  test_that("table-driven fixed-step methods (eulers/midpoints/heuns/rk3s) match FD of their own map", {
    ...
    for (meth in c("eulers", "midpoints", "heuns", "rk3s")) {
      ...
      solveB <- function(pp) {
        as.matrix(as.data.frame(
          rxode2::rxSolve(madj, ev, params = pp, method = meth, cores = 1)
        )[, c("depot", "center")])
      }
      fdmax <- 0
      for (pn in cs) {
  ...
  test_that("in-engine cvodesadj fills rx__sens_* matching FD across dose types", {
    ...
    for (nm in names(evs)) {
      ...
      sd <- as.data.frame(rxode2::rxSolve(
        madj,
        ev,
        params = p2,
        method = "cvodesadj",
  ...
  test_that("cvodesadj reset (evid 3) costate jump: post-reset sens matches FD", {
    ...
    ev0 <- et(amt = 100, cmt = "depot") %>% et(4, evid = 3) %>% et(c(1, 2, 6, 8, 12))
    s0 <- as.data.frame(rxode2::rxSolve(
      madj,
      ev0,
      params = p2,
      method = "cvodesadj",
  ...
  test_that("cvodesadj population solve matches the dop853s discrete adjoint", {
    ...
    ev <- do.call(
      rbind,
      lapply(1:3, function(i) {
        e <- et(amt = 100, cmt = "depot", ii = 12, addl = 1) %>% et(c(1, 4, 12, 20))
        e$id <- i
        e
      })
    )
  ...
  test_that("adjoint auto-switch: base methods produce real sensitivities (cvode->cvodesadj)", {
    ...
    for (pn in cs2) {
      ...
      fd <- (solveBase(pp) - solveBase(pm)) / (2 * hh)
      for (k in seq_along(ex$st)) {
        fdref[[paste0(k, pn)]] <- fd[, k]
      }
    }
    for (meth in c("cvode", "rk4", "dop853", "liblsoda")) {
      s <- as.data.frame(rxode2::rxSolve(madj, ev, params = p2, method = meth, atol = 1e-11, rtol = 1e-11, cores = 1))
      m <- 0
      for (pn in cs2) {
        for (k in seq_along(ex$st)) {
  ...
  test_that("in-engine liblsodaadj (P1-P3) == discrete forward sensitivity of liblsoda's own step map", {
    ...
    selfcheck(
      c(ka = 200, cl = 1, v = 10),
      list(maxordn = 5L, maxords = 5L, atol = 1e-9, rtol = 1e-9),
      1e-6,
      minSwitch = 1L
    )

    # primal identical to plain liblsoda on the same fixed step (P1 config)
    sd <- as.data.frame(do.call(
      rxode2::rxSolve,
      c(list(
        madj,
        ev,
    ...
    solveBase <- function(pp) {
      as.matrix(as.data.frame(do.call(
        rxode2::rxSolve,
        c(list(
          mbase,
          ev,
  ...
  test_that("in-engine liblsodaadj (P5) interior event jumps: multi-dose / reset / replace / multiply", {
    ...
    for (nm in names(evs)) {
      ...
      sL <- as.data.frame(rxode2::rxSolve(
        madj,
        ev,
        params = p,
        method = "liblsodaadj",
    ...
    sB <- as.data.frame(rxode2::rxSolve(
      madj,
      evBare,
      params = p,
      method = "liblsodaadj",
  ...
  test_that("in-engine liblsodaadj (P5) modeled bioavailability F and lag-time alag jumps", {
    ...
    cases <- list(
      Fsingle = list(
        ...
        cs = c("ka", "cl", "v", "Fbio"),
        p = c(ka = 1.2, cl = 3.5, v = 25, Fbio = 0.7),
      ...
      alag = list(
        m = "d/dt(depot)=-ka*depot\nd/dt(center)=ka*depot-(cl/v)*center\nalag(depot)=tlag",
        cs = c("ka", "cl", "v", "tlag"),
        p = c(ka = 1.2, cl = 3.5, v = 25, tlag = 0.7),
        ev = et(et(amt = 100, cmt = "depot"), c(1, 2, 4, 8, 12))
      )
    )
    for (nm in names(cases)) {
      cc <- cases[[nm]]
      ex <- rxode2::.rxAdjointExpand(cc$m, cc$cs)
      madj <- rxode2::rxode2(ex$text)
      Sys.setenv(RX_LSADJ_SELFCHECK = "1")
      msg <- capture.output(
        sL <- as.data.frame(rxode2::rxSolve(
  ...
  test_that("in-engine liblsodaadj (P5) infusions: constant rate + modeled rate()/dur() duals", {
    ...
    cases <- list(
      const = list(
        ...
        cs = c("ka", "cl", "v"),
        p = c(ka = 1.2, cl = 3.5, v = 25),
        ev = et(et(amt = 100, cmt = "center", rate = 50), obs),
        gp = NULL
      ),
      mrate = list(
        m = "d/dt(depot)=-ka*depot\nd/dt(center)=ka*depot-(cl/v)*center\nrate(center)=Rin",
        cs = c("ka", "cl", "v", "Rin"),
        p = c(ka = 1.2, cl = 3.5, v = 25, Rin = 40),
        ev = et(et(amt = 100, cmt = "center", rate = -1), obs),
        gp = "Rin"
      ),
      mdur = list(
        m = "d/dt(depot)=-ka*depot\nd/dt(center)=ka*depot-(cl/v)*center\ndur(center)=Dur",
        cs = c("ka", "cl", "v", "Dur"),
        p = c(ka = 1.2, cl = 3.5, v = 25, Dur = 3),
        ev = et(et(amt = 100, cmt = "center", rate = -2), obs),
        gp = "Dur"
      )
    )
    for (nm in names(cases)) {
  ...
  test_that("in-engine liblsodaadj (P6) auto-switch from liblsoda + parallel across subjects", {
    ...
    sAuto <- as.data.frame(rxode2::rxSolve(
      madj,
      ev,
      params = p,
      method = "liblsoda",
    ...
    sExpl <- as.data.frame(rxode2::rxSolve(
      madj,
      ev,
      params = p,
      method = "liblsodaadj",
  ...
  test_that("in-engine dop54s / dp54s / vern98s (explicit-RK tableau additions)", {
    ...
    solveBase <- function(pp, m) {
      as.matrix(as.data.frame(
        rxode2::rxSolve(mbase, ev, params = pp, method = m, cores = 1, atol = 1e-11, rtol = 1e-11)
      )[, ex$st])
    }
    for (info in list(c("dop54s", "dop54"), c("dp54s", "dop54"), c("vern98s", "vern98"))) {
      ...
      for (cn in grep("rx__sens", names(s), value = TRUE)) {
        expect_equal(sAuto[[cn]], s[[cn]])
      }
    }
  })
  ...
  test_that("in-engine libode explicit-RK family (discrete-adjoint tableau batch)", {
    ...
    bases <- c(
      ...
      "s1110a",
      "o129"
    ) # >16-stage (need enlarged rksTableau)
    ...
    baseSolve <- function(m) {
      as.matrix(as.data.frame(
        rxode2::rxSolve(mbase, ev, params = p, method = m, cores = 1, atol = 1e-11, rtol = 1e-11)
      )[, ex$st])
    }
    for (base in bases) {
  ...
  test_that("in-engine abs (Adams-Bashforth) discrete adjoint == FD of the AB solve", {
    ...
    for (ord in c(1L, 2L, 4L)) {
      ...
      for (ev in list(
        et(amt = 100, cmt = "depot") %>% et(c(1, 4, 8, 12)),
        ...
        solveAbs <- function(pp) {
          as.matrix(as.data.frame(
            do.call(rxode2::rxSolve, c(list(madj, ev, params = pp, method = "abs"), cc))
          )[, ex$st])
        }
        mx <- 0
        for (pn in cs) {
  ...
  test_that("in-engine abs interior event jumps (reset/replace/multiply) + parallel", {
    ...
    ex <- rxode2::.rxAdjointExpand(mText, cs)
    madj <- rxode2::rxode2(ex$text)
    ctl <- list(hmin = 0.02, maxordn = 4L, cores = 1)
    obs <- c(1, 2, 6, 8, 12)
    solveAbs <- function(pp, ev) {
      as.matrix(as.data.frame(
        do.call(rxode2::rxSolve, c(list(madj, ev, params = pp, method = "abs"), ctl))
      )[, ex$st])
    }
    evs <- list(
      reset = et(et(et(et(amt = 100, cmt = "depot"), 4, evid = 3), amt = 100, cmt = "depot", time = 4), obs),
    ...
    evp <- et(amt = 100, cmt = "depot", ii = 6, addl = 2) %>% et(c(1, 4, 7, 10, 16)) %>% et(id = 1:4)
    s1 <- as.data.frame(do.call(
      rxode2::rxSolve,
      c(list(madj, evp, pd, method = "abs"), list(hmin = 0.02, maxordn = 4L, cores = 1))
    ))
    s4 <- as.data.frame(do.call(
      rxode2::rxSolve,
      c(list(madj, evp, pd, method = "abs"), list(hmin = 0.02, maxordn = 4L, cores = 4))
    ))
    for (cn in grep("rx__sens", names(s1), value = TRUE)) {
      expect_equal(s1[[cn]], s4[[cn]])
    }
