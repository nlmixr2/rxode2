rxTest({
  ...
  ev <- rbind(
    data.frame(time = doseTimes, amt = 100, evid = 1, cmt = 1, wt = ifelse(doseTimes < 24, 70, 90)),
    data.frame(time = obsTimes, amt = 0, evid = 0, cmt = 1, wt = ifelse(obsTimes < 24, 70, 90))
  )
  ev <- ev[order(ev$time), ]
  ev$id <- 1
  solveIt <- function(eta) {
    rxSolve(
      mod,
      params = c(tcl = tclVal, tv = tvVal, eta.cl = eta),
      events = ev,
  ...
  test_that("an ordinary solve after the live-driven sentinels still works", {
    ...
    s <- rxSolve(m, c(cl = 1, v = 10), et(amt = 100) |> et(c(1, 2, 3.5)), returnType = "data.frame")
