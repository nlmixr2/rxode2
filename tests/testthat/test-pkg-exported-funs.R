...
rxTest({
  ...
  .writeMmPackage <- function(pkgDir) {
    ...
    writeLines("YEAR: 2026\nCOPYRIGHT HOLDER: Test Author", file.path(pkgDir, "LICENSE"))
  ...
  test_that("mm package: install, attach, verify integration, detach, verify cleanup", {
    ...
    .out <- system2(
      ...
      args = c("CMD", "INSTALL", "--no-multiarch", paste0("--library=", .libDir), .pkgDir),
      env = paste0("R_LIBS=", paste(c(.libDir, .libPaths()), collapse = .Platform$path.sep)),
      stdout = TRUE,
      stderr = TRUE
    )
    ...
    expect_equal(sort(.mm$rxFun), sort(c("mm", "mm_dC", "mm_dVmax", "mm_dKm")))
    ...
    expect_equal(rxode2::rxSolve(.uiNam, .evUi)$A1, rxode2::rxSolve(.uiPos, .evUi)$A1)
