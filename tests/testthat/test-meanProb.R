rxTest({
  if (!.Call(`_rxode2_isIntel`)) {
    test_that("test meanProb()", {
      ...
      t1 <- c(
        "0%" = mn,
        "25%" = m + (s / sqrt(10)) * qt(0.25, 9),
        "50%" = m,
        "75%" = m + (s / sqrt(10)) * qt(0.75, 9),
        "100%" = mx
      )
      ...
      t1.100 <- c(
        "0%" = mn,
        "25%" = m + (s / sqrt(100)) * qt(0.25, 99),
        "50%" = m,
        "75%" = m + (s / sqrt(100)) * qt(0.75, 99),
        "100%" = mx
      )
      ...
      t1.p100 <- c(
        "0%" = mn,
        "25%" = m + s * sqrt(1.0 + (1.0 / 100)) * qt(0.25, 99),
        "50%" = m,
        "75%" = m + s * sqrt(1.0 + (1.0 / 100)) * qt(0.75, 99),
        "100%" = mx
      )
      ...
      expect_equal(meanProbs(x2, onlyProbs = FALSE), setNames(rep(NA_real_, length(t2)), names(t2)))
      ...
      ci2 <- pk1 |>
        dplyr::group_by(time) |>
        dplyr::reframe(
          eff = meanProbs(blood, c(0.025, 0.5, 0.975), na.rm = TRUE, names = FALSE),
          p1 = c(0.025, 0.5, 0.975),
      ...
      ci2 <- pk1 |>
        dplyr::group_by(time) |>
        dplyr::reframe(
          eff = meanProbs(blood, c(0.025, 0.5, 0.975), na.rm = TRUE, names = FALSE, pred = TRUE),
          p1 = c(0.025, 0.5, 0.975),
          Percentile = c("2.5%", "50%", "97.5%")
        ) |>
        dplyr::arrange(time, p1)
      ...
      ci2 <- pk1 |>
        dplyr::group_by(time) |>
        dplyr::reframe(
          eff = meanProbs(blood, c(0.025, 0.5, 0.975), na.rm = TRUE, names = FALSE, n = 100),
          p1 = c(0.025, 0.5, 0.975),
          Percentile = c("2.5%", "50%", "97.5%")
        ) |>
        dplyr::arrange(time, p1)
      ...
      ci2 <- pk1 |>
        dplyr::group_by(time) |>
        dplyr::reframe(
          eff = meanProbs(blood, c(0.025, 0.5, 0.975), na.rm = TRUE, names = FALSE, pred = TRUE, n = 100),
          p1 = c(0.025, 0.5, 0.975),
          Percentile = c("2.5%", "50%", "97.5%")
        ) |>
        dplyr::arrange(time, p1)
      ...
      ci2 <- pk1 |>
        dplyr::group_by(time) |>
        dplyr::reframe(
          eff = meanProbs(blood, c(0.025, 0.5, 0.975), na.rm = TRUE, names = FALSE, useT = FALSE),
          p1 = c(0.025, 0.5, 0.975),
          Percentile = c("2.5%", "50%", "97.5%")
        ) |>
        dplyr::arrange(time, p1)
      ...
      ci2 <- pk1 |>
        dplyr::group_by(time) |>
        dplyr::reframe(
          eff = meanProbs(blood, c(0.025, 0.5, 0.975), na.rm = TRUE, names = FALSE, useT = FALSE, pred = TRUE),
          p1 = c(0.025, 0.5, 0.975),
          Percentile = c("2.5%", "50%", "97.5%")
        ) |>
        dplyr::arrange(time, p1)
