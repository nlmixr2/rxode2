rxTest({
  ...
  .mexp <- list(
    "1cmt" = paste("matExp()", "k_depot_central <- ka", "k_central_output <- cl/v", "cp <- central/v", sep = "\n"),
    "2cmt" = paste(
      "matExp()",
      "k_depot_central <- ka",
  ...
  test_that("a pure linear matExp sensitivity model emits no indLin() forcing", {
    for (.nm in names(.mexp)) {
      ...
      expect_equal(.rxMemDoIndLin(rxModelVars(suppressMessages(rxode2(.code)))), 1L, info = .nm)
    }
  })
  ...
  test_that("the second- and third-order blocks emit no forcing either", {
    ...
    .code <- rxSensMatExp(model = .mexp[["2cmt"]], calcSens = c("ka", "cl", "v"), calcSens2 = c("cl", "v"))
    expect_equal(.forcings(.code), character(0))
    expect_equal(.rxMemDoIndLin(rxModelVars(suppressMessages(rxode2(.code)))), 1L)
    .code3 <- rxSensMatExp(model = .mexp[["2cmt"]], calcSens = c("cl", "v"), calcSens2 = c("cl", "v"), calcSens3 = "v")
  ...
  test_that("the sensitivities are still the derivatives of the primal", {
    ...
    .s <- suppressMessages(rxSolve(.m, .th, .ev, method = "indLin", atol = 1e-12, rtol = 1e-12, cores = 1L))
    .fd <- function(nm, h) {
      .up <- .th
      .up[[nm]] <- .th[[nm]] + h
      .dn <- .th
      .dn[[nm]] <- .th[[nm]] - h
      (suppressMessages(rxSolve(.p, .up, .ev, method = "indLin", atol = 1e-12, rtol = 1e-12))$cp -
        suppressMessages(rxSolve(.p, .dn, .ev, method = "indLin", atol = 1e-12, rtol = 1e-12))$cp) /
        (2 * h)
    }
    for (.nm in c("ka", "cl", "v")) {
      .an <- .s[[paste0("rx__sens_central_BY_", .nm, "__")]] / .th[["v"]]
      if (.nm == "v") {
        .an <- .an - .s$central / .th[["v"]]^2
      }
  ...
  test_that("the second-order sensitivities are right on the exponential path", {
    ...
    .ev <- as.data.frame(et(amt = 100, cmt = "depot") |> et(.obs))
    .run <- function(m, th) {
      suppressMessages(rxSolve(m, th, .ev, method = "indLin", atol = 1e-12, rtol = 1e-12, cores = 1L))
    }
    .s <- .run(.m, .th)
    ...
    for (.p in c("cl", "v")) {
      for (.q in c("cl", "v")) {
        ...
        expect_equal(
          .s[[paste0("rx__sens_central_BY_", .p, "_BY_", .q, "__")]],
