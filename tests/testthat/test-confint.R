rxTest({
  ...
  test_that("confint() uses the study dimension with nStud > 1 (#1308)", {
    ...
    rxSetSeed(42)
    .s <- suppressMessages(rxSolve(.ciModel, .ciEt, thetaMat = .ciThetaMat, nStud = 20, nSub = 10))
    ...
    rxSetSeed(42)
    .s2 <- suppressMessages(rxSolve(.ciModel, .ciEt |> et(id = 1:10), thetaMat = .ciThetaMat, nStud = 20))
  ...
  test_that("confint() says whether the thetaMat was simulated (#1308)", {
    rxSetSeed(42)
    .used <- suppressMessages(rxSolve(.ciModel, .ciEt, thetaMat = .ciThetaMat, nStud = 20, nSub = 10))
    expect_message(confint(.used, "cp"), regexp = "drew from")
    # with nStud <= 1 rxSolve() ignores the thetaMat, so the interval carries no
    # parameter uncertainty
    rxSetSeed(42)
    .notUsed <- suppressWarnings(rxSolve(.ciModel, .ciEt, thetaMat = .ciThetaMat, nStud = 1, nSub = 10))
    expect_message(confint(.notUsed, "cp"), regexp = "did not draw from")
    # unless it is forced
    rxSetSeed(42)
    .forced <- rxSolve(.ciModel, .ciEt, thetaMat = .ciThetaMat, nStud = 1, nSub = 10, simVariability = TRUE)
  ...
  test_that("confint() gives no band when each study holds one subject (#1308)", {
    ...
    # instead.
    rxSetSeed(42)
    .s <- suppressMessages(rxSolve(.ciModel, .ciEt, thetaMat = .ciThetaMat, nStud = 20))
  ...
  test_that("plot.rxSolveConfint1 works with output tranformation", {
    .s <- rxWithSeed(42, suppressMessages(rxSolve(.ciModel, .ciEt, thetaMat = .ciThetaMat, nStud = 1, nSub = 10)))
    .ci <- suppressMessages(confint(.s, "cp", level = 0.95, ci = FALSE))
