rxTest({
  ...
  test_that("reserved variables are not promoted when appending", {
    for (v in c("t", "time", "tlast", "newind", "rxFlag", "M_PI", "pi", "NA", "NaN", "Inf")) {
      ui <- .base()
      expect_error(
        ui <- do.call(model, list(ui, str2lang(paste0("cp2 <- cp * ", v)), append = quote(cp))),
        NA,
        info = v
      )
  ...
  test_that("a piped model using t still solves", {
    ui <- .base()
    ui <- do.call(model, list(ui, quote(cp2 <- cp * exp(-t / 10)), append = quote(cp)))
    s <- rxSolve(
      ui,
      et(amt = 100, ii = 24, addl = 2) |> et(seq(0, 72, by = 12)),
  ...
  test_that("reserved variables are not added as covariates with auto=FALSE", {
    for (v in c("t", "time", "pi", "M_PI", "NA", "Inf")) {
      ui <- .base()
      ui <- do.call(model, list(ui, str2lang(paste0("cp2 <- cp * ", v)), append = quote(cp), auto = FALSE))
      expect_equal(ui$iniDf$name, c("tka", "tcl", "add.sd"), info = v)
      expect_false(v %in% ui$allCovs, info = v)
  ...
  test_that("a reserved variable in an error line is rejected, not promoted", {
    ...
    expect_error(do.call(model, list(.base(), quote(cp ~ add(t)))), "estimated or modeled")
  })
  ...
  test_that("E is kept out of the ini block but is still an ordinary covariate", {
    ...
    expect_error(do.call(model, list(.base(), quote(cp ~ add(E)))), "estimated or modeled")
  })
