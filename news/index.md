...
- Allow user to change the internal serialization type with
  ...
  `rxode2` is loaded, once loaded it keeps the option initially set.
  This is set to `xz` which is from base R, but could be sped up with
  either `"qs2"` (more future proof) or `"qdata"` (a bit faster).
