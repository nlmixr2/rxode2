...
```{r obsdata}
obs <- nlme::Theoph
names(obs) <- toupper(names(obs))
obs$CMT  <- ifelse(obs$TIME == 0 & obs$AMT > 0, 1, 2)
obs$EVID <- ifelse(obs$AMT > 0, 1, 0)
obs$DV   <- obs$CONC
head(obs)
```
...
simDat <- rxSolve(
  pkModel,
  obs,
  nSub  = nRep,          # subjects per replicate
...
vpc(
  sim  = simDat[simDat$EVID == 0, ],
  obs  = obs[obs$EVID == 0, ],
  obs_cols = list(id = "ID", dv = "DV", idv = "TIME"),
  sim_cols = list(id = "id",  dv = "cp", idv = "time"),
  pi   = c(0.05, 0.95),
  ci   = c(0.05, 0.95)
)
```
