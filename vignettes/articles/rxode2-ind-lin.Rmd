...
| `atol`/`rtol` | Maximum relative error vs LSODA |
|---|---|
| `1e-4` | 1.7e-01 |
| `1e-6` | 1.7e-02 |
| default (`1e-8`/`1e-6`) | 5.3e-03 |
| `1e-8` | 1.7e-03 |
| `1e-10` | 1.7e-04 |

The relinearization is first order, so the error falls like the square root of
the tolerance: a hundredfold tighter tolerance buys about a tenfold better
answer, and costs about ten times as many steps. `$counts$slvr` reports how many
were taken.
...
```{r}
maxRelErr(solveMm(method = "indLin", hmax = 1))    # 5.3e-03
maxRelErr(solveMm(method = "indLin", hmax = 0.01)) # 4.2e-03
```
