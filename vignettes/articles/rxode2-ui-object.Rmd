...
```{r pipeFull}
m2 <- m1 |>
  model(KA <- exp(tka+eta.ka),append=NA) |>
  model(CL <- exp(tcl+eta.cl + covClWt*WT), append=NA, cov="WT") |>
  model(V2 <- exp(tv2), append=NA) |>
  model(Q <-exp(tq), append=NA) |>
  model(V3 <- exp(tv3), append=NA) |>
  model(fdepot <- expit(tfdepot), append=NA) |>
  model(durDepot <- exp(tDurDepot), append=NA) |>
  model(rateDepot <- exp(tRateDepot), append=NA) |>
  model(C2 ~ pow(c2.pow.sd, c2.pow), append=TRUE) |>
...
Inside the parsed `rxode2` function, there is another element called `$predDf`. This
