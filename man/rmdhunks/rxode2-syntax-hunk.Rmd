...
- a whole covariance block, using one of the matrix-valued distributions,
  ie `prior(eta.cl, eta.v) ~ lkj_corr(2)`.  The names given have to be
...
The R name is used whenever R parameterizes the distribution the same way
'Stan' does (`dnorm()`, `dlnorm()`, `dgamma()`, `dbeta()`, `dcauchy()`,
`dunif()`, ...).  When there is no faithful R equivalent, the 'Stan' name is
used instead (`student_t()`, `lkj_corr()`, `inv_wishart()`, ...).

Both spellings are accepted, and the arguments may be given positionally or
by name, so these are all the same prior:
...
`dt()` is deliberately **not** accepted as a spelling of `student_t()`:
R's `dt()` is the standardized (or noncentral) t, while the 'Stan'
`student_t(nu, mu, sigma)` is a location-scale t, so treating them as the
same would silently change the prior.  Use `student_t()` with its own
parameterization.
...
- `assertRxUiNormalPriors()` -- for a method that supports priors but only
  normal ones.  That covers `dnorm()`/`normal()`, `std_normal()`, and the
  `multi_normal()` family, which is what the shorthand above produces for
  correlated parameters.  Anything else, including a covariance matrix prior
  such as `lkj_corr()` or `inv_wishart()`, is an error
