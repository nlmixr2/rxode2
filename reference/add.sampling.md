...
- time.units:
  ...
  an optional string specifying the time units. Defaults to the units
  specified when the `EventTable` was initialized.

## Value

eventTable with updated sampling. (Note the event table will be updated
even if you don't reassign the eventTable)
