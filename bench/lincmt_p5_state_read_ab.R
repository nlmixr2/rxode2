...
suppressMessages({
  ...
  tree <- switch(
    Sys.getenv("TREE", if (mode == "base") "base" else "head"),
...
evOf <- function(nSub, nObs, uniform = TRUE) {
  tmax <- 48
  tt <- if (uniform) {
    seq(tmax / nObs, tmax, length.out = nObs)
  } else {
    sort(unique(round(
      cumsum(stats::qexp(seq(0.02, 0.98, length.out = nObs))) /
        sum(stats::qexp(seq(0.02, 0.98, length.out = nObs))) *
        tmax,
      4
    )))
  }
  rxode2::et(amt = 100, ii = 12, addl = 3) |> rxode2::et(tt) |> rxode2::et(id = seq_len(nSub))
}
...
if (mode %in% c("base", "head")) {
  ...
  d <- local({
    ...
    obs <- data.frame(
      ...
      evid = 0L,
      ii = 0,
      dv = s[[vcol]] + stats::rnorm(nrow(s), 0, 0.7)
    )
    dose <- data.frame(id = seq_len(40L), time = 0, amt = 100, evid = 1L, ii = 12, dv = NA_real_)
    dose$addl <- 3L
    obs$addl <- 0L
    d <- rbind(dose, obs)
    d[order(d$id, d$time, -d$evid), ]
  })
  ctl <- nlmixr2est::foceiControl(print = 0L, calcTables = FALSE, covMethod = "")
  tv <- numeric(REPS)
  for (r in seq_len(REPS)) {
    t0 <- proc.time()[["elapsed"]]
  ...
  cat(sprintf(
    "fit tree=%s median %.2f s objf %.6f load %.2f\n",
    Sys.getenv("TREE"),
    stats::median(tv),
    f$objective,
    loadAvg()
