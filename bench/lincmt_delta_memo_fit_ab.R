...
ev <- do.call(
  ...
  lapply(seq_len(nSub), function(i) {
    rbind(
      data.frame(ID = i, TIME = 0, AMT = 100, EVID = 1, DV = 0),
      data.frame(ID = i, TIME = obsT, AMT = 0, EVID = 0, DV = 0)
    )
  })
)
simTruth <- c(tka = log(1.3), tcl = log(2.1), tv = log(21), tq = log(3.3), tvp = log(43))
sim <- rxode2::rxSolve(mod(), ev, cores = 1L, addDosing = FALSE, useLinCmt = TRUE)
...
res <- do.call(
  ...
  lapply(seq_len(REPS), function(r) {
    off <- fitOnce(FALSE)
    on <- fitOnce(TRUE)
    data.frame(rep = r, tOff = off$t, tOn = on$t, objfOff = off$objf, objfOn = on$objf, load = loadAvg())
  })
)
print(res, digits = 6)
cat(sprintf(
  "median fit: off %.2f s, on %.2f s, gain %.3fx; objf diff %.2e\n",
  median(res$tOff),
  median(res$tOn),
  median(res$tOff) / median(res$tOn),
