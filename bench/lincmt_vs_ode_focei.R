...
od <- rxode2::rxSolve(simMod, pars, ev, cores = 1L, useLinCmt = FALSE, atol = 1e-12, rtol = 1e-12)
relDiff <- max(abs(li$cp - od$cp) / pmax(abs(od$cp), 1e-10))
if (relDiff > 1e-6) {
  stop(sprintf("linCmt vs ODE prediction mismatch: %.3g", relDiff))
}
...
nIter <- function(fit) {
  ...
  if (is.null(n) || is.na(n)) {
    n <- tryCatch(nrow(fit$parHist), error = function(e) NA_integer_)
  }
  n
}
