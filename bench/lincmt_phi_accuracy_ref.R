...
evNU <- rxode2::et(rxode2::et(amt = 100, time = 0, cmt = 1), cumsum(seq(0.05, 0.55, length.out = 96)))
...
for (tr in c(1L, 10L)) {
  ...
  for (rn in c("uniform", "nonunif")) {
    ...
    off <- as.data.frame(rxode2::rxSolve(
      mod,
      P3,
      e,
      cores = 1L,
    ...
    on <- as.data.frame(rxode2::rxSolve(
      mod,
      P3,
      e,
      cores = 1L,
    ...
    ref <- as.data.frame(rxode2::rxSolve(
      odeMod,
      P3,
      e,
      cores = 1L,
    ...
    gc <- paste0("d", 0:6)
    a <- as.matrix(off[seq_len(n), gc])
    b <- as.matrix(on[seq_len(n), gc])
    out[[length(out) + 1L]] <- data.frame(
      ...
      gradMaxAbs = max(abs(b - a)),
      gradMaxRel = max(abs(b - a) / pmax(abs(a), 1e-300)),
      gradScale = max(abs(a)),
...
cat(
  "gradMaxAbs vs gradScale: absolute spread between the two orders",
...
cat(sprintf(
  "fresh window, regular sampling:          phiBuild=%d phiRows=%d tailRows=%d\n",
  stU[["phiBuild"]],
  stU[["phiRows"]],
  stU[["seqTailRows"]]
))
