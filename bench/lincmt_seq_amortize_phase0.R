...
for (cell in cells) {
  ...
  for (strat in c("sequential", "hybrid")) {
    ...
    cat(sprintf(
      "%s %s nSub=%d nObs=%d: %.4f s = %.3f us/obs (load %.2f)\n",
      cfg,
      strat,
      nSub,
      nObs,
...
attr(res, "provenance") <- list(
  script = "bench/lincmt_seq_amortize_phase0.R",
  cfg = cfg,
  date = format(Sys.time()),
  build = "pkgbuild::compile_dll(debug=FALSE)"
)
saveRDS(res, file.path(path.expand(outDir), sprintf("seq_amortize_phase0_%s.rds", cfg)))

# artifacts for the callgrind profile run (light script, rxode2 only)
saveRDS(
  list(modelText = rxode2::rxNorm(im), pars = mkPars(40L), cfg = cfg),
