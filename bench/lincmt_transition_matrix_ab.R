...
if (WHAT == "solve") {
  ...
  cells <- list(
    list(nm = "2cmt uniform 400x1000", ncmt = 2, dirs = 0:4, nSub = 400L, ev = function() {
      rxode2::et(rxode2::et(amt = 100, time = 0, cmt = 1), seq(0.1, 100, by = 0.1))
    }),
    list(nm = "3cmt uniform 400x1000", ncmt = 3, dirs = 0:6, nSub = 400L, ev = function() {
      rxode2::et(rxode2::et(amt = 100, time = 0, cmt = 1), seq(0.1, 100, by = 0.1))
    }),
    list(nm = "2cmt multidose 400x1000", ncmt = 2, dirs = 0:4, nSub = 400L, ev = function() {
      rxode2::et(rxode2::et(amt = 100, time = 0, cmt = 1, ii = 24, addl = 3), seq(0.1, 100, by = 0.1))
    }),
    list(nm = "3cmt multidose 400x1000", ncmt = 3, dirs = 0:6, nSub = 400L, ev = function() {
      rxode2::et(rxode2::et(amt = 100, time = 0, cmt = 1, ii = 24, addl = 3), seq(0.1, 100, by = 0.1))
    }),
    list(nm = "2cmt nonuniform 400x1000", ncmt = 2, dirs = 0:4, nSub = 400L, ev = function() {
      rxode2::et(rxode2::et(amt = 100, time = 0, cmt = 1), cumsum(seq(0.02, 0.18, length.out = 1000)))
    }),
    list(nm = "3cmt nonuniform 400x1000", ncmt = 3, dirs = 0:6, nSub = 400L, ev = function() {
      rxode2::et(rxode2::et(amt = 100, time = 0, cmt = 1), cumsum(seq(0.02, 0.18, length.out = 1000)))
    })
  )
  res <- do.call(
    ...
    lapply(cells, function(cl) {
      ...
      nObs <- cl$nSub * 1000L
      invisible(rxode2::rxSolve(mod, pars, ev, cores = 1L, addDosing = FALSE, linCmtSensType = "AD"))
  ...
  suppressMessages(devtools::load_all("~/src/nlmixr2est", quiet = TRUE, helpers = FALSE))
  set.seed(1234)
  mod <- function() {
  ...
  sim <- suppressWarnings(rxSolve(
    mod,
    evF,
    params = c(simP, prop.sd = 0.15),
  ...
  dat$addl <- 0
  dat <- rbind(
    transform(d0[, c("id", "time", "amt", "evid", "ii", "addl")], dv = NA),
  ...
  tf <- vapply(
    ...
    function(r) {
      ...
      f <- suppressWarnings(suppressMessages(nlmixr2(
        ...
        dat,
        est = "focei",
        control = foceiControl(calcTables = FALSE, print = 0L, maxOuterIterations = OUTER)
      )))
