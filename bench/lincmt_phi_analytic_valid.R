...
.gradModel <- function(ncmt, oral0, dirs) {
  args <- sprintf("rx__PTR__, t, 1, %d, %d, %%d, %%d, %%s, cl, v, q, vp, q2, vp2, ka", ncmt, oral0)
  function(trans) {
    a <- sprintf(args, -1L, -1L, trans)
    lines <- c(
      sprintf("cp=linCmtB(%s)", a),
      vapply(
        dirs,
        function(k) {
...
.ev <- function(nSub = 3L) {
  do.call(
    ...
    lapply(seq_len(nSub), function(i) {
      ...
      obs <- data.frame(
        id = i,
        time = sort(c(0.4, 1.1, 2.7, 4.9, 7.3, 9.9, 13.2, 15.8, 19.4, 22.1, 27.3, 31.9, 38.2, 44.7, 52.1) + sh),
        amt = 0,
        evid = 0,
        cmt = 1,
        rate = 0,
...
for (ncmt in 1:3) {
  for (oral0 in c(0L, 1L)) {
    ...
    for (trans in if (ncmt == 2L) c(1L, 2L, 3L, 4L, 5L) else c(1L, 2L)) {
      ...
      ana <- sol(2L)
      rev <- sol(0L, "ADr")
      dT <- .colScaled(ana, tail0)
      dR <- .colScaled(ana, rev)
      dRT <- .colScaled(tail0, rev)
      if (dT > worstTail) {
        worstTail <- dT
        where <- sprintf("%dcmt oral%d trans%d", ncmt, oral0, trans)
      }
...
for (ncmt in 2:3) {
  ...
  p1 <- sol(1L)
  s1 <- rxode2::linCmtSeqStats(TRUE)
  p2 <- sol(2L)
  s2 <- rxode2::linCmtSeqStats(TRUE)
  d <- .colScaled(p2, p1)
  message(sprintf(
    "  %dcmt oral: %.3e   probeRows=%d analyticRows=%d",
    ncmt,
    d,
    s1[["phiRows"]],
    s2[["phiAnalyticRows"]]
