...
cells <- list(
  ...
  list(
    ...
    p = P2,
    ev = rep40(rxode2::et(
      rxode2::et(amt = 100, time = 0, cmt = 1, ii = 12, addl = 9),
...
res <- list()
for (cl in cells) {
  invisible(one(cl, FALSE))
  invisible(one(cl, TRUE)) # warm both
  rxode2::linCmtSeqStats(TRUE)
  invisible(one(cl, TRUE))
  st <- rxode2::linCmtSeqStats(TRUE)
  offT <- numeric(ROUNDS)
  onT <- numeric(ROUNDS)
  ld <- numeric(ROUNDS)
  for (r in seq_len(ROUNDS)) {
    # alternate within each round
    ld[r] <- loadAvg()
    offT[r] <- one(cl, FALSE)
    onT[r] <- one(cl, TRUE)
  }
  ...
  res[[length(res) + 1L]] <- data.frame(
    cell = cl$nm,
    offUs = 1e6 * median(offT) / n,
    onUs = 1e6 * median(onT) / n,
