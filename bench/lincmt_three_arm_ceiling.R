...
mkDat <- function(nSub, nObs, nDose = 1L, tmax = 32) {
  ...
  set.seed(2003004)
  simDat <- data.frame(
    ID = rep(seq_len(nSub), each = length(obsT)),
    TIME = sim$time,
  ...
  doseRows <- data.frame(
    ID = rep(seq_len(nSub), each = length(doseT)),
    TIME = rep(doseT, nSub),
    DV = NA_real_,
    AMT = 100,
    EVID = 1,
...
numCmt <- function(d) {
  d$CMT <- ifelse(d$CMT == "depot", 1L, 2L)
  d
}
...
modTxt <- function(im) tryCatch(rxode2::rxNorm(im), error = function(e) paste(deparse(im), collapse = "\n"))
...
etaMax <- max(
  abs(as.matrix(innerFit$linCmt$eta[-1]) - as.matrix(innerFit$ode$eta[-1])),
  abs(as.matrix(innerFit$matExp$eta[-1]) - as.matrix(innerFit$ode$eta[-1]))
)
cat(
  "posthoc objf:",
  paste(sprintf("%s=%.6f", names(objs), objs), collapse = " "),
  " max |eta diff| vs ode:",
  format(etaMax, digits = 3),
  "\n"
)
...
if (mode == "floor") {
  ...
  for (cell in list(c(40L, 1000L), c(400L, 1000L))) {
    ...
    for (a in names(plainTxt)) {
      ...
      for (r in seq_len(nRep)) {
        t0 <- proc.time()[["elapsed"]]
        s <- do.call(rxode2::rxSolve, c(list(m, pars, ev, cores = 1L, addDosing = FALSE), xtr))
      ...
      cat(sprintf(
        "%s %s floor nSub=%d nObs=%d: %.4f s = %.3f us/obs (load %.2f)\n",
        cfg,
        a,
        nSub,
        nObs,
  ...
  for (ci in seq_len(nrow(cells))) {
    ...
    for (a in names(arms)) {
      ...
      if (a == "matExp" && Sys.getenv("CAP_MATEXP", "1") == "1" && nSub * nObs > 40L * 1000L) {
        rows[[length(rows) + 1L]] <- data.frame(
          cfg = cfg,
          arm = a,
          nSub = nSub,
      ...
      f <- innerFit[[a]]
      th <- f$env$fullTheta
      if (sum(thetaCols) == length(th)) {
        pars[, thetaCols] <- rep(th, each = nSub)
      }
      xtr <- list()
      if (a == "matExp") {
        xtr$method <- "indLin"
      ...
      for (r in seq_len(nRep)) {
        t0 <- proc.time()[["elapsed"]]
        s <- do.call(rxode2::rxSolve, c(list(im, pars, ev, cores = 1L, addDosing = FALSE, useLinCmt = FALSE), xtr))
        tset[r] <- proc.time()[["elapsed"]] - t0
        stopifnot(nrow(s) > 0)
      }
      rows[[length(rows) + 1L]] <- data.frame(
      ...
      cat(sprintf(
        "%s %s nSub=%d nObs=%d nDose=%d: %.4f s (load %.2f)\n",
        cfg,
        a,
        nSub,
        nObs,
  ...
  for (a in fitArms) {
    ...
    for (r in seq_len(nRep)) {
      ...
      rows[[length(rows) + 1L]] <- data.frame(
        ...
        rep = r,
        sec = z$sec,
        objf = z$fit$objective,
        load = loadAvg()
      )
      cat(sprintf("%s %s 40x10 rep %d: %.2f s objf %.4f\n", cfg, a, r, z$sec, z$fit$objective))
    }
  }
  if (cfg == "2cmt") {
