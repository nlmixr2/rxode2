...
g3 = rx__sens_central_BY_ka/V", ncmt))
  ...
  ts <- vapply(
    seq_len(as.integer(Sys.getenv("SREPS", "3"))),
    function(r) {
      t0 <- proc.time()[["elapsed"]]
      invisible(rxSolve(m, p, evm, cores = 1L, returnType = "data.frame", linCmtSensType = "AD"))
      proc.time()[["elapsed"]] - t0
    },
    0
  )
  data.frame(
    cell = sprintf("%dcmt %dx%d", ncmt, nSub, nObs),
    usObs = median(ts) / (nSub * nObs) * 1e6,
    load = loadAvg()
  )
}
if (mode %in% c("all", "solve")) {
  res$solve <- rbind(
    solveCell(2L, 400L, 1000L),
    solveCell(2L, 40L, 200L),
    solveCell(3L, 400L, 1000L),
    solveCell(3L, 40L, 200L)
  )
}
if (mode %in% c("all", "fit")) {
  ...
  dat$addl <- 0
  dat <- rbind(
    d0[, c("id", "time", "amt", "evid", "ii", "addl")] |> transform(dv = NA),
...
res$tree <- tree
res$time <- format(Sys.time())
dir.create("bench/results", showWarnings = FALSE)
saveRDS(res, out)
cat("tree:", tree, "\n")
if (!is.null(res$solve)) {
  print(res$solve)
}
if (!is.null(res$fit)) {
  print(res$fit)
