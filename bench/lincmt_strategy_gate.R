...
.gatePars <- function(cfg) {
  c(cl = cfg$p1, v = cfg$v1, q = cfg$p2, vp = cfg$p3, q2 = cfg$p4, vp2 = cfg$p5, ka = cfg$ka)
}
...
.gateEv <- function(nDoses, nObs, nSub, dtDose = 0.5, dtObs = 0.2) {
  ...
  do.call(
    ...
    lapply(seq_len(nSub), function(i) {
      rbind(
        data.frame(id = i, time = doseT, amt = 100, evid = 1, cmt = 1, rate = 0, ii = 0, ss = 0),
        data.frame(id = i, time = obsT, amt = 0, evid = 0, cmt = 1, rate = 0, ii = 0, ss = 0)
      )
    })
  )
}
...
.gateSupOverhead <- function(m, cfg, nObs = 200L, nSub = 44L, repsProd = 5L, repsProto = 10L) {
  ev <- do.call(
    rbind,
    lapply(seq_len(nSub), function(i) {
      rbind(
        data.frame(id = i, time = c(0, 24), amt = 100, evid = 1, cmt = 1, rate = 0, ii = 0, ss = 0),
        data.frame(
          id = i,
          time = seq(0.25, 72, length.out = nObs),
          amt = 0,
          evid = 0,
          cmt = 1,
          rate = 0,
...
benchLinCmtStrategyGate <- function(
  cfgIdx = c(1L, 4L, 6L),
  ...
  for (cfg in cfgs) {
    m <- .gateGradModel(cfg)
    ov <- .gateSupOverhead(m, cfg, nSub = nSub, repsProd = repsProd, repsProto = repsProto)
    cat(sprintf(
      "\n=== %s (npars=%d)  production/prototype overhead factor %.2fx ===\n",
      cfg$name,
      2L * cfg$ncmt + cfg$oral0,
      ov
    ))
    for (nDoses in doseGrid) {
      for (nObs in obsGrid) {
        ...
        tH <- .gateTime(function() .gateProtoHybrid(cfg, nDoses, nObs), repsProto, inner = 20L)
        ...
        rows[[length(rows) + 1L]] <- data.frame(
          ...
          protoHybPerSub = tH,
          adrPerSub = adrPerSub,
          protoHybOverAdr = tH / adrPerSub,
          overhead = ov,
          supPrimes = st[["primes"]],
          supDoses = st[["doses"]],
          supCons = st[["consolidations"]],
          supRows = st[["rows"]],
          best1 = names(best1)[which.min(best1)],
          bestN = names(bestN)[which.min(bestN)],
          bestOverAdr1 = tR1 / min(best1),
          bestOverAdrN = tRN / min(bestN),
          stringsAsFactors = FALSE
        )
        cat(sprintf(
          paste0(
            "  doses=%3d obs=%3d | 1thr fwd %.3f adr %.3f sup %.3f hybLB %.3f hyb~%.3f",
          ...
          tHestN,
          tH / adrPerSub,
          names(best1)[which.min(best1)],
          tR1 / min(best1),
          names(bestN)[which.min(bestN)],
          tRN / min(bestN)
        ))
      }
    }
