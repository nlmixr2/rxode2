...
suppressMessages({
  devtools::load_all(Sys.getenv("RXTREE", "~/src/rxode2-lincmt-analytic"), compile = FALSE, quiet = TRUE)
  devtools::load_all(Sys.getenv("NLTREE", "~/src/nlmixr2est"), compile = FALSE, helpers = FALSE, quiet = TRUE)
})
...
optFlags <- tryCatch(
  {
    o <- file.path(Sys.getenv("RXTREE", "~/src/rxode2-lincmt-analytic"), "src", "linCmt.o")
    p <- system(
      sprintf("readelf --debug-dump=info %s 2>/dev/null | grep -m1 -o 'GNU C++17.*'", path.expand(o)),
      intern = TRUE
    )
...
trueTheta <- list(
  ...
  `3cmt` = c(ka = 1.2, cl = 4, v = 30, q = 8, vp = 60, q2 = 3, vp2 = 200)
)[[cfg]]
odeLines <- list(
  `2cmt` = paste0(
    "d/dt(depot) = -ka*depot;",
...
set.seed(2003004)
dat <- rbind(
  data.frame(ID = seq_len(nSub), TIME = 0, DV = NA_real_, AMT = 100, EVID = 1, CMT = "depot"),
  data.frame(
    ID = rep(seq_len(nSub), each = nObs),
    TIME = sim$time,
...
iniBlock <- paste(
  c(
    sprintf("l%s <- log(%.6g)", names(trueTheta), unname(trueTheta) * 1.2),
    "eta.ka ~ 0.1",
    "eta.cl ~ 0.1",
    "eta.v ~ 0.1",
    "prop.sd <- 0.2"
...
ctlOf <- function(phi) {
  nlmixr2est::foceiControl(
    ...
    print = 0L,
    covMethod = "",
    rxControl = rxode2::rxControl(cores = 1L, linCmtSensType = "AD", linCmtSensPhi = phi)
  )
}
fitOne <- function(phi) {
...
thC <- grepl("^THETA_", pn)
if (sum(thC) == length(fullTheta)) {
  sPar[, thC] <- rep(fullTheta, each = nSub)
}
...
timeSolve <- function(evx, phi, nRep) {
  ...
  for (r in seq_len(nRep)) {
    t0 <- proc.time()[["elapsed"]]
    s <- rxode2::rxSolve(innerModel, sPar, evx, cores = 1L, addDosing = FALSE, useLinCmt = FALSE, linCmtSensPhi = phi)
...
onePass <- function(phi, nRep = 15L) {
  ...
  list(sec = a$sec, rows = a$rows, bigSec = b$sec, bigRows = b$rows, marginalUs = marginalUs)
}
...
for (rd in seq_len(nRound)) {
  ...
  for (phi in arms) {
    ...
    cat(sprintf(
      "round %d phi=%-5s wall %.2f s optimize %.2f nIter %s rows %.3g phiBuild %.3g (load %.2f)\n",
      rd,
      phi,
      z$wall,
      z$time$optimize,
      z$nIter,
      as.numeric(st[["valueCompute"]]),
      as.numeric(st[["phiBuild"]]),
      z$load
    ))
  }
  for (phi in arms) {
    ...
    passes[[length(passes) + 1L]] <- data.frame(
      ...
      bigSec = p$bigSec,
      bigRows = p$bigRows,
      wholeCallUs = 1e6 * p$sec / p$rows,
      marginalUs = p$marginalUs,
      load = loadAvg()
    )
    cat(sprintf(
      "   replay phi=%-5s whole-call %.2f us/row | MARGINAL %.3f us/row (%.4f->%.4f s, %.0f->%.0f rows)\n",
      phi,
      1e6 * p$sec / p$rows,
      p$marginalUs,
      p$sec,
  ...
  saveRDS(
    list(fits = do.call(rbind, rows), passes = do.call(rbind, passes)),
    file.path(path.expand(outDir), "lincmt_fit_solve_fraction.rds")
  )
}

fits <- do.call(rbind, rows)
pass <- do.call(rbind, passes)
agg <- function(d, k) {
  s <- split(d, d$phi)
  vapply(s, function(x) median(x[[k]]), 0)
}
optOn <- agg(fits, "optimize")[["TRUE"]]
optOff <- agg(fits, "optimize")[["FALSE"]]
itOn <- agg(fits, "nIter")[["TRUE"]]
itOff <- agg(fits, "nIter")[["FALSE"]]
rowsOn <- agg(fits, "valueCompute")[["TRUE"]]
rowsOff <- agg(fits, "valueCompute")[["FALSE"]]
...
passOn <- agg(pass, "sec")[["TRUE"]]
passOff <- agg(pass, "sec")[["FALSE"]]
...
summ <- list(
  provenance = list(
    commit = system("git -C ~/src/rxode2-lincmt-analytic rev-parse --short HEAD", intern = TRUE),
    nlCommit = system("git -C ~/src/nlmixr2est rev-parse --short HEAD", intern = TRUE),
    producerFlags = optFlags,
    core = core,
    rounds = nRound,
    cfg = cfg,
    nSub = nSub,
  ...
  derived = c(
    ...
    passSecOff = passOff,
    marginalUsOn = margOn,
    marginalUsOff = margOff,
    solveSecOn = solveSecOn,
    fLowerBound = fLower,
    fDifferentialINVALID = fDiff,
    fReplayINVALID = fReplay
  )
)
...
cat("\n================ paper table ================\n")
cat(sprintf(
  "cell: %s oral, 3 etas, %d subjects x %d uniform obs; FOCEi/bobyqa; %s; %s\n",
  cfg,
  nSub,
  nObs,
  core,
  summ$provenance$date
))
...
cat(sprintf(
  "document why: whole-call %.1f us/row, marginal %.1f us/row, against the\n",
  1e6 * passOn / agg(pass, "rows")[["TRUE"]],
  margOn
))
...
cat(sprintf("load range over the run: %.2f - %.2f\n", summ$provenance$loadRange[1], summ$provenance$loadRange[2]))
