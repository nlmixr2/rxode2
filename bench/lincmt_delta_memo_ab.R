...
cells <- list(
  ...
  list(name = "2cmt uniform 40x100", ncmt = 2, oral0 = 1, dirs = 0:4, ev = quote(mkEv(40L, seq(0.5, 50, by = 0.5))))
)
...
res <- do.call(
  ...
  lapply(cells, function(cl) {
    ...
    data.frame(
      cell = cl$name,
      nObs = nObs,
      usObsOff = 1e6 * tOff / nObs,
      usObsOn = 1e6 * tOn / nObs,
      gain = tOff / tOn,
