...
suppressMessages(devtools::load_all(
  Sys.getenv("RXTREE", "~/src/rxode2-lincmt-analytic"),
  compile = FALSE,
  quiet = TRUE
))
...
if (mode == "solve") {
  ...
  for (cell in cells) {
    ...
    for (r in seq_len(nRep)) {
      ...
      s <- rxode2::rxSolve(im, pars, ev, cores = 1L, addDosing = FALSE, useLinCmt = FALSE, linCmtSensType = "AD")
  ...
  attr(res, "provenance") <- list(
    script = "bench/lincmt_value_memo_ab.R",
    tree = tag,
    cfg = cfg,
  ...
  source(file.path(path.expand("~/src/rxode2-lincmt-analytic/bench"), "lincmt_value_memo_fit_model.R"))
  tset <- numeric(nRep)
  obj <- NA_real_
  for (r in seq_len(nRep)) {
    t0 <- proc.time()[["elapsed"]]
  ...
  cat(sprintf(
    "%s fit 40x100: median %.2f s (reps %s) objf %.6f load %.2f\n",
    tag,
    median(tset),
    paste(round(tset, 2), collapse = "/"),
    obj,
    loadAvg()
  ))
  saveRDS(
    data.frame(tree = tag, sec = median(tset), objf = obj, load = loadAvg()),
