...
.ev <- function(nDoses, nObs, nSub, dtDose = 0.5, dtObs = 0.2) {
  ...
  do.call(
    ...
    lapply(seq_len(nSub), function(i) {
      rbind(
        data.frame(id = i, time = doseT, amt = 100 * (1 + 0.01 * i), evid = 1, cmt = 1),
...
benchLinCmtAutoBoundary <- function(
  nSub = 40L,
  reps = 3L,
    ...
    for (k in ks) {
      ...
      for (sh in shapes) {
        ...
        solve1 <- function(st, cores) {
          rxSolve(mod, .pars, evt, linCmtSensType = st, cores = cores, returnType = "data.frame")
        }
        solve1("AD", 1L) # warm
        row <- data.frame(
          config = cfg$name,
        ...
        cat(sprintf(
          "%-9s m=%d k=%d %3d/%3d  fwd1 %.3f rev1 %.3f (%.2fx)  fwdN %.3f revN %.3f (%.2fx)\n",
          cfg$name,
          m,
          k,
          sh[1],
