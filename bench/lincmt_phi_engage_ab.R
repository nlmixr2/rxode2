...
suppressMessages(devtools::load_all(Sys.getenv("TREE", "."), compile = FALSE, quiet = TRUE))
...
gradModel <- function(ncmt, oral0, dirs) {
  args <- sprintf("rx__PTR__, t, 1, %d, %d, %%d, %%d, 1, p1, v1, p2, p3, p4, p5, ka", ncmt, oral0)
...
NOBS <- 1000L
mkEv <- list(
  uniform = function() rxode2::et(rxode2::et(amt = 100, time = 0, cmt = 1), seq(0.05, 0.05 * NOBS, by = 0.05)),
  multi = function() {
    rxode2::et(rxode2::et(amt = 100, time = 0, cmt = 1, ii = 12, addl = 9), seq(0.12, 0.12 * NOBS, by = 0.12))
  },
  nonunif = function() rxode2::et(rxode2::et(amt = 100, time = 0, cmt = 1), cumsum(seq(0.02, 0.4, length.out = NOBS))),
  infusion = function() {
    rxode2::et(rxode2::et(amt = 100, time = 0, cmt = 1, rate = 2), seq(0.05, 0.05 * NOBS, by = 0.05))
  },
  ss = function() {
    rxode2::et(rxode2::et(amt = 100, time = 0, cmt = 1, ii = 12, ss = 1), seq(0.05, 0.05 * NOBS, by = 0.05))
  }
)
mods <- list(
...
for (cfg in names(mods)) {
  for (rn in names(mkEv)) {
    ...
    md <- mods[[cfg]]
    invisible(rxode2::rxSolve(md$m, md$p, ev, cores = 1L, addDosing = FALSE, linCmtSensType = "AD")) # warm
    ...
    for (r in seq_len(REPS)) {
      ...
      invisible(rxode2::rxSolve(md$m, md$p, ev, cores = 1L, addDosing = FALSE, linCmtSensType = "AD"))
