...
.gradModel <- function(cfg, k) {
  ...
  args <- sprintf("rx__PTR__, t, 1, %d, %d, %%d, %%d, 1, cl, v, q, vp, q2, vp2, ka", cfg$ncmt, cfg$oral0)
  lines <- c(
...
.evBolus <- function(nSub = 3L) {
  do.call(
    ...
    lapply(seq_len(nSub), function(i) {
      ...
      dose <- data.frame(
        id = i,
        time = c(0, 7.5, 13, 24.25) + c(0, sh, 0, sh),
        amt = c(100, 80, 120, 90) * (1 + 0.1 * i),
        evid = 1,
        cmt = 1,
        rate = 0,
        ii = 0,
      ...
      obs <- data.frame(id = i, time = .obsTail(25, 11L, sh), amt = 0, evid = 0, cmt = 1, rate = 0, ii = 0, ss = 0)
      rbind(dose, obs)
    })
  )
}
  ...
  do.call(
    ...
    lapply(seq_len(nSub), function(i) {
      dose <- data.frame(
        ...
        evid = 1,
        cmt = 1,
        rate = c(50, 30, 80) * (1 + 0.1 * i),
        ii = 0,
        ss = 0
      )
      obs <- data.frame(
        id = i,
        time = .obsTail(31.5, 13L, 0.1 * i),
        amt = 0,
        evid = 0,
        cmt = 1,
        rate = 0,
...
.evMixed <- function(nSub = 3L) {
  do.call(
    ...
    lapply(seq_len(nSub), function(i) {
      ...
      obs <- data.frame(id = i, time = .obsTail(27, 13L, 0.2 * i), amt = 0, evid = 0, cmt = 1, rate = 0, ii = 0, ss = 0)
      rbind(dose, obs)
    })
  )
}
...
.evSs <- function(nSub = 2L) {
  do.call(
    ...
    lapply(seq_len(nSub), function(i) {
      ...
      obs <- data.frame(
        id = i,
        time = c(0.7, 2.3, 5.9, 11.1, 13.4, 20.2, 30.5, 47.5, 49.1, 53.3, 59.4, .obsTail(61, 6L, 0)) + 0.15 * i,
        amt = 0,
        evid = 0,
        cmt = 1,
        rate = 0,
...
.evInterleaved <- function(nSub = 3L) {
  do.call(
    ...
    lapply(seq_len(nSub), function(i) {
      ...
      obs <- data.frame(
        id = i,
        time = c(0.5, 1.7, 3.1, 6.2, 8.3, 11.9, 16.4, 22.2, 27.7, 35.1, 48.8, 60.2) + 0.1 * i,
        amt = 0,
        evid = 0,
        cmt = 1,
        rate = 0,
...
.solve <- function(m, cfg, ev, strategy, ...) {
  rxSolve(m, params = .parsFor(cfg), events = ev, returnType = "data.frame", linCmtSensStrategy = strategy, ...)
}
...
.runCase <- function(label, cfg, m, ev, rows, tol = 1e-9, ...) {
  ...
  check(
    sprintf(
      "%-30s rel diff %.2e  subj=%d rows=%d rates=%d cons=%d flush=%d full=%d",
      label,
      worst,
      st[["subjects"]],
      st[["rows"]],
...
if (.doValidate) {
  ...
  for (cfg in .cfgs[c(3, 6)]) {
    ...
    check(
      sprintf(
        "%-30s %d threads == 1 thread x5 (rel diff %.2e), == sequential (%.2e)",
        cfg$name,
        nThr,
        worstN,
        .cmpCols(s1, ref)
  ...
  .benchEv <- function(nDose, nObs, nSub = 44L) {
    do.call(
      ...
      lapply(seq_len(nSub), function(i) {
        ...
        obs <- data.frame(
          id = i,
          time = t0 + seq(0.25, 96, length.out = nObs),
          amt = 0,
          evid = 0,
          cmt = 1,
          rate = 0,
      ...
      for (nDose in c(20L, 100L, 200L)) {
        for (nObs in c(50L, 200L, 400L)) {
          ...
          for (cores in unique(c(1L, nThr))) {
            ...
            tF <- .timeIt(function() .solve(m, cfg, evt, "sequential", cores = cores, linCmtSensType = "AD"))
            tH <- .timeIt(function() .solve(m, cfg, evt, "hybrid", cores = cores))
            cat(sprintf(
              "  %-10s k=%d doses=%3d obs=%3d cores=%2d  sequential %.4fs  forward %.4fs  hybrid %.4fs  ratio %.2fx  ratioFwd %.2fx\n",
              cfg$name,
              k,
              nDose,
              nObs,
...
cat(sprintf("\n%d passed, %d failed\n", nPass, nFail))
if (nFail > 0) {
  stop("hybrid production validation failed")
}
