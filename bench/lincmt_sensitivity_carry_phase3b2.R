...
buildEv <- function() {
  doseDf <- data.frame(time = doseTimes, amt = 100, evid = 1, cmt = 1, wt = wtAt(doseTimes))
  obsDf <- data.frame(time = obsTimes, amt = 0, evid = 0, cmt = 1, wt = wtAt(obsTimes))
...
solve1 <- function(eCl, eV) {
  rxSolve(
    mod1,
    params = c(tcl = tclVal, tv = tvVal, eta.cl = eCl, eta.v = eV),
    events = ev,
...
driveTwoPair1cmt <- function(grid, pairOrder = c(0L, 1L)) {
  nRows <- nrow(grid)
  out <- list(s0 = rep(NA_real_, nRows), s1 = rep(NA_real_, nRows), pred = rep(NA_real_, nRows))
  rawAlast <- 0.0
  for (i in seq_len(nRows)) {
    ...
    if (i > 1) {
      ...
      contrib <- c(
        res$J[1, 1] * cl_i, # pair 0: dA/dcl * d(cl)/d(eta.cl)
        res$J[1, 2] * v_i
      ) # pair 1: dA/dv  * d(v)/d(eta.v)
...
check("raw-value propagation matches real solved cp", max(abs(resA$pred[obsMask] - cpReal)) < 1e-8)
...
h <- 1e-5
fdCl <- {
  p <- solve1(etaCl + h, etaV)
  m <- solve1(etaCl - h, etaV)
  (p$cp[p$evid == 0] - m$cp[m$evid == 0]) / (2 * h)
}
fdV <- {
  p <- solve1(etaCl, etaV + h)
  m <- solve1(etaCl, etaV - h)
  (p$cp[p$evid == 0] - m$cp[m$evid == 0]) / (2 * h)
}
...
check("pair 0 bit-identical under permuted -7 order", identical(resA$s0[obsMask], resB$s0[obsMask]))
check("pair 1 bit-identical under permuted -7 order", identical(resA$s1[obsMask], resB$s1[obsMask]))
...
solve2 <- function(eCl, eV) {
  rxSolve(
    mod2,
    params = c(tcl = tclVal, tv = tvVal, tq = tqVal, tv2 = tv2Val, eta.cl = eCl, eta.v = eV),
    events = ev,
    returnType = "data.frame",
    addDosing = TRUE
  )
...
for (i in seq_len(nrow(grid2))) {
  ...
  if (i > 1) {
    ...
    linCmtCarryLiveTest(
      id = 0L,
      t = rep(grid2$time[i], 5),
      tPrior = rep(grid2$time[i - 1], 5),
  ...
  if (amt_i != 0) {
    rawAlast[1] <- rawAlast[1] + amt_i
  }
  if (grid2$evid[i] == 0) {
    for (r in 0:1) {
      s0Mat[i, r + 1] <- linCmtCarryLiveTest(
        id = 0L,
        t = grid2$time[i],
        tPrior = grid2$time[i],
        theta = thetaRow,
      ...
      s1Mat[i, r + 1] <- linCmtCarryLiveTest(
        id = 0L,
        t = grid2$time[i],
        tPrior = grid2$time[i],
        theta = thetaRow,
...
check("2-cmt raw-value propagation matches real solved cp", max(abs(pred2[obsMask2] - cpReal2)) < 1e-8)
...
fdCl2 <- {
  p <- solve2(etaCl + h, etaV)
  m <- solve2(etaCl - h, etaV)
  (p$cp[p$evid == 0] - m$cp[m$evid == 0]) / (2 * h)
}
fdV2 <- {
  p <- solve2(etaCl, etaV + h)
  m <- solve2(etaCl, etaV - h)
  (p$cp[p$evid == 0] - m$cp[m$evid == 0]) / (2 * h)
}
...
check(
  "pair index >= cap raises a clear R error",
  !is.null(capErr) && grepl("out of range", capErr) && grepl("RX_LINCMT_CARRY_MAXPAIRS", capErr)
)
cat(sprintf("  error message: %s\n", if (is.null(capErr)) "<none>" else capErr))

cat(sprintf("\n%d/%d checks passed\n", nPass, nPass + nFail))
if (nFail > 0) {
  stop("Phase 3b.2 multi-pair carry checks FAILED")
}
