...
.record <- function(cfg, dt, sensType, ndiff) {
  ...
  c(as.numeric(s1$val), as.numeric(s1$J), as.numeric(s1$Jg), as.numeric(s2$val), as.numeric(s2$J), as.numeric(s2$Jg))
}
...
.mkCfg <- function(ncmt, oral0, trans, type, rate0 = 0) {
  ...
  c(
    list(
      name = sprintf(
        "%dcmt-%s-trans%d-type%d%s",
        ncmt,
        if (oral0 == 1L) "oral" else "iv",
        trans,
        type,
        if (rate0 > 0) "-inf" else ""
      ),
...
.masks <- function(ncmt, oral0) {
  full <- diffP1 + diffV1
  if (ncmt >= 2L) {
    full <- full + diffP2 + diffP3
  }
  if (ncmt >= 3L) {
    full <- full + diffP4 + diffP5
  }
  if (oral0 == 1L) {
    full <- full + diffKa
  }
  ...
  if (ncmt >= 2L) {
    ms <- c(ms, diffP1 + diffV1 + diffP2)
  }
  if (oral0 == 1L) {
    ms <- c(ms, diffKa, diffP1 + diffKa)
  }
  unique(ms)
}
  ...
  for (dt in c(0.25, 1.0, 4.0)) {
    for (nd in .masks(cfg$ncmt, cfg$oral0)) {
      ...
      if (!identical(ref, cand)) {
        ...
        bad[[length(bad) + 1L]] <-
          sprintf(
            "%s dt=%g nd=%d: %d/%d entries differ, worst |d| = %.3e",
            cfg$name,
            dt,
            nd,
            length(w),
...
message(sprintf("== ADm (32) vs AD (3): %d cases, %d skipped, %d MISMATCHED", nCase, nSkip, length(bad)))
if (length(bad)) {
  for (b in utils::head(bad, 20)) {
    message("  ", b)
  }
} else {
