...
designs <- list(
  ...
  nonuniform = list(obs = sort(c(exp(seq(log(0.25), log(96), length.out = 150)), seq(1, 99, by = 3.7))), dose = 0),
  interleaved = list(
    obs = as.numeric(sapply(0:4, function(d) 24 * d + c(0.5, 1, 2, 4, 8, 12, 16, 20))),
    dose = c(0, 24, 48, 72, 96)
  )
)

solveLin <- function(thv, des) {
  s <- rxode2::rxSolve(mLin, thv, mkEv(des$obs, des$dose), cores = 1L, addDosing = FALSE, useLinCmt = TRUE)
  s$cp
}
...
solComp2 <- function(k10, k12, k21) {
  ...
  invD0 <- 1 / (L[2] - L[1] + .Machine$double.eps)
  invD1 <- -invD0
...
recurEval <- function(thv, des, h = 1e-7) {
  ...
  A <- c(0, 0, 0)
  dA <- matrix(0, 3, length(nm), dimnames = list(NULL, nm))
  tPrev <- times[1]
  ...
  getPhi <- function(delta, thv2) {
    ...
    mc2 <- micro(thv2)
    sc2 <- solComp2(mc2$k10, mc2$k12, mc2$k21)
    ...
    dP <- lapply(nm, function(p) {
      ...
      tm[[p]] <- tm[[p]] * (1 - h)
      mcp <- micro(tp)
      scp <- solComp2(mcp$k10, mcp$k12, mcp$k21)
      mcm <- micro(tm)
      scm <- solComp2(mcm$k10, mcm$k12, mcm$k21)
...
backfillDemo <- function(thv, span = 100, delta = 0.5) {
  ...
  fwd[[1]] <- A0
  for (i in 1:n) {
    fwd[[i + 1]] <- as.numeric(P %*% fwd[[i]])
  }
  # backfill from the furthest state with the inverse transition
  Pi <- solve(P)
  back <- vector("list", n + 1)
  back[[n + 1]] <- fwd[[n + 1]]
  for (i in n:1) {
    back[[i]] <- as.numeric(Pi %*% back[[i + 1]])
  }
  ...
  data.frame(t = (0:n) * delta, relerr = relerr, growthBound = exp(max(sc$L) * ((n:0) * delta)) * .Machine$double.eps)
}
...
for (dn in names(designs)) {
  des <- designs[[dn]]
  vLin <- solveLin(th, des)
  vOde <- solveOde(th, des)
  pr <- recurEval(th, des)
  ...
  out[[dn]] <- list(
    ...
    uniquePhi = pr$uniquePhi,
    rows = pr$rows
  )
  message(sprintf(
    "%-11s val|prod %.2e  val|ode %.2e  sens|FD %.2e  uniquePhi %d/%d rows",
    dn,
    out[[dn]]$valVsProd,
    out[[dn]]$valVsOde,
    out[[dn]]$sensVsFdMax,
...
message(sprintf(
  "backfill instability: relerr passes 1e-6 at t=%.1f (span 100, Lmax=%.3f); max relerr %.2e",
  if (is.na(iBad)) NA else bf$t[iBad],
  max(solComp2(micro(th)$k10, micro(th)$k12, micro(th)$k21)$L),
  max(bf$relerr)
))
