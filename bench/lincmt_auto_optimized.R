...
.micro <- function(ncmt) {
  ...
  k <- b[["cl"]] / b[["v"]]
  if (ncmt == 1L) {
    return(list(k = k, v = b[["v"]]))
  }
  k12 <- b[["q"]] / b[["v"]]
  k21 <- b[["q"]] / b[["vp"]]
  if (ncmt == 2L) {
    return(list(k = k, v = b[["v"]], k12 = k12, k21 = k21))
  }
  k13 <- b[["q2"]] / b[["v"]]
  k31 <- b[["q2"]] / b[["vp2"]]
...
.expo <- function(ncmt) {
  ...
  K <- matrix(c(-(m$k + m$k12 + m$k13), m$k21, m$k31, m$k12, -m$k21, 0, m$k13, 0, -m$k31), 3, 3, byrow = TRUE)
...
.cfgs <- do.call(
  rbind,
  lapply(1:3, function(ncmt) {
    do.call(
      rbind,
      lapply(0:1, function(oral0) {
        data.frame(
          name = sprintf("%dcmt-%s", ncmt, if (oral0) "oral" else "iv"),
          ncmt = ncmt,
          oral0 = oral0,
          trans = .transFor(ncmt)
        )
...
.dirSets <- function(ncmt, oral0) {
  ...
  if (ncmt >= 2L) {
    sets <- c(sets, list(list(name = "clv", dirs = 0:1)))
  }
  if (oral0 && ncmt >= 2L) {
    sets <- c(sets, list(list(name = "clvka", dirs = c(0L, 1L, 2L * ncmt))))
  }
  sets
}

.gradModel <- function(cfg, dirs) {
  args <- sprintf("rx__PTR__, t, 1, %d, %d, %%d, %%d, %d, p1, v1, p2, p3, p4, p5, ka", cfg$ncmt, cfg$oral0, cfg$trans)
  lines <- c(
  ...
  do.call(
    ...
    lapply(seq_len(nSub), function(i) {
      switch(
        ...
        fewDense = {
          dT <- doses(2L, 12)
          one(i, dT, obs(200L, max(dT) + 0.1, 36))
        },
        heavySparse = {
          dT <- doses(100L, 0.5)
          one(i, dT, obs(50L, max(dT) + 0.1, 24))
        },
        heavyDense = {
          dT <- doses(100L, 0.5)
          one(i, dT, obs(200L, max(dT) + 0.1, 24))
        },
        heavyDense2 = {
          dT <- doses(200L, 0.5)
          one(i, dT, obs(400L, max(dT) + 0.1, 24))
        },
        infusion = {
          dT <- doses(10L, 6)
          one(i, dT, obs(100L, 0.1, max(dT) + 12), rate = 50)
        },
        ss = {
          dT <- doses(6L, 12)
          one(i, dT, obs(100L, 0.1, max(dT) + 24), ss = 1, ii = 12)
        }
      )
    })
  )
...
.provenance <- function() {
  list(
    affinity = tryCatch(system("taskset -cp $PPID 2>/dev/null | sed 's/.*: //'", intern = TRUE), error = function(e) {
      NA
    }),
    loadavg = tryCatch(readLines("/proc/loadavg", n = 1L), error = function(e) NA),
    topProcesses = tryCatch(
      system("ps -eo pid,pcpu,etime,comm --sort=-pcpu --no-headers | head -3", intern = TRUE),
      error = function(e) NA
    ),
    time = format(Sys.time()),
    threads = getRxThreads(),
...
benchLinCmtAutoOptimized <- function(
  nSub = 44L,
  reps = 3L,
  threaded = TRUE,
  nThr = getRxThreads(),
  ...
  evs <- lapply(shapes, .shape, nSub = nSub)
  names(evs) <- shapes
  rows <- .cfgs
  if (!is.null(configs)) {
    rows <- rows[rows$name %in% configs, ]
  }
  for (ci in seq_len(nrow(rows))) {
    ...
    if (proc.time()[["elapsed"]] - t0 > maxSec) {
      cat("MAXSEC reached before", fn, "\n")
    ...
    for (ds in .dirSets(cfg$ncmt, cfg$oral0)) {
      ...
      for (sh in shapes) {
        ...
        row <- data.frame(
          ...
          revN = NA_real_,
          hybN = NA_real_,
          revDiff = .relDiff(rev, fwd, cols),
          hybDiff = .relDiff(hyb, fwd, cols),
          transDiff = transDiff,
        ...
        cat(sprintf(
          "%-9s t%-2d %-6s k=%d %-11s fwd %.3f rev %.3f hyb %.3f  rev/fwd %.2f hyb/fwd %.2f  diffs %.1e/%.1e/%.1e win %s\n",
          cfg$name,
          cfg$trans,
          ds$name,
          row$k,
...
fillThreadedLinCmtAutoOptimized <- function(nSub = 44L, reps = 3L, nThr = getRxThreads(), maxSec = Inf, maxLoad = 2) {
  t0 <- proc.time()[["elapsed"]]
  evs <- lapply(.shapes, .shape, nSub = nSub)
  names(evs) <- .shapes
  ...
  for (fn in fs) {
    ...
    if (load1 > maxLoad) {
      stop(sprintf("load average %.1f > %.1f: not measuring threaded cells", load1, maxLoad))
    }
    ...
    prov$threaded <- .provenance()
    attr(res, "provenance") <- prov
    tmp <- paste0(fn, ".tmp")
    saveRDS(res, tmp)
    file.rename(tmp, fn)
