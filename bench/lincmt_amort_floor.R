...
thn <- list(
  `1cmt` = c("ka", "cl", "v"),
  `2cmt` = c("ka", "cl", "v", "q", "vp"),
  `3cmt` = c("ka", "cl", "v", "q", "vp", "q2", "vp2")
)[[cfg]]
tt <- list(`1cmt` = c(1.2, 4, 30), `2cmt` = c(1.2, 4, 30, 8, 60), `3cmt` = c(1.2, 4, 30, 8, 60, 3, 200))[[cfg]]
...
warm <- tset[-1]
cat(sprintf(
  "FLOOR %s 400x1000: first %.4f s; warm median %.4f s = %.3f us/obs (loads %s)\n",
  cfg,
  tset[1],
  median(warm),
  median(warm) / (400 * 1000) * 1e6,
...
if (nzchar(Sys.getenv("RUN"))) {
  ...
  row <- data.frame(
    cfg = cfg,
    tree = Sys.getenv("RXTREE", "HEAD"),
    firstSec = tset[1],
    warmMedianSec = median(warm),
