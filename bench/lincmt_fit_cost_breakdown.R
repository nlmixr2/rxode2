...
cat(sprintf(
  "controls: outerOpt=%s innerOpt=%s hessianMethod=%s fast=%s\n",
  ctl$outerOpt,
  if (is.null(ctl$innerOpt)) "NA" else ctl$innerOpt,
...
ctlSetup <- nlmixr2est::foceiControl(
  calcTables = FALSE,
  print = 0L,
  covMethod = "",
  maxOuterIterations = 0L,
...
print(res$time)
cat(sprintf(
  "wall full %.2f s; posthoc %.2f; setupish %.2f; solve pass %.4f s; parHist %s; objf %.4f (load %.2f)\n",
  res$wallFull,
  res$wallPost,
  res$wallSetup,
