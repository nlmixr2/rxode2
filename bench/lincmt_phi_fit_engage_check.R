...
suppressMessages(devtools::load_all("~/src/rxode2-lincmt-analytic", quiet = TRUE, compile = FALSE))
...
simData <- function(simTimes) {
  ...
  dose <- data.frame(ID = seq_len(nSub), TIME = 0, DV = NA_real_, AMT = 100, EVID = 1, CMT = "depot")
  d <- rbind(dose, obs)
...
report <- function(tag, r) {
  s <- r$stats
  cat(sprintf(
    "%-12s phiBuild=%-7d phiRows=%-8d reuse=%-6s objf=%.4f\n",
    tag,
    s[["phiBuild"]],
    s[["phiRows"]],
