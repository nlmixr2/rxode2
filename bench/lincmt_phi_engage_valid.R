...
regimens <- list(
  ...
  ss = function() rxode2::et(rxode2::et(amt = 100, time = 0, cmt = 1, ii = 12, ss = 1), seq(0.25, 24, by = 0.25))
)

runArm <- function(mod, p, ev, phi) {
  rxode2::linCmtSeqStats(TRUE)
  s <- rxode2::rxSolve(mod, p, ev, cores = 1L, addDosing = FALSE, linCmtSensType = "AD", linCmtSensPhi = phi)
...
cat("nonuniform phiBuild with the control on:", sum(res$phiBuildOn[res$regimen == "nonunif"]), "\n")
...
cat(
  "infusion phiRows (rows after the rate stops; non-zero is expected):",
