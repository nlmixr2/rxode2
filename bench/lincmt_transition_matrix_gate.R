...
message("== lincmt_transition_matrix_gate (mode ", Sys.getenv("RX_LINCMT_ABLATE", "0"), ") ==")
...
cells <- list(
  list(name = "2cmt uniform 400x1000", ncmt = 2, oral0 = 1, dirs = 0:4, nSub = 400L, obsT = seq(0.1, 100, by = 0.1)),
  list(name = "3cmt uniform 400x1000", ncmt = 3, oral0 = 1, dirs = 0:6, nSub = 400L, obsT = seq(0.1, 100, by = 0.1)),
  list(
    name = "2cmt nonuniform 400x1000",
    ncmt = 2,
    oral0 = 1,
    dirs = 0:4,
    nSub = 400L,
    obsT = cumsum(seq(0.02, 0.18, length.out = 1000))
  ),
  list(
    name = "3cmt nonuniform 400x1000",
    ncmt = 3,
    oral0 = 1,
    dirs = 0:6,
    nSub = 400L,
...
res <- do.call(
  ...
  lapply(cells, function(cl) {
    ...
    invisible(rxode2::rxSolve(mod, pars, ev, cores = 1L, addDosing = FALSE, linCmtSensType = "AD")) # warm
    ts <- vapply(
      seq_len(REPS),
      function(r) {
