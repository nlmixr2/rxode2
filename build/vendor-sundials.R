...
if (!nzchar(.sdr_ver)) {
  .sdr_ver <- tryCatch(as.character(packageVersion("sundialr")), error = function(e) {
    stop(
      "Set SUNDIALR_VERSION (e.g. 'SUNDIALR_VERSION=0.1.7 ",
...
if (!length(.tarball) && !nzchar(Sys.getenv("SUNDIALR_VERSION"))) {
  ...
  .tarball <- tryCatch(
    utils::download.packages("sundialr", destdir = tempdir(), repos = getOption("repos"), type = "source")[1, 2],
...
if (!length(.tarball) || !file.exists(.tarball)) {
  ...
  .cran_urls <- paste0("https://cran.r-project.org/src/contrib/", c("", "Archive/sundialr/"), .tarball_name)
  .ok <- FALSE
  for (.u in .cran_urls) {
  ...
  if (!.ok) {
    stop(
      "Could not download the sundialr ",
      .sdr_ver,
      " source tarball from ",
      "CRAN (tried src/contrib and src/contrib/Archive).",
      call. = FALSE
    )
  }
  .tarball <- .dest
...
.nested <- list.files(file.path(.td, "sundialr", "src"), pattern = "^sundials-mod-.*\\.tar\\.gz$", full.names = TRUE)
if (length(.nested)) {
...
if (!length(.sun_dirs)) {
  stop(
    "Unable to locate SUNDIALS source tree inside sundialr tarball.\n",
    "Checked: ",
    .td,
    "\n",
    "Run 'list.dirs(\"",
    .td,
    "\", recursive=TRUE)' to inspect the layout.",
    call. = FALSE
  )
}
.sun <- .sun_dirs[1]
...
if (length(.missing)) {
  warning(
    "The following source files were not found in the SUNDIALS tree and were skipped:\n  ",
...
if (!dir.exists(.inc_src)) {
  stop(
    "Unable to locate the SUNDIALS public headers (sundialr/inst/include) ",
    "inside the sundialr tarball.",
    call. = FALSE
  )
}
...
.fix_calloc <- function(.path) {
  ...
  .new <- gsub(
    "malloc\\(sizeof\\(struct (SUNMemoryHelper_Ops_|SUNMemoryHelper_|SUNMemory_)\\)\\)",
    "calloc(1, sizeof(struct \\1))",
    .new
  )
...
for (.f in c(
  "src/sundials_sundials_nonlinearsolver.c",
