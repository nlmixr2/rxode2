...
description: >
  This covers basic events supported for `rxode2` models with an example rxode2 model
---
...
## Review: rxode2 Syntax

Before we dive into events, let's quickly review what we learned about `rxode2` syntax.

### Quick Quiz

```{r syntaxReviewQuiz, echo=FALSE}
quiz(
  question("Which of the following is the correct way to define an ODE for a compartment named 'depot' in rxode2?",
    answer("`depot = -ka * depot`"),
    answer("`d/dt(depot) = -ka * depot`", correct = TRUE),
    answer("`ddt(depot) = -ka * depot`"),
    answer("`dot(depot) = -ka * depot`")
  ),
  question("If you want to define a variable but suppress it from the output, which operator should you use?",
    answer("`=`"),
    answer("`<-`"),
    answer("`~`", correct = TRUE),
    answer("`==`")
...
rxode2 tutorial; Compile it so we can continue:
...
table in the format that `rxode2` expects.
...
With the model built from `rxode2`, and the event table built above, we
...
chosen by `rxode2`; This is because the event table that was supplied
...
Recall `rxode2` you can use `add.sampling(` samples `)` or `et(` samples `)` to add
...
`tibble`, `data.frame`, `data.table` or `rxode2` event table):
...
`rxode2` event tables are very similar to the `NONMEM`/`Monolix`
...
is, it is print out in an `rxode2` event table.
...
bioavailability affect the `rxode2` system?
...
The `rxode2` event table needs to be setup specially for each type of
steady state record.
...
`drop_units` are useful to set and drop the rxode2 event table units.
...
`rxode2` event tables, you do not have to apply to the column(s) in the
dataset, but `rxode2` applies the unit changes where it needs to apply them.
...
`rxode2` to take advantage of its threading capabilities or
pre-compiled models.

Using the `deSolve`'s example for the event data frame and adapting it
for `rxode2` you simply wrap the `deSolve` data frame in the `et()` function;
...
Great job! You've learned how to create and manage event tables in `rxode2`. Now, let's look at a more modern and modular way to define your models. In the next tutorial, **[rxode2 model functions](rxode2-model-functions.Rmd)**, you will learn how to use the UI syntax to package your parameters and equations together.
