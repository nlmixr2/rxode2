...
.rxFFlagsFilter <- function() {
  ...
  if (!is.character(.fc) || length(.fc) == 0L || !nzchar(.fc[1]) || !is.character(.fflags) || length(.fflags) == 0L) {
    return("")
  }
  ...
  if (!is.character(.ver) || length(.ver) == 0L || !any(grepl("flang", .ver, ignore.case = TRUE))) {
    return("")
  }
  ...
  .d <- tempfile("rxff")
  if (!dir.create(.d, showWarnings = FALSE)) {
    return("")
  }
  ...
  .bad <- .cand[vapply(
    ...
    function(.f) {
      .out <- tryCatch(
        suppressWarnings(
          system2(.fcmd[1], c(.fcmd[-1], .f, "-c", shQuote(.src), "-o", shQuote(.obj)), stdout = TRUE, stderr = TRUE)
        ),
        error = function(e) character(0)
      )
...
.in <- gsub("#@FFLAGS_FILTER@", tryCatch(.rxFFlagsFilter(), error = function(e) ""), .in, fixed = TRUE)
