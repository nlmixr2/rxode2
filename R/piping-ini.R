...
.iniHandleEtaDist <- function(expr, rxui, envir) {
  .ini <- rxui$iniDf
  .tgt <- vapply(as.list(expr[[2]])[-1], function(y) paste(deparse(y), collapse = ""), character(1), USE.NAMES = FALSE)
  ...
  if (inherits(.new, "try-error")) {
    stop(
      "cannot declare the distribution '",
  ...
  if (!any(names(.newDf) == "etaDist")) {
    stop("the installed 'lotri' does not support declared random effect ", "distributions", call. = FALSE)
  }
  if (!any(names(.ini) == "etaDist")) {
