...
.adaptiveDosingFns <- c("evid_", "bolus", "infuse", "infuseDur", "reset", "replace", "multiply", "phantom", "obs")
...
#' @keywords internal
#' @export
.rxPrune <- function(x, envir = parent.frame(), strAssign = list()) {
  if (is.name(x) || is.atomic(x)) {
    ...
    if (identical(x[[1]], quote(`if`))) {
      ...
      if (length(envir$.if > 0)) {
        ...
        if (any(envir$.def1 == .f2)) {
          .ret <- paste0(
            ...
            .if,
            "*(",
            .rxPrune(x[[3]], envir = envir, strAssign),
            ")+(1-(",
            .if,
            "))*(",
            .f2,
          ...
          .ret <- paste0(
            ...
            .rxPrune(x[[3]], envir = envir, strAssign = strAssign),
            ")",
            ifelse(any(envir$.else == .f2), paste0("+", .f2), "")
          )
        }
      ...
      exists("..captureN", envir = envir, inherits = FALSE) &&
