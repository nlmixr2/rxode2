...
.modelHandleModelLines <- function(modelLines, rxui, modifyIni=FALSE, append=NULL,
  ...
  if (length(.v$rm) > 0) {
    lapply(.v$rm, function(x) {
      .removeVariableFromIniDf(x, rxui, promote=ifelse(.in(x, .v$err), NA, FALSE))
    })
  }
  if (length(.v$new) > 0) {
  ...
  if (length(line) == 2L) {
    if (identical(line[[1]], quote(`-`))) {
      if (is.name(line[[2]])) {
        ...
        if (is.name(line[[2]][[2]]) &&
              .in(as.character(line[[2]][[1]]),
                  c("F", "f", "alag", "lag", "dur", "rate"))) {
          return(TRUE)
    ...
    if (modifyIni && .isQuotedLineRhsModifiesEstimates(line, rxui)) {
      ...
      if (is.null(.ret)) {
        ...
        if (.isDrop) {
          ...
          if (length(.predDf$cond) == 1L && any(.in(.ret0, .predDf$line))) {
            .predDf <- NULL
            .lstExpr <- .lstExpr[-.ret]
          } else {
            for (.i in .ret0) {
...
.addVariableToIniDf <- function(var, rxui, toEta=NA, value=1, promote=FALSE) {
  if (.in(var, c("pi", "M_E", "M_E", "E", "M_PI", "M_PI_2",
  ...
  if (.isEta) {
    ...
    if (is.na(promote)) {
      ...
      if (!is.null(.varSelect$covariateNames)) {
        if (.in(var, .varSelect$covariateNames)) {
          if (isTRUE(getOption("rxode2.verbose.pipe", TRUE))) {
