...
rxSolveAdjoint <- function(
  ...
  .primal <- as.data.frame(rxode2::rxSolve(object, params = params, events, returnType = "data.frame", ...))
  .obsTimes <- sort(unique(.primal$time))
  .build <- .rxAdjointSolveBuild(object, calcSens, events, adjStates = adjStates)
  .sens <- .rxAdjointSolveEvalC(.build, params, .obsTimes, denseBy = denseBy, atol = atol, rtol = rtol)
  .sensCols <- setdiff(names(.sens), "time")
  .idx <- match(.primal$time, .sens$time)
  for (.cn in .sensCols) {
    .primal[[.cn]] <- .sens[[.cn]][.idx]
  }
  .primal
}
...
.rxAdjointSolve <- function(
  ...
  .fwd <- as.data.frame(rxode2::rxSolve(
    rxode2::rxode2(.mText),
    params = params,
    .fev,
  ...
  .out <- matrix(NA_real_, length(outTimes), length(.cols), dimnames = list(NULL, .cols))
  for (.ii in seq_along(outTimes)) {
    ...
    .sol <- as.data.frame(rxode2::rxSolve(
      .revMod,
      params = params,
      .dat,
      inits = .inits,
...
.rxAdjointSolveBuild <- function(object, calcSens, events, adjStates = NULL) {
  ...
  for (.k in which(.rateCol %in% c(-1, -2))) {
    ...
    .infusSym[[length(.infusSym) + 1L]] <- list(
      cmt = .c,
      t1 = .t1,
      amt = .amt,
      kind = .kind,
  ...
  .fLhs <- unlist(lapply(names(.dFexpr), function(.c) {
    vapply(
      calcSens,
      function(p) {
        paste0("rx__dFdp_", .c, "_", p, "__=", .dFexpr[[.c]][[p]])
      },
      character(1)
    )
  }))
  ...
  .fRHSlhs <- if (.needF) {
    vapply(
      ...
      function(i) {
        ...
        paste0("rx__fRHS_", i, "__=", rxode2::rxFromSE(.d))
      },
      character(1)
    )
  } else {
  ...
  .jacLines <- unlist(lapply(.st, function(j) {
    lapply(c(.st, calcSens), function(x) {
...
#' @export
#' @keywords internal
.rxAdjointSolveEvalC <- function(build, params, outTimes, denseBy = 0.01, atol = 1e-10, rtol = 1e-10) {
  .st <- build$st
  ...
  .infus <- lapply(build$infusSym, function(z) {
    ...
    list(cmt = z$cmt, t1 = z$t1, t2 = z$t1 + z$amt / .Rv, R = .Rv, dRdp = stats::setNames(.dRp, calcSens), amt = z$amt)
  })
  .infT <- unlist(lapply(.infus, function(z) c(z$t1, z$t2)))
  .denseT <- sort(unique(c(
    seq(0, max(outTimes), by = denseBy),
    outTimes,
    build$evR$time,
    .doses$tau,
    .doses$tau - denseBy,
  ...
  .cj <- if (nrow(.evR) > 0L) {
    ...
    list(as.integer(match(.evR$time, .denseT) - 1L), as.integer(.cmt0), ifelse(.evR$evid == 5L, 0.0, .evR$amt))
  } else {
    list(integer(0), integer(0), numeric(0))
  }
  ...
  for (.j in which(.doses$rate == 0)) {
    ...
    if (.c %in% names(build$dFexpr)) {
      .w <- numeric(.ns)
      .w[.ci] <- .doses$amt[.j]
      .cc <- vapply(calcSens, function(p) .fwd[[paste0("rx__dFdp_", .c, "_", p, "__")]][.k], numeric(1))
  ...
  .dual <- if (length(.duals)) {
    list(
      as.integer(vapply(.duals, function(d) d$k, integer(1))),
...
.rxAdjointGrad <- function(
  ...
  .rxAdjointGradEval(.b, params, obsTimes, obs, weight = weight, denseBy = denseBy, atol = atol, rtol = rtol)
}
...
.rxAdjointGradPop <- function(
  ...
  for (.id in .ids) {
    ...
    .g <- .g + .evalFn(.build, .p, .d$time, .d$dv, denseBy = denseBy, atol = atol, rtol = rtol)
  }
  .g
}
...
.rxAdjointGradBuild <- function(object, calcSens, pred, events, errModel = NULL) {
  ...
  .dhdy <- lapply(.st, function(i) {
    .d <- symengine::D(.ph, .rxSEres(i))
    rxode2::rxFromSE(.d)
  })
  names(.dhdy) <- .st
  .dhdp <- lapply(calcSens, function(p) {
    .d <- symengine::D(.ph, .rxSEres(p))
    rxode2::rxFromSE(.d)
  })
  names(.dhdp) <- calcSens
  ...
  .lamLines <- vapply(
    ...
    function(i) {
      paste0(
        ...
        .lam(i),
        ")=",
        paste(vapply(.st, function(j) paste0("rx__df_", j, "_dy_", i, "__*", .lam(j)), character(1)), collapse = "+")
      )
    },
    character(1)
  )
  .muLines <- vapply(
    ...
    function(p) {
      .base <- paste(
        vapply(.st, function(j) paste0(.lam(j), "*rx__df_", j, "_dy_", p, "__"), character(1)),
        collapse = "+"
      )
        ...
        paste0(
          ...
          paste(
            vapply(
              .infCmts,
              function(cc) {
                paste0(.lam(cc), "*rxInfF_", cc, "_", p, "__")
              },
              character(1)
            ),
            collapse = "+"
...
.rxAdjEvalNum <- function(txt, params) {
  ...
  ## rounding-family functions that base R either does not spell or spells
  ## differently; floor/trunc/sign resolve to base R, which matches the C
  ## semantics.  Without these a dosing modifier like `dur(central) <-
  ## fround(d1, 1)` evaluates to NA and silently zeroes the dose duals it feeds.
  assign("ceil", function(x) ceiling(x), envir = .env)
  ## C's round() rounds a half away from zero; base R's rounds it to even
  assign("round", function(x) trunc(x + sign(x) * 0.5), envir = .env)
  assign("fround", function(x, digits) round(x, digits), envir = .env)
  assign("fprec", function(x, digits) signif(x, digits), envir = .env)
  ## transfer of sign, where y == 0 counts as positive (sign(0) would give 0)
  assign("fsign", function(x, y) ifelse(y >= 0, abs(x), -abs(x)), envir = .env)
...
#' @export
#' @keywords internal
.rxAdjointGradEval <- function(build, params, obsTimes, obs, weight = 1, denseBy = 0.01, atol = 1e-10, rtol = 1e-10) {
  .st <- build$st
  calcSens <- build$calcSens
  ...
  for (.c in names(build$lagStr)) {
    ...
    .dLag[[.c]] <- stats::setNames(
      vapply(build$dLagStr[[.c]], function(s) .rxAdjEvalNum(s, params), numeric(1)),
      calcSens
    )
  }
  .doses <- build$doses
  ...
  if (is.null(errModel)) {
    ...
    if (!is.null(errModel$add)) {
      .errGrad[errModel$add] <- sum(.dLdvar * 2 * .addV)
    } # dv/d(add)  = 2 add
    if (!is.null(errModel$prop)) {
      .errGrad[errModel$prop] <- sum(.dLdvar * 2 * .propV * .fobs^2)
    } # dv/d(prop)
  }
  .cvec <- lapply(seq_along(obsTimes), function(i) {
    vapply(.st, function(s) .dLdf[i] * .atObs(paste0("rx__dhdy_", s, "__"))[i], numeric(1))
  })
  ...
  .fPost <- function(t) {
    vapply(
      .st,
      function(i) {
        .fwd[[paste0("rx__fRHS_", i, "__")]][which(.fwd$time >= t - 1e-9)[1]]
      },
      numeric(1)
    )
  }
  .fPre <- function(t) {
    .k <- max(which(.fwd$time < t - 1e-9))
  ...
  for (p in calcSens) {
    .state[paste0("rx__adjMu_", p, "__")] <- 0
  }
  ...
  for (.bi in seq_along(.breaks)) {
    ...
    for (.j in .dHere) {
      ...
      if (.c %in% names(.dFexpr)) {
        .lamc <- .state[.lam(.c)]
        for (p in calcSens) {
          .gDose[p] <- .gDose[p] + .lamc * .doses$amt[.j] * .atT(paste0("rx__dFdp_", .c, "_", p, "__"), .tau)
        }
      }
      ...
      if (.c %in% names(.dLag)) {
        ...
        for (p in calcSens) {
          .gDose[p] <- .gDose[p] + .cross * .dLag[[.c]][[p]]
        }
      }
    }
    ## modeled-rate infusion moving-boundary term at tau2 = tau1 + amt/R
    for (.z in .infus) {
      if (abs(.z$t2 - .tau) < 1e-9) {
        .lamc <- .state[.lam(.z$cmt)]
        .dTau2 <- -(.z$amt / .z$R^2) * .z$dRdp
        for (p in calcSens) {
          .gDose[p] <- .gDose[p] + .z$R * .lamc * .dTau2[[p]]
        }
      }
    }
    ...
    for (.er in which(abs(.evR$time - .tau) < 1e-9)) {
      ...
      if (.evR$evid[.er] == 5L) {
        .state[.lam(.cc)] <- 0
      } else if (.evR$evid[.er] == 6L) {
        .state[.lam(.cc)] <- .evR$amt[.er] * .state[.lam(.cc)]
      }
    }
    for (.o in which(abs(obsTimes - .tau) < 1e-9)) {
      .ci <- .cvec[[.o]]
      for (i in .st) {
        .state[.lam(i)] <- .state[.lam(i)] + .ci[[i]]
      }
    }
    if (.bi < length(.breaks)) {
      .tLo <- .breaks[.bi + 1]
  ...
  .gExpl <- vapply(
    calcSens,
    function(p) {
      sum(.dLdf * .atObs(paste0("rx__dhdp_", p, "__")))
    },
    numeric(1)
  )
  .gErr <- vapply(
    calcSens,
    function(p) {
      if (p %in% names(.errGrad)) .errGrad[[p]] else 0
    },
    numeric(1)
  )
...
#' @export
#' @keywords internal
.rxAdjointGradEvalC <- function(build, params, obsTimes, obs, weight = 1, denseBy = 0.01, atol = 1e-10, rtol = 1e-10) {
  ...
  .gExpl <- vapply(
    calcSens,
    function(p) {
      sum(.dLdf * .fwd[[paste0("rx__dhdp_", p, "__")]][.oidx])
    },
    numeric(1)
  )
  .gErr <- vapply(
