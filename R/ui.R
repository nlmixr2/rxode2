...
.rxEtaDistIniToModel <- function(fun) {
  ...
  .isBlock <- function(nm) {
    which(vapply(seq_along(.lst), function(x) {
      .e <- .lst[[x]]
  ...
  .wi <- .isBlock("ini")
  .wm <- .isBlock("model")
  if (length(.wi) != 1L || length(.wm) != 1L) return(fun)
  if (length(.lst[[.wi]]) < 2L || length(.lst[[.wm]]) < 2L) return(fun)
  .iniBody <- .lst[[.wi]][[2]]
  .modBody <- .lst[[.wm]][[2]]
  if (!is.call(.iniBody) || !identical(.iniBody[[1]], quote(`{`))) return(fun)
  if (!is.call(.modBody) || !identical(.modBody[[1]], quote(`{`))) return(fun)
  .iniExpr <- as.list(.iniBody)[-1]
  .isDecl <- vapply(.iniExpr, function(.e) {
    is.call(.e) && identical(.e[[1]], quote(`~`)) && length(.e) == 3L &&
      is.call(.e[[2]]) && is.name(.e[[2]][[1]]) &&
      as.character(.e[[2]][[1]]) %in% c("dist", "etaDist")
  }, logical(1))
  if (!any(.isDecl)) return(fun)
  .lst[[.wi]][[2]] <- as.call(c(list(quote(`{`)), .iniExpr[!.isDecl]))
  .lst[[.wm]][[2]] <- as.call(c(list(quote(`{`)), .iniExpr[.isDecl],
                                as.list(.modBody)[-1]))
  .fun2 <- function() {}
