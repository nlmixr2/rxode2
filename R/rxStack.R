...
rxStack <- function(data, vars = NULL, doSim = TRUE, doIpredSim = TRUE) {
  if (!is.rxStackData(data)) {
    stop("this data cannot be used with `rxStack`", call. = FALSE)
  }
  .nd <- names(data)
  ...
  if (doIpredSim) {
    .doIpredSim <- all(vapply(
      vars,
      function(x) {
        if (x == "ipredSim") {
          return(TRUE)
        }
        if (substr(x, 0, 9) == "ipredSim.") {
          return(TRUE)
        }
        FALSE
      },
  ...
  if (length(vars) == 1L && (.doSim || .doIpredSim) && any(.nd == "CMT")) {
    .vars <- vapply(
      vars,
      function(x) {
        if (.doSim) {
    ...
    if (!is.null(.mv)) {
      ...
      if (.doIpredSim) {
        .ret <- c(.ret, list(time = data[["time"]], value = data[["ipredSim"]], trt = .cmt))
      }
