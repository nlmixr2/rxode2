...
.rxGetLambdaFromPred1AndIni <- function(env, pred1) {
  ...
  if (.rxTransformHasALambdaParameter(pred1$transform)) {
    .cnd <- pred1$cond
    .w <- which(!is.na(match(env$iniDf$err, c("boxCox", "yeoJohnson"))) & env$iniDf$condition == .cnd)
    if (length(.w) == 1L) {
...
.rxGetVarianceForErrorProp <- function(env, pred1) {
  ...
  if (!is.na(pred1$b)) {
    ...
    .cnd <- pred1$cond
    .w <- which(!is.na(match(env$iniDf$err, c("prop", "propF", "propT"))) & env$iniDf$condition == .cnd)
    if (length(.w) == 1L) {
      .p1 <- str2lang(env$iniDf$name[.w])
    } else {
...
.rxGetVarianceForErrorPow <- function(env, pred1) {
  ...
  if (!is.na(pred1$c)) {
    ...
    .w <- which(!is.na(match(env$iniDf$err, c("pow2", "powF2", "powT2"))) & env$iniDf$condition == .cnd)
    if (length(.w) == 1L) {
...
.handleSingleErrTypeNormOrTFoceiBase <- function(env, pred1, errNum=1L, rxPredLlik=TRUE) {
  ...
  if (!is.na(match(type, c("norm", "t", "cauchy", "dnorm")))) {
