...
.rxGetVarianceForErrorAddProp <- function(env, pred1) {
  if (!is.na(pred1$a)) {
    ...
    .cnd <- pred1$cond
    .w <- which(.in(env$iniDf$err, c("add", "lnorm", "probitNorm", "logitNorm")) & env$iniDf$condition == .cnd)
    if (length(.w) == 1L) {
      .p1 <- str2lang(env$iniDf$name[.w])
    } else {
  ...
  if (!is.na(pred1$b)) {
    ...
    .cnd <- pred1$cond
    .w <- which(.in(env$iniDf$err, c("prop", "propT", "propF")) & env$iniDf$condition == .cnd)
    if (length(.w) == 1L) {
      .p2 <- str2lang(env$iniDf$name[.w])
    } else {
      stop("cannot find proportional standard deviation", call.=FALSE)
...
.rxGetVarianceForErrorAddPow <- function(env, pred1) {
  ...
  if (!is.na(pred1$b)) {
    ...
    .cnd <- pred1$cond
    .w <- which(.in(env$iniDf$err, c("pow", "powF", "powT")) & env$iniDf$condition == .cnd)
    if (length(.w) == 1L) {
      .p2 <- str2lang(env$iniDf$name[.w])
    } else {
