...
rxAppendModel_ <- function(model1, model2, common = TRUE) {
  ...
  if (common && length(.bind) == 0) {
    warning(
      "the appended models have no variables in common (use `common=FALSE` to suppress this warning)",
      call. = FALSE
    )
  }
  ...
  if (length(.both) > 0) {
    ...
    .minfo(paste0("keeping initialization from first model: '", paste(.both, collapse = "', '"), "'"))
  ...
  if (length(.both) > 0) {
    ...
    .complex1 <- which(vapply(
      .both,
      function(v) {
        .eta <- .ini1eta[.ini1eta$name == v, "neta1"]
        any(
          (.ini1eta$neta1 == .eta & .ini1eta$neta2 != .eta) |
    ...
    .complex2 <- which(vapply(
      .both,
      function(v) {
        .eta <- .ini2eta[.ini2eta$name == v, "neta1"]
        any(
          (.ini2eta$neta1 == .eta & .ini2eta$neta2 != .eta) |
    ...
    if (length(.err) > 0) {
      stop(
        "duplicated parameter has covariance, will not append models: '",
        paste0(.err, collapse = "', '"),
        "'",
        call. = FALSE
      )
