...
"
  ...
  cat("\n")
  cat(paste(paste0("#define RxMvTrans_", .nmvt, " ", seq_along(.nmvt) - 1), collapse = "\n"))
  cat("\n")
  ...
  cat(paste0(
    "\n#define RxTransNames CharacterVector _en(",
    length(.n),
    ");",
    paste(paste0("_en[", seq_along(.n) - 1, ']="', .n, '";'), collapse = ""),
    "e.names() = _en;"
  ))
  cat("\n")
...
.generateRandomUiFun <- function(fun, args, vals = NULL) {
  ...
  if (is.null(vals)) {
    ...
    .f <- paste0(
      ...
      fun,
      " <- function(",
      paste0(paste0(args, ifelse(is.na(vals), "", " = "), ifelse(is.na(vals), "", vals)), collapse = ", "),
      ") ",
      paste(deparse(.ret), collapse = "\n"),
      "\n"
    )
...
.generateRandomUiFuns <- function() {
  ...
  .lst <- c(
    .lst,
    .lst2,
    list("logit" = c("x" = NA, "low" = 0, "high" = 1), "expit" = c("x" = NA, "low" = 0, "high" = 1))
  )
  paste0(
...
.rxodeBuildCode <- function() {
  ...
  .mkGrammar <- function(stem, ident) {
    dparser::mkdparse(
      devtools::package_file(paste0("inst/", stem, ".g")),
  ...
  try(
    {
      ...
      file.copy(
        devtools::package_file("src/rxode2_types.h"),
  ...
  rxResidualError <- utils::read.csv(devtools::package_file("inst/residualErrors.csv"), check.names = FALSE)
  usethis::use_data(rxResidualError, overwrite = TRUE)
  .l <- readLines(devtools::package_file("R/rxResidualError.R"))
  .l <- sub(
    "[#][']\\s*@format\\s*.*",
    sprintf("#' @format A data frame with %d columns and %d rows", dim(rxResidualError)[2], dim(rxResidualError)[1]),
    .l
  )
  ...
  .l <- sub(
    "[#][']\\s*@format\\s*.*",
    sprintf(
      "#' @format A data frame with %d columns and %d rows",
      dim(rxReservedKeywords)[2],
      dim(rxReservedKeywords)[1]
    ),
    .l
  )
  ...
  .l <- sub(
    ...
    sprintf(
      "#' @format A data frame with %d columns and %d rows",
      dim(rxSyntaxFunctions)[2],
      dim(rxSyntaxFunctions)[1]
    ),
    .l
  )
...
.createRxUiBlessedList <- function() {
  ...
  .blessed <- deparse(str2lang(paste0(".rxUiBlessed <- ", paste(deparse(.blessed), collapse = "\n"))))
  writeLines(
    c("## created by .createRxUiBlessedList() in ui-assign-parts.R edit there", .blessed),
    devtools::package_file("R/rxUiBlessed.R")
  )
  message("saved!")
