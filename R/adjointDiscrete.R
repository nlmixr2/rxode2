...
.rxDiscreteAdjointBuild <- function(object, calcSens) {
  ...
  assign("Rx_pow_di", function(a, b) a^b, .env)
  assign("Rx_pow", function(a, b) a^b, .env)
  assign("expit", function(x, a = 0, b = 1) a + (b - a) / (1 + exp(-x)), .env)
  assign("logit", function(x, a = 0, b = 1) log((x - a) / (b - x)), .env)
  .setState <- function(x) {
    for (i in seq_len(.ns)) {
      assign(.st[i], x[i], envir = .env)
    }
  }
  .setParams <- function(params) {
    .nm <- names(params)
    for (i in seq_along(params)) {
      assign(.nm[i], params[[i]], envir = .env)
    }
  }
  ...
  list(
    ...
    FXe = function(x, p) {
      .setState(x)
      .setParams(p)
      matrix(.evList(.FXexprP), .ns, .ns)
    },
    FPe = function(x, p) {
      .setState(x)
      .setParams(p)
      matrix(.evList(.FPexprP), .ns, .np)
    },
    ## pre-parsed bioavailability forms for the dose jump
    fStrP = .fStrP,
    dFdpStrP = .dFdpStrP,
    evScalar = function(pexpr, params) {
      .setState(numeric(.ns))
      .setParams(params)
      eval(pexpr, .env)
    }
...
.rxRk4Step <- function(build, Xn, params, h) {
  .k1 <- build$Fe(Xn, params)
  .a2 <- Xn + h / 2 * .k1
  .k2 <- build$Fe(.a2, params)
  .a3 <- Xn + h / 2 * .k2
  .k3 <- build$Fe(.a3, params)
  .a4 <- Xn + h * .k3
  .k4 <- build$Fe(.a4, params)
  list(Xnext = Xn + h / 6 * (.k1 + 2 * .k2 + 2 * .k3 + .k4), stages = list(Xn, .a2, .a3, .a4))
}
...
#' @export
#' @keywords internal
.rxDiscreteAdjointGrad <- function(build, stages, params, h, obsSteps, cov, doses = NULL, lam0Cov = NULL) {
  .ns <- build$ns
  .np <- build$np
  .nStep <- length(stages)
  .lam <- numeric(.ns)
  ...
  for (n in .nStep:1) {
    ...
    .k3b <- h / 3 * .Xbar
    .k4b <- h / 6 * .Xbar
    .a4b <- t(build$FXe(.a4, params)) %*% .k4b
    .Xbar <- .Xbar + .a4b
    .k3b <- .k3b + h * .a4b
    .mu <- .mu + t(build$FPe(.a4, params)) %*% .k4b
    .a3b <- t(build$FXe(.a3, params)) %*% .k3b
    .Xbar <- .Xbar + .a3b
    .k2b <- .k2b + h / 2 * .a3b
    .mu <- .mu + t(build$FPe(.a3, params)) %*% .k3b
    .a2b <- t(build$FXe(.a2, params)) %*% .k2b
    .Xbar <- .Xbar + .a2b
    .k1b <- .k1b + h / 2 * .a2b
    .mu <- .mu + t(build$FPe(.a2, params)) %*% .k2b
    .a1b <- t(build$FXe(.a1, params)) %*% .k1b
    .Xbar <- .Xbar + .a1b
    .mu <- .mu + t(build$FPe(.a1, params)) %*% .k1b
    .Ybar <- as.vector(.Xbar) # dG/dY_{n-1}, Y = post-dose state at step n-1
    for (.d in .dstep[[as.character(n - 1L)]]) {
      # transpose of the dose-parameter jump
      .j <- .rxDiscreteDoseJump(build, .d, params)
...
.rxAdjointExpand <- function(object, calcSens, stiff = FALSE) {
  ...
  .odeLines <- vapply(
    .st,
    function(i) {
      sprintf("d/dt(%s)=%s", i, .fromSE(paste0("rx__d_dt_", i, "__")))
    },
    character(1)
  )
  ...
  for (k in seq_len(.ns)) {
    ...
    if (!is.null(.fSE)) {
      .fLines <- c(.fLines, sprintf("f(%s)=%s", .st[k], rxode2::rxFromSE(.fSE)))
    }
    .lSE <- get0(paste0("rx_lag_", .st[k], "_"), envir = .model, inherits = FALSE)
    if (!is.null(.lSE)) {
      .fLines <- c(.fLines, sprintf("alag(%s)=%s", .st[k], rxode2::rxFromSE(.lSE)))
    }
    .rSE <- get0(paste0("rx_rate_", .st[k], "_"), envir = .model, inherits = FALSE)
    if (!is.null(.rSE)) {
      .fLines <- c(.fLines, sprintf("rate(%s)=%s", .st[k], rxode2::rxFromSE(.rSE)))
    }
  ...
  for (i in seq_len(.ns)) {
    for (j in seq_len(.ns)) {
      .fxLines <- c(
        .fxLines,
        sprintf("rx__adjFX_%d_%d__=%s", i - 1L, j - 1L, .fromSE(paste0("rx__df_", .st[i], "_dy_", .st[j], "__")))
      )
    }
  }
  ...
  .fxdLines <- character(0)
  .tauLines <- character(0)
  .dtauLines <- character(0)
  .hasDelayAdj <- FALSE
  .fxdMat <- matrix("0", .ns, .ns)
  .tauMat <- matrix("0", .ns, .ns)
  ...
  for (i in seq_len(.ns)) {
    ...
    for (.dc in .dcalls) {
      ...
      if (.dcTxt %in% .seen) {
        next
      } # identical() subst already captured all copies
      .seen <- c(.seen, .dcTxt)
      ...
      .djTxt <- gsub(.gName, paste0("delay(", .stateJ, ",", .tau, ")"), rxode2::rxFromSE(.dj), fixed = TRUE)
      .fxdMat[i, .j] <- if (identical(.fxdMat[i, .j], "0")) {
        .djTxt
      } else {
      ...
      if (any(.dtauByP != "0")) {
        .dtauList[[i]] <- c(.dtauList[[i]], list(list(stateJ = .stateJ, tau = .tau, djac = .djTxt, dtauByP = .dtauByP)))
        if (is.null(.dtauMat[[i]])) {
          .dtauMat[[i]] <- vector("list", .ns)
        }
  ...
  for (k in seq_len(.ns)) {
    ...
    for (p in seq_len(.np)) {
      .expr <- if (is.null(.fSE)) {
        ...
        .d <- symengine::D(.fSE, .rxSEres(calcSens[p]))
        rxode2::rxFromSE(.d)
      }
    ...
    for (k in seq_len(.ns)) {
      ...
      for (p in seq_len(.np)) {
        .expr <- if (is.null(.lSE)) {
          ...
          .d <- symengine::D(.lSE, .rxSEres(calcSens[p]))
          rxode2::rxFromSE(.d)
        }
  ...
  .drateLines <- character(0)
  .hasRateAdj <- FALSE
  for (k in seq_len(.ns)) {
    if (
      !is.null(get0(paste0("rx_rate_", .st[k], "_"), envir = .model, inherits = FALSE)) ||
        !is.null(get0(paste0("rx_dur_", .st[k], "_"), envir = .model, inherits = FALSE))
    ) {
      .hasRateAdj <- TRUE
    }
  }
  if (.hasRateAdj) {
    for (k in seq_len(.ns)) {
      ...
      for (p in seq_len(.np)) {
        .expr <- if (!is.null(.rSE)) {
          # rate(): d(rate)/dtheta
          .d <- symengine::D(.rSE, .rxSEres(calcSens[p]))
          rxode2::rxFromSE(.d)
        } else if (!is.null(.dSE)) {
          # dur(): d(1/dur)/dtheta = -dur'/dur^2
          .d <- -symengine::D(.dSE, .rxSEres(calcSens[p])) / (.dSE * .dSE)
          rxode2::rxFromSE(.d)
        } else {
          "0"
        }
      ...
      for (j in seq_len(.ns)) {
        ...
        for (p in seq_len(.np)) {
          .expr <- if (is.null(.dfx)) {
            ...
            .d <- symengine::D(.dfx, .rxSEres(calcSens[p]))
            rxode2::rxFromSE(.d)
          }
        ...
        for (m in seq_len(.ns)) {
          .expr <- if (is.null(.dfx)) {
            ...
            .d <- symengine::D(.dfx, .rxSEres(.st[m]))
            rxode2::rxFromSE(.d)
          }
  ...
  .afterStiff <- if (isTRUE(stiff)) {
    .ns * .ns + 2L * .ns * .np + .ns * .ns * .np + .ns * .ns * .ns
  } else {
    .ns * .ns + 2L * .ns * .np
  }
