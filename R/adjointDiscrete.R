...
.rxDiscreteAdjointBuild <- function(object, calcSens) {
  ...
  .fromSE <- function(nm) { .d <- get0(nm, envir = .model, inherits = FALSE)
  rxode2::rxFromSE(.d) }
  ...
  .setParams <- function(params) { .nm <- names(params)
  for (i in seq_along(params)) assign(.nm[i], params[[i]], envir = .env) }
  ...
  list(st = .st, ns = .ns, np = .np, calcSens = calcSens,
       ...
       evP = function(cmtStr, params) { .setState(numeric(.ns))
       .setParams(params)
       eval(cmtStr, .env) },
       Fe  = function(x, p) { .setState(x)
       .setParams(p)
       .evList(.FexprP) },
       FXe = function(x, p) { .setState(x)
       .setParams(p)
       matrix(.evList(.FXexprP), .ns, .ns) },
       FPe = function(x, p) { .setState(x)
       .setParams(p)
       matrix(.evList(.FPexprP), .ns, .np) },
       ## pre-parsed bioavailability forms for the dose jump
       fStrP = .fStrP, dFdpStrP = .dFdpStrP,
       evScalar = function(pexpr, params) { .setState(numeric(.ns))
       .setParams(params)
       eval(pexpr, .env) })
}
...
.rxAdjointExpand <- function(object, calcSens, stiff = FALSE) {
  ...
  for (k in seq_len(.ns)) {
    .fSE <- get0(paste0("rx_f_", .st[k], "_"), envir = .model, inherits = FALSE)
    for (p in seq_len(.np)) {
      .expr <- if (is.null(.fSE)) "0" else { .d <- symengine::D(.fSE, calcSens[p])
      rxode2::rxFromSE(.d) }
  ...
  if (.hasLagAdj) {
    for (k in seq_len(.ns)) {
      .lSE <- get0(paste0("rx_lag_", .st[k], "_"), envir = .model, inherits = FALSE)
      for (p in seq_len(.np)) {
        .expr <- if (is.null(.lSE)) "0" else { .d <- symengine::D(.lSE, calcSens[p])
        rxode2::rxFromSE(.d) }
    ...
    for (k in seq_len(.ns)) {
      ...
      for (p in seq_len(.np)) {
        .expr <- if (!is.null(.rSE)) {              # rate(): d(rate)/dtheta
          .d <- symengine::D(.rSE, calcSens[p])
          rxode2::rxFromSE(.d)
        } else if (!is.null(.dSE)) {                # dur(): d(1/dur)/dtheta = -dur'/dur^2
          .d <- -symengine::D(.dSE, calcSens[p]) / (.dSE * .dSE)
          rxode2::rxFromSE(.d)
        } else "0"
  ...
  if (isTRUE(stiff)) {
    for (i in seq_len(.ns)) for (j in seq_len(.ns)) {
      ...
      for (p in seq_len(.np)) {
        .expr <- if (is.null(.dfx)) "0" else { .d <- symengine::D(.dfx, calcSens[p])
        rxode2::rxFromSE(.d) }
        .jpLines <- c(.jpLines, sprintf("rx__adjJp_%d_%d_%d__=%s", i - 1L, j - 1L, p - 1L, .expr))
      }
      for (m in seq_len(.ns)) {
        .expr <- if (is.null(.dfx)) "0" else { .d <- symengine::D(.dfx, .st[m])
        rxode2::rxFromSE(.d) }
