#ifndef ODE_RKTMY7S_BRIDGE_H_
#define ODE_RKTMY7S_BRIDGE_H_

#include "ode/ode_rktmy7s.h"

class RxRktmy7s : public ode::OdeRktmy7s {
    t_dydt                  dydt_;
    int                    *full_neq_;
...
public:
    RxRktmy7s(t_dydt dydt, int *full_neq, int neqOde,
            rx_solving_options_ind *ind, double *yp,
            long unsigned max_steps)
        : ode::OdeRktmy7s((unsigned long)neqOde),
...
protected:
    ...
    void after_step(double) override {
        if (ind_->err != 0) throw std::runtime_error("rktmy7s: ode error");
        if (nstep_ > max_steps_) {
            if (ind_->rc[0] == 0) ind_->rc[0] = -2019;
            ind_->err = 1;
            throw std::runtime_error("rktmy7s: max steps exceeded");
        }
    }
};

#endif // ODE_RKTMY7S_BRIDGE_H_
