...
extern "C" void ind_grk4a_0(rx_solve *rx, rx_solving_options *op, int solveid, int *neq,
...
extern "C" void ind_grk4a(rx_solve *rx, int solveid,
                            t_dydt c_dydt, t_update_inis u_inis) {
  rx_solving_options *op = rx->op;
  int neq[2] = { op->neq, 0 };
  ind_grk4a_0(rx, op, solveid, neq, c_dydt, u_inis);
}
...
#pragma omp atomic read
    ...
    if (localAbort == 0) {
      ...
      ind_grk4a_0(rx, op, solveid, neq, dydt, update_inis);
      if (op->badSolve) {
#ifdef _OPENMP
...
extern "C" void grk4a_solveWith1Pt(int *neq, double *yp, double *xp, double xout,
