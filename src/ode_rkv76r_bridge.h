#ifndef ODE_RKV76R_BRIDGE_H_
#define ODE_RKV76R_BRIDGE_H_

#include "ode/ode_rkv76r.h"

class RxRkv76r : public ode::OdeRkv76r {
    t_dydt                  dydt_;
    int                    *full_neq_;
...
public:
    RxRkv76r(t_dydt dydt, int *full_neq, int neqOde,
            rx_solving_options_ind *ind, double *yp,
            long unsigned max_steps)
        : ode::OdeRkv76r((unsigned long)neqOde),
...
protected:
    ...
    void step_(double dt) override {
        ...
        // Stage 10
        for (unsigned long i = 0; i < neq_; i++)
            soltemp_[i] = sol_[i] + dt * (a101*k_[0][i]+a103*k_[2][i]+a104*k_[3][i]+a105*k_[4][i]+a106*k_[5][i]+a107*k_[6][i]);
        t_stage_ = t_ + dt;
    ...
    void after_step(double) override {
        if (ind_->err != 0) throw std::runtime_error("rkv76r: ode error");
        if (nstep_ > max_steps_) {
            if (ind_->rc[0] == 0) ind_->rc[0] = -2019;
            ind_->err = 1;
            throw std::runtime_error("rkv76r: max steps exceeded");
        }
    }
};

#endif // ODE_RKV76R_BRIDGE_H_
