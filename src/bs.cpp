...
extern "C" void ind_bs_0(rx_solve *rx, rx_solving_options *op, int solveid, int *neq,
        ...
        if (handleExtraDose(neq, ind->BadDose, ind->InfusionRate, ind->dose, yp, xout,
            ...
            if (!localBadSolve && !isSameTime(ind->extraDoseNewXout, xp)) {
              ...
              if (neqOde > 0) {
                  ...
                  sys.xout_ = ind->extraDoseNewXout;
                  sys.sign_ = (dt > 0) ? 1 : -1;
                  try {
...
extern "C" void ind_bs(rx_solve *rx, int solveid,
  ...
  neq[0] = op->neq;
  neq[1] = 0;
  ind_bs_0(rx, op, solveid, neq, c_dydt, u_inis);
}
