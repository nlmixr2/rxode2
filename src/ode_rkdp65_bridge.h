#ifndef ODE_RKDP65_BRIDGE_H_
#define ODE_RKDP65_BRIDGE_H_

#include "ode/ode_rkdp65.h"

class RxRkdp65 : public ode::OdeRkdp65 {
    t_dydt                  dydt_;
    int                    *full_neq_;
...
public:
    RxRkdp65(t_dydt dydt, int *full_neq, int neqOde,
            rx_solving_options_ind *ind, double *yp,
            long unsigned max_steps)
        : ode::OdeRkdp65((unsigned long)neqOde),
...
protected:
    ...
    void step_(double dt) override {
        ...
        // Stage 8
        for (unsigned long i = 0; i < neq_; i++)
            soltemp_[i] = sol_[i] + dt * (a81*k_[0][i]+a82*k_[1][i]+a83*k_[2][i]+a84*k_[3][i]+a85*k_[4][i]+a86*k_[5][i]);
        t_stage_ = t_ + dt;
        ode_fun_(soltemp_, k_[7]);
        // Primary (higher-order) solution -> sol_
        for (unsigned long i = 0; i < neq_; i++)
            sol_[i] = sol_[i]+dt*(b1*k_[0][i]+b3*k_[2][i]+b4*k_[3][i]+b5*k_[4][i]+b6*k_[5][i]+b7*k_[6][i]+b8*k_[7][i]);
    ...
    void after_step(double) override {
        if (ind_->err != 0) throw std::runtime_error("rkdp65: ode error");
        if (nstep_ > max_steps_) {
            if (ind_->rc[0] == 0) ind_->rc[0] = -2019;
            ind_->err = 1;
            throw std::runtime_error("rkdp65: max steps exceeded");
        }
    }
};

#endif // ODE_RKDP65_BRIDGE_H_
