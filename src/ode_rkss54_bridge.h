#ifndef ODE_RKSS54_BRIDGE_H_
#define ODE_RKSS54_BRIDGE_H_

#include "ode/ode_rkss54.h"

class RxRkss54 : public ode::OdeRkss54 {
    t_dydt                  dydt_;
    int                    *full_neq_;
...
public:
    RxRkss54(t_dydt dydt, int *full_neq, int neqOde,
            rx_solving_options_ind *ind, double *yp,
            long unsigned max_steps)
        : ode::OdeRkss54((unsigned long)neqOde),
...
protected:
    ...
    void after_step(double) override {
        if (ind_->err != 0) throw std::runtime_error("rkss54: ode error");
        if (nstep_ > max_steps_) {
            if (ind_->rc[0] == 0) ind_->rc[0] = -2019;
            ind_->err = 1;
            throw std::runtime_error("rkss54: max steps exceeded");
        }
    }
};

#endif // ODE_RKSS54_BRIDGE_H_
