#ifndef ODE_RKLK5B_BRIDGE_H_
#define ODE_RKLK5B_BRIDGE_H_
#include "ode/ode_rklk5b.h"
class RxRklk5b : public ode::OdeRklk5b {
    t_dydt dydt_; int *full_neq_; rx_solving_options_ind *ind_; double t_stage_;
public:
    RxRklk5b(t_dydt dydt, int *full_neq, int neqOde, rx_solving_options_ind *ind, double *yp)
        : ode::OdeRklk5b((unsigned long)neqOde), dydt_(dydt), full_neq_(full_neq), ind_(ind), t_stage_(0.0)
    { set_sol_external(yp); }
protected:
    ...
    void step_(double dt) override {
        ...
        for(unsigned long i=0;i<neq_;i++) sol_[i]+=dt*(c3*k_[2][i]+c5*k_[4][i]+c6*k_[5][i]);
    }
};
#endif
