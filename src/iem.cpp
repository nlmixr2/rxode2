...
extern "C" void ind_iem_0(rx_solve *rx, rx_solving_options *op, int solveid, int *neq,
  ...
  // implicit_euler is a fixed-step stepper; epsilon controls Newton convergence
      ...
      if (ind->err){
        ...
        if (handleExtraDose(neq, ind->BadDose, ind->InfusionRate, ind->dose, yp, xout,
            ...
            if (!localBadSolve && !isSameTime(ind->extraDoseNewXout, xp)) {
              ...
              const char* err_msg = "iem failed";
              postSolve(neq, &istate, ind->rc, &i, yp, &err_msg, 14, true, ind, op, rx);
            ...
            if (!localBadSolve) {
              ...
              if (!isSameTime(xout, ind->extraDoseNewXout)) {
                ...
                copyLinCmt(neq, ind, op, yp);
                const char* err_msg = "iem failed";
                postSolve(neq, &istate, ind->rc, &idx, yp, &err_msg, 14, false, ind, op, rx);
                if (*(ind->rc) < 0) localBadSolve = 1;
...
extern "C" void ind_iem(rx_solve *rx, int solveid,
                          t_dydt c_dydt, t_update_inis u_inis){
  rx_solving_options *op = rx->op;
  int neq[2];
  neq[0] = op->neq;
  neq[1] = 0;
  ind_iem_0(rx, op, solveid, neq, c_dydt, u_inis);
}
...
#pragma omp atomic read
    ...
    if (localAbort == 0){
      ...
      ind_iem_0(rx, op, solveid, neq, dydt, update_inis);
...
extern "C" void iem_solveWith1Pt(int *neq, double *yp, double *xp, double xout,
