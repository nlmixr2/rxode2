...
struct symplectic_system {
    ...
    symplectic_system(rx_solving_options_ind *ind, t_dydt c_dydt, int* neq)
    ...
    struct coor_func_t {
        const symplectic_system& parent_;
        coor_func_t(const symplectic_system& parent) : parent_(parent) {}
    ...
    struct momentum_func_t {
        const symplectic_system& parent_;
        momentum_func_t(const symplectic_system& parent) : parent_(parent) {}
...
static inline void sem_do_steps(rx_solving_options_ind *ind, rx_solving_options *op, symplectic_system& sys, std::pair<zero_copy_state, zero_copy_state>& state, double xp, double xout) {
  typedef boost::numeric::odeint::symplectic_euler<zero_copy_state> stepper_type;
...
extern "C" void ind_sem_0(rx_solve *rx, rx_solving_options *op, int solveid, int *neq,
  ...
  symplectic_system sys(ind, c_dydt, neq);
      ...
      if (ind->err){
        ...
        if (handleExtraDose(neq, ind->BadDose, ind->InfusionRate, ind->dose, yp, xout,
            ...
            if (!localBadSolve && !isSameTime(ind->extraDoseNewXout, xp)) {
              ...
              if (neqOde > 0) {
                  ...
                  sem_do_steps(ind, op, sys, state, xp, ind->extraDoseNewXout);
                  std::copy(state.first.begin(), state.first.end(), yp);
                  std::copy(state.second.begin(), state.second.begin() + (neqOde - N), yp + N);
              }

              copyLinCmt(neq, ind, op, yp);
              const char* err_msg = "sem failed";
              postSolve(neq, &istate, ind->rc, &i, yp, &err_msg, 15, true, ind, op, rx);
              ...
              if (!isSameTime(xout, ind->extraDoseNewXout)) {
                preSolve(op, ind, ind->extraDoseNewXout, xout, yp);
                  ...
                  if (neqOde > 0) {
                      ...
                      auto state = std::make_pair(q, p);
                      sem_do_steps(ind, op, sys, state, ind->extraDoseNewXout, xout);
                      std::copy(state.first.begin(), state.first.end(), yp);
                      std::copy(state.second.begin(), state.second.begin() + (neqOde - N), yp + N);
                  }
                ...
                copyLinCmt(neq, ind, op, yp);
                const char* err_msg = "sem failed";
                postSolve(neq, &istate, ind->rc, &idx, yp, &err_msg, 15, false, ind, op, rx);
                if (*(ind->rc) < 0) localBadSolve = 1;
        ...
        if (!localBadSolve && !isSameTime(xout, xp)) {
          ...
          if (neqOde > 0) {
              ...
              auto state = std::make_pair(q, p);
              sem_do_steps(ind, op, sys, state, xp, xout);
              std::copy(state.first.begin(), state.first.end(), yp);
              std::copy(state.second.begin(), state.second.begin() + (neqOde - N), yp + N);
          }
...
extern "C" void ind_sem(rx_solve *rx, int solveid,
                          t_dydt c_dydt, t_update_inis u_inis){
  rx_solving_options *op = rx->op;
  int neq[2];
  neq[0] = op->neq;
  neq[1] = 0;
  ind_sem_0(rx, op, solveid, neq, c_dydt, u_inis);
}
...
#pragma omp atomic read
    ...
    if (localAbort == 0){
      ...
      ind_sem_0(rx, op, solveid, neq, dydt, update_inis);
...
extern "C" void sem_solveWith1Pt(int *neq, double *yp, double *xp, double xout,
 int *istate, rx_solving_options *op, rx_solving_options_ind *ind) {
  ...
  if (neqOde > 0) {
      ...
      auto state = std::make_pair(q, p);
      symplectic_system sys(ind, dydt, neq);
      sem_do_steps(ind, op, sys, state, *xp, xout);
      std::copy(state.first.begin(), state.first.end(), yp);
      std::copy(state.second.begin(), state.second.begin() + (neqOde - N), yp + N);
      if (ind->rc[0] < 0) {
