#ifndef ODE_RKLK5A_BRIDGE_H_
#define ODE_RKLK5A_BRIDGE_H_
#include "ode/ode_rklk5a.h"
class RxRklk5a : public ode::OdeRklk5a {
    t_dydt dydt_; int *full_neq_; rx_solving_options_ind *ind_; double t_stage_;
public:
    RxRklk5a(t_dydt dydt, int *full_neq, int neqOde, rx_solving_options_ind *ind, double *yp)
        : ode::OdeRklk5a((unsigned long)neqOde), dydt_(dydt), full_neq_(full_neq), ind_(ind), t_stage_(0.0)
    { set_sol_external(yp); }
protected:
    ...
    void step_(double dt) override {
        ...
        for(unsigned long i=0;i<neq_;i++) soltemp_[i]=sol_[i]+dt*(b41*k_[0][i]+b42*k_[1][i]);
        ...
        for(unsigned long i=0;i<neq_;i++) sol_[i]+=dt*(c1*k_[0][i]+c3*k_[2][i]+c5*k_[4][i]+c6*k_[5][i]);
    }
};
#endif
