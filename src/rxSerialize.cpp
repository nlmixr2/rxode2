...
// (linCmtSensStrategy/linCmtMaxDosesInPhase2/linCmtSupersededDoseCeiling)
// after sensH.
...
#define W_RX_BOOL(field) { uint8_t _b = (uint8_t)(rx->field); sWrite(f, &_b, 1, #field); }
  ...
  W_RX_DBL(sensH);
  W_RX_I32(linCmtSensStrategy); W_RX_I32(linCmtMaxDosesInPhase2);
  W_RX_I32(linCmtSupersededDoseCeiling);
...
#define R_RX_BOOL(field) rx->field = (decltype(rx->field))sReadU8(f, #field)
  ...
  R_RX_DBL(sensH);
  if (fmt >= 4u) {
    R_RX_I32(linCmtSensStrategy); R_RX_I32(linCmtMaxDosesInPhase2);
    R_RX_I32(linCmtSupersededDoseCeiling);
  } else {
    rx->linCmtSensStrategy = 0; // forward (sequential)
    rx->linCmtMaxDosesInPhase2 = 5;
    rx->linCmtSupersededDoseCeiling = 30;
  }
