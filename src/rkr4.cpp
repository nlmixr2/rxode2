...
static inline void rkr4_do_steps(RxRkr4 *solver, rx_solving_options_ind *ind, rx_solving_options *op,
...
extern "C" void ind_rkr4_0(rx_solve *rx, rx_solving_options *op, int solveid, int *neq,
  ...
  RxRkr4 *solver = NULL;
  if (neqOde > 0) {
    solver = new RxRkr4(c_dydt, neq, neqOde, ind, getSolve(0));
      ...
      if (ind->err) {
        ...
        if (handleExtraDose(neq, ind->BadDose, ind->InfusionRate, ind->dose, yp, xout,
          ...
          if (!localBadSolve && !isSameTime(ind->extraDoseNewXout, xp)) {
            ...
            if (neqOde > 0)
              rkr4_do_steps(solver, ind, op, c_dydt, neq, yp, xp, ind->extraDoseNewXout);
          ...
          if (!localBadSolve) {
            ...
            if (!isSameTime(xout, ind->extraDoseNewXout)) {
              ...
              if (neqOde > 0)
                rkr4_do_steps(solver, ind, op, c_dydt, neq, yp, ind->extraDoseNewXout, xout);
        ...
        if (!localBadSolve && !isSameTime(xout, xp)) {
          ...
          if (neqOde > 0)
            rkr4_do_steps(solver, ind, op, c_dydt, neq, yp, xp, xout);
...
#pragma omp atomic read
    ...
    if (localAbort == 0) {
      ...
      ind_rkr4_0(rx, op, _id, neq, dydt, update_inis);
...
extern "C" void rkr4_solveWith1Pt(int *neq, double *yp, double *xp, double xout,
  ...
  int neqOde = eff - op->numLin - op->numLinSens;
    ...
    if (neqOde > 0) {
    RxRkr4 solver(dydt, neq, neqOde, ind, yp);
    solver.set_quiet(true);
    rkr4_do_steps(&solver, ind, op, dydt, neq, yp, *xp, xout);
