...
static inline void rkf1412_do_steps(rx_solving_options_ind *ind, rx_solving_options *op,
  ...
  RxRkf1412 solver(c_dydt, neq, neqOde, ind, yp, (long unsigned)op->mxstep);
      ...
      if (ind->err) {
        ...
        if (handleExtraDose(neq, ind->BadDose, ind->InfusionRate, ind->dose, yp, xout,
          ...
          if (!localBadSolve && !isSameTime(ind->extraDoseNewXout, xp)) {
            ...
            if (neqOde > 0)
              rkf1412_do_steps(ind, op, c_dydt, neq, yp, xp, ind->extraDoseNewXout);
          ...
          if (!localBadSolve) {
            ...
            if (!isSameTime(xout, ind->extraDoseNewXout)) {
              ...
              if (neqOde > 0)
                rkf1412_do_steps(ind, op, c_dydt, neq, yp, ind->extraDoseNewXout, xout);
              copyLinCmt(neq, ind, op, yp);
              const char* err_msg = "rkf1412 failed";
        ...
        if (!localBadSolve && !isSameTime(xout, xp)) {
          ...
          if (neqOde > 0)
            rkf1412_do_steps(ind, op, c_dydt, neq, yp, xp, xout);
          copyLinCmt(neq, ind, op, yp);
          const char* err_msg = "rkf1412 failed";
          postSolve(neq, &istate, ind->rc, &i, yp, &err_msg, 7, true, ind, op, rx);
          if (*(ind->rc) < 0) localBadSolve = 1;
...
extern "C" void rkf1412_solveWith1Pt(int *neq, double *yp, double *xp, double xout,
  ...
  if (neqOde > 0) {
    rkf1412_do_steps(ind, op, dydt, neq, yp, *xp, xout);
