...
extern "C" void ind_ck54_0(rx_solve *rx, rx_solving_options *op, int solveid, int *neq,
  ...
  typedef boost::numeric::odeint::runge_kutta_cash_karp54<zero_copy_state> error_stepper_type;
      ...
      if (ind->err){
        ...
        if (handleExtraDose(neq, ind->BadDose, ind->InfusionRate, ind->dose, yp, xout,
            ...
            if (!localBadSolve && !isSameTime(ind->extraDoseNewXout, xp)) {
              ...
              const char* err_msg = "ck54 failed";
...
extern "C" void ind_ck54(rx_solve *rx, int solveid,
                          t_dydt c_dydt, t_update_inis u_inis){
  rx_solving_options *op = rx->op;
  int neq[2];
  neq[0] = op->neq;
  neq[1] = 0;
  ind_ck54_0(rx, op, solveid, neq, c_dydt, u_inis);
}

extern "C" void par_ck54(rx_solve *rx){
  rx_solving_options *op = rx->op;
#ifdef _OPENMP
  int cores = op->cores;
#else
...
#pragma omp atomic read
    ...
    if (localAbort == 0){
      ...
      ind_ck54_0(rx, op, solveid, neq, dydt, update_inis);
