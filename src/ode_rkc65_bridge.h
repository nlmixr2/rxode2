#ifndef ODE_RKC65_BRIDGE_H_
#define ODE_RKC65_BRIDGE_H_

#include "ode/ode_rkc65.h"

class RxRkc65 : public ode::OdeRkc65 {
    t_dydt                  dydt_;
    int                    *full_neq_;
...
public:
    RxRkc65(t_dydt dydt, int *full_neq, int neqOde,
            rx_solving_options_ind *ind, double *yp,
            long unsigned max_steps)
        : ode::OdeRkc65((unsigned long)neqOde),
...
protected:
    ...
    void step_(double dt) override {
        ...
        for (unsigned long i = 0; i < neq_; i++)
            solemb_[i] = sol_[i] + dt * ((d1 - b1) * k_[0][i] + (d3 - b3) * k_[2][i] + (d4 - b4) * k_[3][i] + (d5 - b5) * k_[4][i] + (d6 - b6) * k_[5][i] + (d7 - b7) * k_[6][i] + (d8 - b8) * k_[7][i] + (d9 - b9) * k_[8][i]);
    }

    void after_step(double) override {
        if (ind_->err != 0) throw std::runtime_error("rkc65: ode error");
        if (nstep_ > max_steps_) {
            if (ind_->rc[0] == 0) ind_->rc[0] = -2019;
            ind_->err = 1;
            throw std::runtime_error("rkc65: max steps exceeded");
        }
    }
};

#endif // ODE_RKC65_BRIDGE_H_
