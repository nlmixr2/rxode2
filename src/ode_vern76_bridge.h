...
class RxVern76 : public ode::OdeVern76 {
    t_dydt                  dydt_;
    int                    *full_neq_;
...
public:
    RxVern76(t_dydt dydt, int *full_neq, int neqOde,
             rx_solving_options_ind *ind, double *yp,
             long unsigned max_steps)
        : ode::OdeVern76((unsigned long)neqOde),
...
protected:
    ...
    void step_(double dt) override {
        ...
        // Stage 9 (a92=0)
        for (unsigned long i = 0; i < neq_; i++)
            soltemp_[i] = sol_[i] + dt * (a91 * k_[0][i] + a93 * k_[2][i]
                                         + a94 * k_[3][i] + a95 * k_[4][i]
                                         + a96 * k_[5][i] + a97 * k_[6][i]
                                         + a98 * k_[7][i]);
        t_stage_ = t_ + c9 * dt;
        ode_fun_(soltemp_, k_[8]);

        // Stage 10 (a102=0, a108=a109=0)
        for (unsigned long i = 0; i < neq_; i++)
            soltemp_[i] = sol_[i] + dt * (a101 * k_[0][i] + a103 * k_[2][i]
                                         + a104 * k_[3][i] + a105 * k_[4][i]
                                         + a106 * k_[5][i] + a107 * k_[6][i]);
        t_stage_ = t_ + c10 * dt;
        ode_fun_(soltemp_, k_[9]);

        // solemb_ from OLD sol_ (6th-order embedded, uses k_[9] via d10)
        for (unsigned long i = 0; i < neq_; i++)
            solemb_[i] = sol_[i] + dt * (d1  * k_[0][i] + d4  * k_[3][i]
                                        + d5  * k_[4][i] + d6  * k_[5][i]
                                        + d7  * k_[6][i] + d10 * k_[9][i]);

        // sol_ update to 7th-order (no k_[9])
        for (unsigned long i = 0; i < neq_; i++)
    ...
    void after_step(double /*t*/) override {
        if (ind_->err != 0) throw std::runtime_error("vern76: ode error");
        if (nstep_ > max_steps_) {
            if (ind_->rc[0] == 0) ind_->rc[0] = -2019;
            ind_->err = 1;
            throw std::runtime_error("vern76: max steps exceeded");
        }
    }
};

#endif // ODE_VERN76_BRIDGE_H_
