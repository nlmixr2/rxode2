#ifndef ODE_RKT98A_BRIDGE_H_
#define ODE_RKT98A_BRIDGE_H_

#include "ode/ode_rkt98a.h"

class RxRkt98a : public ode::OdeRkt98a {
    t_dydt                  dydt_;
    int                    *full_neq_;
...
public:
    RxRkt98a(t_dydt dydt, int *full_neq, int neqOde,
            rx_solving_options_ind *ind, double *yp,
            long unsigned max_steps)
        : ode::OdeRkt98a((unsigned long)neqOde),
...
protected:
    ...
    void step_(double dt) override {
        ...
        // Stage 16
        for (unsigned long i = 0; i < neq_; i++)
            soltemp_[i] = sol_[i] + dt * (a161*k_[0][i]+a166*k_[5][i]+a167*k_[6][i]+a168*k_[7][i]+a169*k_[8][i]+a1610*k_[9][i]+a1611*k_[10][i]+ a1612*k_[11][i]+a1613*k_[12][i]+a1614*k_[13][i]);
        t_stage_ = t_ + dt;
        ode_fun_(soltemp_, k_[15]);
        // Primary (higher-order) solution -> sol_
        for (unsigned long i = 0; i < neq_; i++)
            sol_[i] = sol_[i]+dt*(b1*k_[0][i]+b8*k_[7][i]+b9*k_[8][i]+b10*k_[9][i]+b11*k_[10][i]+b12*k_[11][i]+b13*k_[12][i]+b14*k_[13][i]+b15*k_[14][i]+b16*k_[15][i]);
    ...
    void after_step(double) override {
        if (ind_->err != 0) throw std::runtime_error("rkt98a: ode error");
        if (nstep_ > max_steps_) {
            if (ind_->rc[0] == 0) ind_->rc[0] = -2019;
            ind_->err = 1;
            throw std::runtime_error("rkt98a: max steps exceeded");
        }
    }
};

#endif // ODE_RKT98A_BRIDGE_H_
