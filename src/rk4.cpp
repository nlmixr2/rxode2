...
static inline void rk4_do_steps(rx_solving_options_ind *ind, rx_solving_options *op, rxode2_system& sys, zero_copy_state& state, double xp, double xout) {
  typedef boost::numeric::odeint::runge_kutta4<zero_copy_state> stepper_type;
...
extern "C" void ind_rk4_0(rx_solve *rx, rx_solving_options *op, int solveid, int *neq,
    ...
    if (getEvid(ind, ind->ix[i]) != 3 && !isSameTime(xout, xp)) {
      if (ind->err){
        ...
        if (handleExtraDose(neq, ind->BadDose, ind->InfusionRate, ind->dose, yp, xout,
            ...
            if (!localBadSolve && !isSameTime(ind->extraDoseNewXout, xp)) {
              ...
              if (neqOde > 0) {
                  ...
                  rk4_do_steps(ind, op, sys, state, xp, ind->extraDoseNewXout);
              }

              copyLinCmt(neq, ind, op, yp);
              const char* err_msg = "rk4 failed";
            ...
            if (!localBadSolve) {
              ...
              if (!isSameTime(xout, ind->extraDoseNewXout)) {
                ...
                if (neqOde > 0) {
                    zero_copy_state state(yp, neqOde);
                    rk4_do_steps(ind, op, sys, state, ind->extraDoseNewXout, xout);
                }
        ...
        if (!localBadSolve && !isSameTime(xout, xp)) {
          ...
          if (neqOde > 0) {
              zero_copy_state state(yp, neqOde);
              rk4_do_steps(ind, op, sys, state, xp, xout);
          }
...
extern "C" void ind_rk4(rx_solve *rx, int solveid,
                          t_dydt c_dydt, t_update_inis u_inis){
  rx_solving_options *op = rx->op;
  int neq[2];
  neq[0] = op->neq;
  neq[1] = 0;
  ind_rk4_0(rx, op, solveid, neq, c_dydt, u_inis);
}

extern "C" void par_rk4(rx_solve *rx){
  rx_solving_options *op = rx->op;
#ifdef _OPENMP
  int cores = op->cores;
#else
...
#pragma omp atomic read
    ...
    if (localAbort == 0){
      ...
      ind_rk4_0(rx, op, solveid, neq, dydt, update_inis);
