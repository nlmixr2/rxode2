#ifndef ODE_RKO129_BRIDGE_H_
#define ODE_RKO129_BRIDGE_H_

#include "ode/ode_rko129.h"

class RxRko129 : public ode::OdeRko129 {
    t_dydt                  dydt_;
    int                    *full_neq_;
...
public:
    RxRko129(t_dydt dydt, int *full_neq, int neqOde,
            rx_solving_options_ind *ind, double *yp,
            long unsigned max_steps)
        : ode::OdeRko129((unsigned long)neqOde),
...
protected:
    ...
    void step_(double dt) override {
        ...
        // Stage 22
        for (unsigned long i = 0; i < neq_; i++)
            soltemp_[i] = sol_[i] + dt * (a221*k_[0][i]+a222*k_[1][i]+a223*k_[2][i]+a224*k_[3][i]+a225*k_[4][i]+a226*k_[5][i]+a227*k_[6][i]+ a228*k_[7][i]+a229*k_[8][i]+a2210*k_[9][i]+a2211*k_[10][i]+a2212*k_[11][i]+a2213*k_[12][i]+ a2214*k_[13][i]+a2215*k_[14][i]+a2216*k_[15][i]+a2217*k_[16][i]+a2218*k_[17][i]+a2219*k_[18][i]+ a2220*k_[19][i]+a2221*k_[20][i]);
        t_stage_ = t_ + c22 * dt;
        ode_fun_(soltemp_, k_[21]);
        // Stage 23
        for (unsigned long i = 0; i < neq_; i++)
            soltemp_[i] = sol_[i] + dt * (a231*k_[0][i]+a232*k_[1][i]+a233*k_[2][i]+a234*k_[3][i]+a235*k_[4][i]+a236*k_[5][i]+a237*k_[6][i]+ a238*k_[7][i]+a239*k_[8][i]+a2310*k_[9][i]+a2311*k_[10][i]+a2312*k_[11][i]+a2313*k_[12][i]+ a2314*k_[13][i]+a2315*k_[14][i]+a2316*k_[15][i]+a2317*k_[16][i]+a2318*k_[17][i]+a2319*k_[18][i]+ a2320*k_[19][i]+a2321*k_[20][i]+a2322*k_[21][i]);
        t_stage_ = t_ + c23 * dt;
        ode_fun_(soltemp_, k_[22]);
        // Stage 24
        for (unsigned long i = 0; i < neq_; i++)
            soltemp_[i] = sol_[i] + dt * (a241*k_[0][i]+a242*k_[1][i]+a243*k_[2][i]+a244*k_[3][i]+a245*k_[4][i]+a246*k_[5][i]+a247*k_[6][i]+ a248*k_[7][i]+a249*k_[8][i]+a2410*k_[9][i]+a2411*k_[10][i]+a2412*k_[11][i]+a2413*k_[12][i]+ a2414*k_[13][i]+a2415*k_[14][i]+a2416*k_[15][i]+a2417*k_[16][i]+a2418*k_[17][i]+a2419*k_[18][i]+ a2420*k_[19][i]+a2421*k_[20][i]+a2422*k_[21][i]+a2423*k_[22][i]);
        t_stage_ = t_ + c24 * dt;
        ode_fun_(soltemp_, k_[23]);
        // Stage 25
        for (unsigned long i = 0; i < neq_; i++)
            soltemp_[i] = sol_[i] + dt * (a251*k_[0][i]+a252*k_[1][i]+a253*k_[2][i]+a254*k_[3][i]+a255*k_[4][i]+a256*k_[5][i]+a257*k_[6][i]+ a258*k_[7][i]+a259*k_[8][i]+a2510*k_[9][i]+a2511*k_[10][i]+a2512*k_[11][i]+a2513*k_[12][i]+ a2514*k_[13][i]+a2515*k_[14][i]+a2516*k_[15][i]+a2517*k_[16][i]+a2518*k_[17][i]+a2519*k_[18][i]+ a2520*k_[19][i]+a2521*k_[20][i]+a2522*k_[21][i]+a2523*k_[22][i]+a2524*k_[23][i]);
        t_stage_ = t_ + dt;
        ode_fun_(soltemp_, k_[24]);
        // Stage 26
        for (unsigned long i = 0; i < neq_; i++)
            soltemp_[i] = sol_[i] + dt * (a261*k_[0][i]+a262*k_[1][i]+a263*k_[2][i]+a264*k_[3][i]+a265*k_[4][i]+a266*k_[5][i]+a267*k_[6][i]+ a268*k_[7][i]+a269*k_[8][i]+a2610*k_[9][i]+a2611*k_[10][i]+a2612*k_[11][i]+a2613*k_[12][i]+ a2614*k_[13][i]+a2615*k_[14][i]+a2616*k_[15][i]+a2617*k_[16][i]+a2618*k_[17][i]+a2619*k_[18][i]+ a2620*k_[19][i]+a2621*k_[20][i]+a2622*k_[21][i]+a2623*k_[22][i]+a2624*k_[23][i]+a2625*k_[24][i]);
        t_stage_ = t_ + c26 * dt;
        ode_fun_(soltemp_, k_[25]);
        // Stage 27
        for (unsigned long i = 0; i < neq_; i++)
            soltemp_[i] = sol_[i] + dt * (a271*k_[0][i]+a272*k_[1][i]+a273*k_[2][i]+a274*k_[3][i]+a275*k_[4][i]+a276*k_[5][i]+a277*k_[6][i]+ a278*k_[7][i]+a279*k_[8][i]+a2710*k_[9][i]+a2711*k_[10][i]+a2712*k_[11][i]+a2713*k_[12][i]+ a2714*k_[13][i]+a2715*k_[14][i]+a2716*k_[15][i]+a2717*k_[16][i]+a2718*k_[17][i]+a2719*k_[18][i]+ a2720*k_[19][i]+a2721*k_[20][i]+a2722*k_[21][i]+a2723*k_[22][i]+a2724*k_[23][i]+a2725*k_[24][i]+ a2726*k_[25][i]);
        t_stage_ = t_ + c27 * dt;
        ode_fun_(soltemp_, k_[26]);
        // Stage 28
        for (unsigned long i = 0; i < neq_; i++)
            soltemp_[i] = sol_[i] + dt * (a281*k_[0][i]+a282*k_[1][i]+a283*k_[2][i]+a284*k_[3][i]+a285*k_[4][i]+a286*k_[5][i]+a287*k_[6][i]+ a288*k_[7][i]+a289*k_[8][i]+a2810*k_[9][i]+a2811*k_[10][i]+a2812*k_[11][i]+a2813*k_[12][i]+ a2814*k_[13][i]+a2815*k_[14][i]+a2816*k_[15][i]+a2817*k_[16][i]+a2818*k_[17][i]+a2819*k_[18][i]+ a2820*k_[19][i]+a2821*k_[20][i]+a2822*k_[21][i]+a2823*k_[22][i]+a2824*k_[23][i]+a2825*k_[24][i]+ a2826*k_[25][i]+a2827*k_[26][i]);
        t_stage_ = t_ + c28 * dt;
        ode_fun_(soltemp_, k_[27]);
        // Stage 29
        for (unsigned long i = 0; i < neq_; i++)
            soltemp_[i] = sol_[i] + dt * (a291*k_[0][i]+a292*k_[1][i]+a293*k_[2][i]+a294*k_[3][i]+a295*k_[4][i]+a296*k_[5][i]+a297*k_[6][i]+ a298*k_[7][i]+a299*k_[8][i]+a2910*k_[9][i]+a2911*k_[10][i]+a2912*k_[11][i]+a2913*k_[12][i]+ a2914*k_[13][i]+a2915*k_[14][i]+a2916*k_[15][i]+a2917*k_[16][i]+a2918*k_[17][i]+a2919*k_[18][i]+ a2920*k_[19][i]+a2921*k_[20][i]+a2922*k_[21][i]+a2923*k_[22][i]+a2924*k_[23][i]+a2925*k_[24][i]+ a2926*k_[25][i]+a2927*k_[26][i]+a2928*k_[27][i]);
        t_stage_ = t_ + dt;
        ode_fun_(soltemp_, k_[28]);
        // Primary (higher-order) solution -> sol_
        for (unsigned long i = 0; i < neq_; i++)
            sol_[i] = sol_[i]+dt*(b1*k_[0][i]+b2*k_[1][i]+b3*k_[2][i]+b4*k_[3][i]+b5*k_[4][i]+b6*k_[5][i]+b7*k_[6][i]+b8*k_[7][i]+b9*k_[8][i]+b10*k_[9][i]+ b11*k_[10][i]+b12*k_[11][i]+b13*k_[12][i]+b14*k_[13][i]+b15*k_[14][i]+b16*k_[15][i]+b17*k_[16][i]+b18*k_[17][i]+ b19*k_[18][i]+b20*k_[19][i]+b21*k_[20][i]+b22*k_[21][i]+b23*k_[22][i]+b24*k_[23][i]+b25*k_[24][i]+b26*k_[25][i]+ b27*k_[26][i]+b28*k_[27][i]+b29*k_[28][i]);
        // Embedded (lower-order) solution -> solemb_
        for (unsigned long i = 0; i < neq_; i++)
            solemb_[i] = sol_[i] + dt * ((d1 - b1) * k_[0][i] + (d2 - b2) * k_[1][i] + (d3 - b3) * k_[2][i] + (d4 - b4) * k_[3][i] + (d5 - b5) * k_[4][i] + (d6 - b6) * k_[5][i] + (d7 - b7) * k_[6][i] + (d8 - b8) * k_[7][i] + (d9 - b9) * k_[8][i] + (d10 - b10) * k_[9][i] + (d11 - b11) * k_[10][i] + (d12 - b12) * k_[11][i] + (d13 - b13) * k_[12][i] + (d14 - b14) * k_[13][i] + (d15 - b15) * k_[14][i] + (d16 - b16) * k_[15][i] + (d17 - b17) * k_[16][i] + (d18 - b18) * k_[17][i] + (d19 - b19) * k_[18][i] + (d20 - b20) * k_[19][i] + (d21 - b21) * k_[20][i] + (d22 - b22) * k_[21][i] + (d23 - b23) * k_[22][i] + (d24 - b24) * k_[23][i] + (d25 - b25) * k_[24][i] + (d26 - b26) * k_[25][i] + (d27 - b27) * k_[26][i] + (d28 - b28) * k_[27][i] + (d29 - b29) * k_[28][i]);
    }

    void after_step(double) override {
        if (ind_->err != 0) throw std::runtime_error("rko129: ode error");
        if (nstep_ > max_steps_) {
            if (ind_->rc[0] == 0) ind_->rc[0] = -2019;
            ind_->err = 1;
            throw std::runtime_error("rko129: max steps exceeded");
        }
    }
};

#endif // ODE_RKO129_BRIDGE_H_
