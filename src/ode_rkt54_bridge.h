#ifndef ODE_RKT54_BRIDGE_H_
#define ODE_RKT54_BRIDGE_H_

#include "ode/ode_rkt54.h"

class RxRkt54 : public ode::OdeRkt54 {
    t_dydt                  dydt_;
    int                    *full_neq_;
...
public:
    RxRkt54(t_dydt dydt, int *full_neq, int neqOde,
            rx_solving_options_ind *ind, double *yp,
            long unsigned max_steps)
        : ode::OdeRkt54((unsigned long)neqOde),
...
protected:
    ...
    void step_(double dt) override {
        ...
        for (unsigned long i = 0; i < neq_; i++)
            sol_[i] = sol_[i] + dt*(b1*k_[0][i] + b2*k_[1][i] + b3*k_[2][i] + b4*k_[3][i] + b5*k_[4][i] + b6*k_[5][i]);
        // Stage 7 (FSAL: k_[6] = f(t+dt, y_new))
        t_stage_ = t_ + dt;
        ode_fun_(sol_, k_[6]);
        // Embedded (lower-order) solution -> solemb_
        for (unsigned long i = 0; i < neq_; i++)
            solemb_[i] = sol_[i] + dt * ((d1 - b1) * k_[0][i] + (d2 - b2) * k_[1][i] + (d3 - b3) * k_[2][i] + (d4 - b4) * k_[3][i] + (d5 - b5) * k_[4][i] + (d6 - b6) * k_[5][i] + (d7 - b7) * k_[6][i]);
    }

    void after_step(double) override {
        if (ind_->err != 0) throw std::runtime_error("rkt54: ode error");
        if (nstep_ > max_steps_) {
            if (ind_->rc[0] == 0) ind_->rc[0] = -2019;
            ind_->err = 1;
            throw std::runtime_error("rkt54: max steps exceeded");
        }
    }
};

#endif // ODE_RKT54_BRIDGE_H_
