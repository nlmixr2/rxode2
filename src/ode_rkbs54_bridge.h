#ifndef ODE_RKBS54_BRIDGE_H_
#define ODE_RKBS54_BRIDGE_H_

#include "ode/ode_rkbs54.h"

class RxRkbs54 : public ode::OdeRkbs54 {
    t_dydt                  dydt_;
    int                    *full_neq_;
...
public:
    RxRkbs54(t_dydt dydt, int *full_neq, int neqOde,
            rx_solving_options_ind *ind, double *yp,
            long unsigned max_steps)
        : ode::OdeRkbs54((unsigned long)neqOde),
...
protected:
    ...
    void step_(double dt) override {
        ...
        for (unsigned long i = 0; i < neq_; i++)
            sol_[i] = sol_[i]+dt*(b1*k_[0][i]+b3*k_[2][i]+b4*k_[3][i]+b5*k_[4][i]+b6*k_[5][i]+b7*k_[6][i]);
    ...
    void after_step(double) override {
        if (ind_->err != 0) throw std::runtime_error("rkbs54: ode error");
        if (nstep_ > max_steps_) {
            if (ind_->rc[0] == 0) ind_->rc[0] = -2019;
            ind_->err = 1;
            throw std::runtime_error("rkbs54: max steps exceeded");
        }
    }
};

#endif // ODE_RKBS54_BRIDGE_H_
