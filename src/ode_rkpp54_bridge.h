#ifndef ODE_RKPP54_BRIDGE_H_
#define ODE_RKPP54_BRIDGE_H_

#include "ode/ode_rkpp54.h"

class RxRkpp54 : public ode::OdeRkpp54 {
    t_dydt                  dydt_;
    int                    *full_neq_;
...
public:
    RxRkpp54(t_dydt dydt, int *full_neq, int neqOde,
            rx_solving_options_ind *ind, double *yp,
            long unsigned max_steps)
        : ode::OdeRkpp54((unsigned long)neqOde),
...
protected:
    ...
    void after_step(double) override {
        if (ind_->err != 0) throw std::runtime_error("rkpp54: ode error");
        if (nstep_ > max_steps_) {
            if (ind_->rc[0] == 0) ind_->rc[0] = -2019;
            ind_->err = 1;
            throw std::runtime_error("rkpp54: max steps exceeded");
        }
    }
};

#endif // ODE_RKPP54_BRIDGE_H_
