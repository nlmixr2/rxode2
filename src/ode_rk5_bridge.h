#ifndef ODE_RK5_BRIDGE_H_
#define ODE_RK5_BRIDGE_H_
#include "ode/ode_rk5.h"
class RxRk5 : public ode::OdeRk5 {
    t_dydt dydt_; int *full_neq_; rx_solving_options_ind *ind_; double t_stage_;
public:
    RxRk5(t_dydt dydt, int *full_neq, int neqOde, rx_solving_options_ind *ind, double *yp)
        : ode::OdeRk5((unsigned long)neqOde), dydt_(dydt), full_neq_(full_neq), ind_(ind), t_stage_(0.0)
    { set_sol_external(yp); }
protected:
    ...
    void step_(double dt) override {
        ...
        for(unsigned long i=0;i<neq_;i++) soltemp_[i]=sol_[i]+dt*(b61*k_[0][i]+b62*k_[1][i]+b63*k_[2][i]+b64*k_[3][i]);
        t_stage_=t_+a6*dt; ode_fun_(soltemp_, k_[5]);
        for(unsigned long i=0;i<neq_;i++) sol_[i]+=dt*(c1*k_[0][i]+c3*k_[2][i]+c4*k_[3][i]+c5*k_[4][i]+c6*k_[5][i]);
    }
};
#endif
