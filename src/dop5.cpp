...
extern "C" void ind_dop5_0(rx_solve *rx, rx_solving_options *op, int solveid, int *neq,
  ...
  typedef boost::numeric::odeint::runge_kutta_dopri5<zero_copy_state> error_stepper_type;
      ...
      if (ind->err){
        ...
        if (handleExtraDose(neq, ind->BadDose, ind->InfusionRate, ind->dose, yp, xout,
            ...
            if (!localBadSolve && !isSameTime(ind->extraDoseNewXout, xp)) {
              ...
              const char* err_msg = "dop5 failed";
...
extern "C" void ind_dop5(rx_solve *rx, int solveid,
  ...
  neq[0] = op->neq;
  neq[1] = 0;
  ind_dop5_0(rx, op, solveid, neq, c_dydt, u_inis);
}
...
#pragma omp atomic read
    ...
    if (localAbort == 0){
      ...
      ind_dop5_0(rx, op, solveid, neq, dydt, update_inis);
