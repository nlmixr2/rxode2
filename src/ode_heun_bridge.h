#ifndef ODE_HEUN_BRIDGE_H_
#define ODE_HEUN_BRIDGE_H_
#include "ode/ode_heun.h"
class RxHeun : public ode::OdeHeun {
    t_dydt dydt_; int *full_neq_; rx_solving_options_ind *ind_; double t_stage_;
public:
    RxHeun(t_dydt dydt, int *full_neq, int neqOde, rx_solving_options_ind *ind, double *yp)
        : ode::OdeHeun((unsigned long)neqOde), dydt_(dydt), full_neq_(full_neq), ind_(ind), t_stage_(0.0)
    { set_sol_external(yp); }
protected:
    ...
    void step_(double dt) override {
        ...
        for (unsigned long i=0;i<neq_;i++) sol_[i]+=dt*(b1*k_[0][i]+b2*k_[1][i]);
    }
};
#endif
