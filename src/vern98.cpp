...
#undef min
#undef max
#include "ode_vern98_bridge.h"

static inline void vern98_do_steps(RxVern98 *solver, rx_solving_options_ind *ind, rx_solving_options *op,
...
extern "C" void ind_vern98_0(rx_solve *rx, rx_solving_options *op, int solveid, int *neq,
  ...
  RxVern98 *solver = NULL;
  if (neqOde > 0) {
    solver = new RxVern98(c_dydt, neq, neqOde, ind, getSolve(0), (long unsigned)op->mxstep);
      ...
      if (ind->err) {
        ...
        if (handleExtraDose(neq, ind->BadDose, ind->InfusionRate, ind->dose, yp, xout,
          ...
          if (!localBadSolve && !isSameTime(ind->extraDoseNewXout, xp)) {
            ...
            if (neqOde > 0)
              vern98_do_steps(solver, ind, op, c_dydt, neq, yp, xp, ind->extraDoseNewXout);
            copyLinCmt(neq, ind, op, yp);
            const char* err_msg = "vern98 failed";
          ...
          if (!localBadSolve) {
            ...
            if (!isSameTime(xout, ind->extraDoseNewXout)) {
              ...
              if (neqOde > 0)
                vern98_do_steps(solver, ind, op, c_dydt, neq, yp, ind->extraDoseNewXout, xout);
              copyLinCmt(neq, ind, op, yp);
              const char* err_msg = "vern98 failed";
        ...
        if (!localBadSolve && !isSameTime(xout, xp)) {
          ...
          if (neqOde > 0)
            vern98_do_steps(solver, ind, op, c_dydt, neq, yp, xp, xout);
          copyLinCmt(neq, ind, op, yp);
          const char* err_msg = "vern98 failed";
          postSolve(neq, &istate, ind->rc, &i, yp, &err_msg, 7, true, ind, op, rx);
          if (*(ind->rc) < 0) localBadSolve = 1;
...
extern "C" void ind_vern98(rx_solve *rx, int solveid,
                             t_dydt c_dydt, t_update_inis u_inis) {
  rx_solving_options *op = rx->op;
  int neq[2] = { op->neq, 0 };
  ind_vern98_0(rx, op, solveid, neq, c_dydt, u_inis);
}

extern "C" void par_vern98(rx_solve *rx) {
  rx_solving_options *op = rx->op;
#ifdef _OPENMP
  int cores = op->cores;
#else
...
#pragma omp atomic read
    ...
    if (localAbort == 0) {
      ...
      ind_vern98_0(rx, op, solveid, neq, dydt, update_inis);
      if (op->badSolve) {
#ifdef _OPENMP
...
extern "C" void vern98_solveWith1Pt(int *neq, double *yp, double *xp, double xout,
                                      int *istate, rx_solving_options *op,
                                      rx_solving_options_ind *ind) {
  int eff = rxEffNeq(ind, op);
  int neqOde = eff - op->numLin - op->numLinSens;
    if (neqOde > 0) {
    RxVern98 solver(dydt, neq, neqOde, ind, yp, (long unsigned)op->mxstep);
    solver.set_quiet(true);
    if (op->atol2 != NULL) solver.set_abstol(op->ATOL);
    if (op->rtol2 != NULL) solver.set_reltol(op->RTOL);
    vern98_do_steps(&solver, ind, op, dydt, neq, yp, *xp, xout);
