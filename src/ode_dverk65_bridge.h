#ifndef ODE_DVERK65_BRIDGE_H_
#define ODE_DVERK65_BRIDGE_H_

#include "ode/ode_dverk65.h"

class RxDverk65 : public ode::OdeDverk65 {
    t_dydt                  dydt_;
    int                    *full_neq_;
...
public:
    RxDverk65(t_dydt dydt, int *full_neq, int neqOde,
            rx_solving_options_ind *ind, double *yp,
            long unsigned max_steps)
        : ode::OdeDverk65((unsigned long)neqOde),
...
protected:
    ...
    void step_(double dt) override {
        ...
        // Stage 7
        for (unsigned long i = 0; i < neq_; i++)
            soltemp_[i] = sol_[i] + dt * (a71*k_[0][i]+a72*k_[1][i]+a73*k_[2][i]+a74*k_[3][i]+a75*k_[4][i]);
        t_stage_ = t_ + c7 * dt;
        ode_fun_(soltemp_, k_[6]);
        // Stage 8
        for (unsigned long i = 0; i < neq_; i++)
            soltemp_[i] = sol_[i] + dt * (a81*k_[0][i]+a82*k_[1][i]+a83*k_[2][i]+a84*k_[3][i]+a85*k_[4][i]+a87*k_[6][i]);
        t_stage_ = t_ + dt;
        ode_fun_(soltemp_, k_[7]);
        // Primary (higher-order) solution -> sol_
        for (unsigned long i = 0; i < neq_; i++)
            sol_[i] = sol_[i]+dt*(b1*k_[0][i]+b3*k_[2][i]+b4*k_[3][i]+b5*k_[4][i]+b7*k_[6][i]+b8*k_[7][i]);
        // Embedded (lower-order) solution -> solemb_
        for (unsigned long i = 0; i < neq_; i++)
            solemb_[i] = sol_[i] + dt * ((d1 - b1) * k_[0][i] + (d3 - b3) * k_[2][i] + (d4 - b4) * k_[3][i] + (d5 - b5) * k_[4][i] + (d6 - b6) * k_[5][i] + (d7 - b7) * k_[6][i] + (d8 - b8) * k_[7][i]);
    }

    void after_step(double) override {
        if (ind_->err != 0) throw std::runtime_error("dverk65: ode error");
        if (nstep_ > max_steps_) {
            if (ind_->rc[0] == 0) ind_->rc[0] = -2019;
            ind_->err = 1;
            throw std::runtime_error("dverk65: max steps exceeded");
        }
    }
};

#endif // ODE_DVERK65_BRIDGE_H_
