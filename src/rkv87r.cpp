...
#undef min
#undef max
#include "ode_rkv87r_bridge.h"

static inline void rkv87r_do_steps(RxRkv87r *solver, rx_solving_options_ind *ind, rx_solving_options *op,
...
extern "C" void ind_rkv87r_0(rx_solve *rx, rx_solving_options *op, int solveid, int *neq,
  ...
  RxRkv87r *solver = NULL;
  if (neqOde > 0) {
    solver = new RxRkv87r(c_dydt, neq, neqOde, ind, getSolve(0), (long unsigned)op->mxstep);
      ...
      if (ind->err) {
        ...
        if (handleExtraDose(neq, ind->BadDose, ind->InfusionRate, ind->dose, yp, xout,
          ...
          if (!localBadSolve && !isSameTime(ind->extraDoseNewXout, xp)) {
            ...
            if (neqOde > 0)
              rkv87r_do_steps(solver, ind, op, c_dydt, neq, yp, xp, ind->extraDoseNewXout);
            copyLinCmt(neq, ind, op, yp);
            const char* err_msg = "rkv87r failed";
          ...
          if (!localBadSolve) {
            ...
            if (!isSameTime(xout, ind->extraDoseNewXout)) {
              ...
              if (neqOde > 0)
                rkv87r_do_steps(solver, ind, op, c_dydt, neq, yp, ind->extraDoseNewXout, xout);
              copyLinCmt(neq, ind, op, yp);
              const char* err_msg = "rkv87r failed";
        ...
        if (!localBadSolve && !isSameTime(xout, xp)) {
          ...
          if (neqOde > 0)
            rkv87r_do_steps(solver, ind, op, c_dydt, neq, yp, xp, xout);
          copyLinCmt(neq, ind, op, yp);
          const char* err_msg = "rkv87r failed";
          postSolve(neq, &istate, ind->rc, &i, yp, &err_msg, 7, true, ind, op, rx);
          if (*(ind->rc) < 0) localBadSolve = 1;
...
extern "C" void ind_rkv87r(rx_solve *rx, int solveid,
                            t_dydt c_dydt, t_update_inis u_inis) {
  rx_solving_options *op = rx->op;
  int neq[2]; neq[0] = op->neq; neq[1] = 0;
  ind_rkv87r_0(rx, op, solveid, neq, c_dydt, u_inis);
}
...
#pragma omp atomic read
    ...
    if (localAbort == 0) {
      ...
      ind_rkv87r_0(rx, op, solveid, neq, dydt, update_inis);
      if (op->badSolve) {
#ifdef _OPENMP
...
extern "C" void rkv87r_solveWith1Pt(int *neq, double *yp, double *xp, double xout,
                                     int *istate, rx_solving_options *op,
                                     rx_solving_options_ind *ind) {
  int eff = rxEffNeq(ind, op);
  int neqOde = eff - op->numLin - op->numLinSens;
    if (neqOde > 0) {
    RxRkv87r solver(dydt, neq, neqOde, ind, yp, (long unsigned)op->mxstep);
    solver.set_quiet(true);
    if (op->atol2 != NULL) solver.set_abstol(op->ATOL);
    if (op->rtol2 != NULL) solver.set_reltol(op->RTOL);
    rkv87r_do_steps(&solver, ind, op, dydt, neq, yp, *xp, xout);
