#ifndef ODE_RKDP85_BRIDGE_H_
#define ODE_RKDP85_BRIDGE_H_

#include "ode/ode_rkdp85.h"

class RxRkdp85 : public ode::OdeRkdp85 {
    t_dydt                  dydt_;
    int                    *full_neq_;
...
public:
    RxRkdp85(t_dydt dydt, int *full_neq, int neqOde,
            rx_solving_options_ind *ind, double *yp,
            long unsigned max_steps)
        : ode::OdeRkdp85((unsigned long)neqOde),
...
protected:
    ...
    void step_(double dt) override {
        ...
        for (unsigned long i = 0; i < neq_; i++)
            solemb_[i] = sol_[i] + dt * ((d1 - b1) * k_[0][i] + (d6 - b6) * k_[5][i] + (d7 - b7) * k_[6][i] + (d8 - b8) * k_[7][i] + (d9 - b9) * k_[8][i] + (d10 - b10) * k_[9][i] + (d11 - b11) * k_[10][i] + (d12 - b12) * k_[11][i]);
    }

    void after_step(double) override {
        if (ind_->err != 0) throw std::runtime_error("rkdp85: ode error");
        if (nstep_ > max_steps_) {
            if (ind_->rc[0] == 0) ind_->rc[0] = -2019;
            ind_->err = 1;
            throw std::runtime_error("rkdp85: max steps exceeded");
        }
    }
};

#endif // ODE_RKDP85_BRIDGE_H_
