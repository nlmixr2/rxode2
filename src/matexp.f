...
!---  A SINGULAR PADE DENOMINATOR IS REPORTED THROUGH IFLAG, NOT THROUGH
!     THE R API: THIS RUNS INSIDE indLin()'S OMP PARALLEL REGION, WHERE
!     CALLING RWarn FROM A WORKER THREAD IS A CRASH HAZARD.  NORMALISE TO
!     THE DOCUMENTED CONVENTION (<0 IS A PROBLEM); DGESV RETURNS >0 HERE.
      IF ( IFLAG.NE.0 ) THEN
         IFLAG = -IABS(IFLAG)
         RETURN
      ENDIF
