...
static inline void em_do_steps(rx_solving_options_ind *ind, rx_solving_options *op, rxode2_system& sys, zero_copy_state& state, double xp, double xout) {
  typedef boost::numeric::odeint::euler<zero_copy_state> stepper_type;
...
extern "C" void ind_em_0(rx_solve *rx, rx_solving_options *op, int solveid, int *neq,
    ...
    if (getEvid(ind, ind->ix[i]) != 3 && !isSameTime(xout, xp)) {
      if (ind->err){
        ...
        if (handleExtraDose(neq, ind->BadDose, ind->InfusionRate, ind->dose, yp, xout,
            ...
            if (!localBadSolve && !isSameTime(ind->extraDoseNewXout, xp)) {
              ...
              if (neqOde > 0) {
                  ...
                  em_do_steps(ind, op, sys, state, xp, ind->extraDoseNewXout);
              }

              copyLinCmt(neq, ind, op, yp);
              const char* err_msg = "em failed";
              postSolve(neq, &istate, ind->rc, &i, yp, &err_msg, 20, true, ind, op, rx);
            ...
            if (!localBadSolve) {
              ...
              if (!isSameTime(xout, ind->extraDoseNewXout)) {
                ...
                if (neqOde > 0) {
                    zero_copy_state state(yp, neqOde);
                    em_do_steps(ind, op, sys, state, ind->extraDoseNewXout, xout);
                }

                copyLinCmt(neq, ind, op, yp);
                const char* err_msg = "em failed";
                postSolve(neq, &istate, ind->rc, &idx, yp, &err_msg, 20, false, ind, op, rx);
                if (*(ind->rc) < 0) localBadSolve = 1;
        ...
        if (!localBadSolve && !isSameTime(xout, xp)) {
          ...
          if (neqOde > 0) {
              zero_copy_state state(yp, neqOde);
              em_do_steps(ind, op, sys, state, xp, xout);
          }
...
extern "C" void ind_em(rx_solve *rx, int solveid,
                          t_dydt c_dydt, t_update_inis u_inis){
  rx_solving_options *op = rx->op;
  int neq[2];
  neq[0] = op->neq;
  neq[1] = 0;
  ind_em_0(rx, op, solveid, neq, c_dydt, u_inis);
}
...
#pragma omp atomic read
    ...
    if (localAbort == 0){
      ...
      ind_em_0(rx, op, solveid, neq, dydt, update_inis);
...
extern "C" void em_solveWith1Pt(int *neq, double *yp, double *xp, double xout,
