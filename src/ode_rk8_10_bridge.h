#ifndef ODE_RK8_10_BRIDGE_H_
#define ODE_RK8_10_BRIDGE_H_
#include "ode/ode_rk8_10.h"
class RxRk8_10 : public ode::OdeRk8_10 {
    t_dydt dydt_; int *full_neq_; rx_solving_options_ind *ind_; double t_stage_;
public:
    RxRk8_10(t_dydt dydt, int *full_neq, int neqOde, rx_solving_options_ind *ind, double *yp)
        : ode::OdeRk8_10((unsigned long)neqOde), dydt_(dydt), full_neq_(full_neq), ind_(ind), t_stage_(0.0)
    { set_sol_external(yp); }
protected:
    ...
    void step_(double dt) override {
        ...
        for(unsigned long i=0;i<neq_;i++) soltemp_[i]=sol_[i]+dt*aa4*(k_[0][i]+b43*k_[3][i]);
        ...
        t_stage_=t_+dt; ode_fun_(soltemp_, k_[7]);
        ...
        for(unsigned long i=0;i<neq_;i++) soltemp_[i]=sol_[i]+dt*aa9*(b90*k_[0][i]+b92*k_[2][i]+b93*k_[3][i]+b94*k_[4][i]+b95*k_[5][i]+b96*k_[6][i]+b97*k_[7][i]+b98*k_[8][i]);
        t_stage_=t_+dt; ode_fun_(soltemp_, k_[9]);
        for(unsigned long i=0;i<neq_;i++) sol_[i]+=dt*cv*(c0*k_[0][i]+c3*k_[3][i]+c4*k_[4][i]+c5*k_[5][i]+c6*k_[6][i]+c8*k_[8][i]+c9*k_[9][i]);
    }
};
#endif
