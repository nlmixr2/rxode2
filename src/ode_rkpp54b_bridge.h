#ifndef ODE_RKPP54B_BRIDGE_H_
#define ODE_RKPP54B_BRIDGE_H_

#include "ode/ode_rkpp54b.h"

class RxRkpp54b : public ode::OdeRkpp54b {
    t_dydt                  dydt_;
    int                    *full_neq_;
...
public:
    RxRkpp54b(t_dydt dydt, int *full_neq, int neqOde,
            rx_solving_options_ind *ind, double *yp,
            long unsigned max_steps)
        : ode::OdeRkpp54b((unsigned long)neqOde),
...
protected:
    ...
    void after_step(double) override {
        if (ind_->err != 0) throw std::runtime_error("rkpp54b: ode error");
        if (nstep_ > max_steps_) {
            if (ind_->rc[0] == 0) ind_->rc[0] = -2019;
            ind_->err = 1;
            throw std::runtime_error("rkpp54b: max steps exceeded");
        }
    }
};

#endif // ODE_RKPP54B_BRIDGE_H_
