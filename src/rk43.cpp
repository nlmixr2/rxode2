...
static inline void rk43_do_steps(RxRk43 *solver, rx_solving_options_ind *ind, rx_solving_options *op,
...
extern "C" void ind_rk43_0(rx_solve *rx, rx_solving_options *op, int solveid, int *neq,
  ...
  RxRk43 *solver = NULL;
  if (neqOde > 0) {
    solver = new RxRk43(c_dydt, neq, neqOde, ind, getSolve(0), (long unsigned)op->mxstep);
      ...
      if (ind->err) {
        ...
        if (handleExtraDose(neq, ind->BadDose, ind->InfusionRate, ind->dose, yp, xout,
          ...
          if (!localBadSolve && !isSameTime(ind->extraDoseNewXout, xp)) {
            ...
            if (neqOde > 0)
              rk43_do_steps(solver, ind, op, c_dydt, neq, yp, xp, ind->extraDoseNewXout);
          ...
          if (!localBadSolve) {
            ...
            if (!isSameTime(xout, ind->extraDoseNewXout)) {
              ...
              if (neqOde > 0)
                rk43_do_steps(solver, ind, op, c_dydt, neq, yp, ind->extraDoseNewXout, xout);
        ...
        if (!localBadSolve && !isSameTime(xout, xp)) {
          ...
          if (neqOde > 0)
            rk43_do_steps(solver, ind, op, c_dydt, neq, yp, xp, xout);
...
#pragma omp atomic read
    ...
    if (localAbort == 0) {
      ...
      ind_rk43_0(rx, op, _id, neq, dydt, update_inis);
...
extern "C" void rk43_solveWith1Pt(int *neq, double *yp, double *xp, double xout,
  ...
  int neqOde = eff - op->numLin - op->numLinSens;
    ...
    if (neqOde > 0) {
    RxRk43 solver(dydt, neq, neqOde, ind, yp, (long unsigned)op->mxstep);
    solver.set_quiet(true);
    if (op->atol2 != NULL) solver.set_abstol(op->ATOL);
    if (op->rtol2 != NULL) solver.set_reltol(op->RTOL);
    rk43_do_steps(&solver, ind, op, dydt, neq, yp, *xp, xout);
