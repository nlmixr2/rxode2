...
extern "C" void setIndPointersByThread(rx_solving_options_ind *ind) {
  ...
  if (ncmt) {
    ...
    ind->linCmtConstants = _globals.linCmtConstants + 8*omp_get_thread_num();
  } else {
    ind->alag = NULL;
    ind->cRate =NULL;
    ind->cDur = NULL;
  ...
  case 3: // NumericMatrix
    {
      ...
      for (unsigned int simNum = rx->nsim; simNum--;) {
        ...
        for (unsigned int id = rx->nsub; id--;) {
          ...
          ind->BadDose = &_globals.gBadDose[(op->neq+op->extraCmt)*cid];
          // Hmax defined above.
          ind->sim = simNum+1;
...
SEXP rxSolve_(const RObject &obj, const List &rxControl,
  ...
  if (rxSolveDat->isRxSolve || rxSolveDat->isEnvironment){
    ...
    _globals.linCmtConstants = _globals.gIndSim + n7; //[n9]
    _globals.timeThread = _globals.linCmtConstants + n9;
