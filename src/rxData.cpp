...
SEXP rxSolve_(const RObject &obj, const List &rxControl,
  ...
  if (rxSolveDat->isRxSolve || rxSolveDat->isEnvironment){
    ...
    op->neq = state.size(); // with new linear compartment models,
                            // state.size() includes the solved
                            // systems (and possibly the sensitivities
                            // too)
    ...
    getLinInfo(rxSolveDat->mv, numLinSens,
               numLin, depotLin);
    op->numLinSens = numLin;
    op->numLin = numLin;
