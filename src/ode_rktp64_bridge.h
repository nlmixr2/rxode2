#ifndef ODE_RKTP64_BRIDGE_H_
#define ODE_RKTP64_BRIDGE_H_

#include "ode/ode_rktp64.h"

class RxRktp64 : public ode::OdeRktp64 {
    t_dydt                  dydt_;
    int                    *full_neq_;
...
public:
    RxRktp64(t_dydt dydt, int *full_neq, int neqOde,
            rx_solving_options_ind *ind, double *yp,
            long unsigned max_steps)
        : ode::OdeRktp64((unsigned long)neqOde),
...
protected:
    ...
    void after_step(double) override {
        if (ind_->err != 0) throw std::runtime_error("rktp64: ode error");
        if (nstep_ > max_steps_) {
            if (ind_->rc[0] == 0) ind_->rc[0] = -2019;
            ind_->err = 1;
            throw std::runtime_error("rktp64: max steps exceeded");
        }
    }
};

#endif // ODE_RKTP64_BRIDGE_H_
