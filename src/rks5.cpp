...
#undef min
#undef max
#include "ode_rks5_bridge.h"

static inline void rks5_do_steps(RxRks5 *solver, rx_solving_options_ind *ind, rx_solving_options *op,
...
extern "C" void ind_rks5_0(rx_solve *rx, rx_solving_options *op, int solveid, int *neq,
  ...
  RxRks5 *solver = NULL;
  if (neqOde > 0) {
    solver = new RxRks5(c_dydt, neq, neqOde, ind, getSolve(0));
      ...
      if (ind->err) {
        ...
        if (handleExtraDose(neq, ind->BadDose, ind->InfusionRate, ind->dose, yp, xout,
          ...
          if (!localBadSolve && !isSameTime(ind->extraDoseNewXout, xp)) {
            ...
            if (neqOde > 0)
              rks5_do_steps(solver, ind, op, c_dydt, neq, yp, xp, ind->extraDoseNewXout);

            copyLinCmt(neq, ind, op, yp);
            const char* err_msg = "rks5 failed";
          ...
          if (!localBadSolve) {
            ...
            if (!isSameTime(xout, ind->extraDoseNewXout)) {
              ...
              if (neqOde > 0)
                rks5_do_steps(solver, ind, op, c_dydt, neq, yp, ind->extraDoseNewXout, xout);
        ...
        if (!localBadSolve && !isSameTime(xout, xp)) {
          ...
          if (neqOde > 0)
            rks5_do_steps(solver, ind, op, c_dydt, neq, yp, xp, xout);
...
extern "C" void ind_rks5(rx_solve *rx, int solveid,
                          t_dydt c_dydt, t_update_inis u_inis) {
  rx_solving_options *op = rx->op;
  int neq[2];
  neq[0] = op->neq;
  neq[1] = 0;
  ind_rks5_0(rx, op, solveid, neq, c_dydt, u_inis);
}
...
#pragma omp atomic read
    ...
    if (localAbort == 0) {
      ...
      ind_rks5_0(rx, op, solveid, neq, dydt, update_inis);
...
extern "C" void rks5_solveWith1Pt(int *neq, double *yp, double *xp, double xout,
  ...
  int neqOde = eff - op->numLin - op->numLinSens;
    ...
    if (neqOde > 0) {
    RxRks5 solver(dydt, neq, neqOde, ind, yp);
    solver.set_quiet(true);
    rks5_do_steps(&solver, ind, op, dydt, neq, yp, *xp, xout);
