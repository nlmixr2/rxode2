...
static inline int handleFunctionRnbinom2(transFunctions *tf) {
  if (!strcmp("rnbinom2", tf->v) ||
      !strcmp("rxnbinom2", tf->v) ||
      (tf->isInd = !strcmp("rinbinom2", tf->v))) {
    if (tb.thread != 0) tb.thread = 2;
    int ii = d_get_number_of_children(d_get_child(tf->pn,3))+1;
    if (ii != 2){
      updateSyntaxCol();
      ...
      if (tf->isInd){
        ...
        sAppend(&sbDt, "(double)rinbinom2(&_solveData->subjects[_cSub], %d, (int)", tb.nInd++);
        sAppendN(&sbt, "rinbinom2(", 10);
      } else {
