...
struct vv_system {
    ...
    vv_system(rx_solving_options_ind *ind, t_dydt c_dydt, int* neq)
        : ind_(ind), c_dydt_(c_dydt), neq_(neq), neqOde_(0), N_(0) {
    ...
    void operator()(const zero_copy_state& x, const zero_copy_state& v, zero_copy_state& a, double t) const {
        std::copy(x.data_, x.data_ + N_, y_temp_.begin());
        std::copy(v.data_, v.data_ + N_, y_temp_.begin() + N_);

        for (int i = neqOde_; i < 2 * N_; ++i) {
            dy_temp_[i] = 0.0;
        }

        c_dydt_(neq_, t, y_temp_.data(), dy_temp_.data());

        std::copy(dy_temp_.begin() + N_, dy_temp_.end(), a.data_);
    }
};

static inline void vv_do_steps(rx_solving_options_ind *ind, rx_solving_options *op, vv_system& sys, std::pair<zero_copy_state, zero_copy_state>& state, double xp, double xout) {
  typedef boost::numeric::odeint::velocity_verlet<zero_copy_state> stepper_type;
...
extern "C" void ind_vv_0(rx_solve *rx, rx_solving_options *op, int solveid, int *neq,
      ...
      if (ind->err){
        ...
        if (handleExtraDose(neq, ind->BadDose, ind->InfusionRate, ind->dose, yp, xout,
            ...
            if (!localBadSolve && !isSameTime(ind->extraDoseNewXout, xp)) {
              ...
              if (neqOde > 0) {
                  ...
                  vv_do_steps(ind, op, sys, state, xp, ind->extraDoseNewXout);
                  std::copy(state.first.begin(), state.first.end(), yp);
                  std::copy(state.second.begin(), state.second.begin() + (neqOde - N), yp + N);
              }

              copyLinCmt(neq, ind, op, yp);
              const char* err_msg = "vv failed";
              postSolve(neq, &istate, ind->rc, &i, yp, &err_msg, 18, true, ind, op, rx);
            ...
            if (!localBadSolve) {
              ...
              if (!isSameTime(xout, ind->extraDoseNewXout)) {
                ...
                if (neqOde > 0) {
                    ...
                    auto state = std::make_pair(q, p);
                    vv_do_steps(ind, op, sys, state, ind->extraDoseNewXout, xout);
                    std::copy(state.first.begin(), state.first.end(), yp);
                    std::copy(state.second.begin(), state.second.begin() + (neqOde - N), yp + N);
                }
                ...
                copyLinCmt(neq, ind, op, yp);
                const char* err_msg = "vv failed";
                postSolve(neq, &istate, ind->rc, &idx, yp, &err_msg, 18, false, ind, op, rx);
                if (*(ind->rc) < 0) localBadSolve = 1;
        ...
        if (!localBadSolve && !isSameTime(xout, xp)) {
          ...
          if (neqOde > 0) {
              ...
              auto state = std::make_pair(q, p);
              vv_do_steps(ind, op, sys, state, xp, xout);
              std::copy(state.first.begin(), state.first.end(), yp);
              std::copy(state.second.begin(), state.second.begin() + (neqOde - N), yp + N);
          }
...
extern "C" void ind_vv(rx_solve *rx, int solveid,
                          t_dydt c_dydt, t_update_inis u_inis){
  rx_solving_options *op = rx->op;
  int neq[2];
  neq[0] = op->neq;
  neq[1] = 0;
  ind_vv_0(rx, op, solveid, neq, c_dydt, u_inis);
}
...
#pragma omp atomic read
    ...
    if (localAbort == 0){
      ...
      ind_vv_0(rx, op, solveid, neq, dydt, update_inis);
...
extern "C" void vv_solveWith1Pt(int *neq, double *yp, double *xp, double xout,
