...
/*
 * rxode2's side of the rxode2lincmt tables (src/rxode2lincmtLink.cpp).  The
 * _p_* pointers start at harmless stubs and are re-bound by `.linkAll()`;
 * call the linCmt kernels only through them.  Self-contained: uses the struct
 * tags, so it can be included before rxode2.h.
 */
...
double rxode2LinCmtAFwd(struct rx_solve_s *rx, int id, double _t, int linCmt, int ncmt,
                        int oral0, int which, int trans, double p1, double v1,
                        double p2, double p3, double p4, double p5, double ka);
double rxode2LinCmtBFwd(struct rx_solve_s *rx, int id, double _t, int linCmt, int ncmt,
