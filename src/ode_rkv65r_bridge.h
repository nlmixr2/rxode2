#ifndef ODE_RKV65R_BRIDGE_H_
#define ODE_RKV65R_BRIDGE_H_

#include "ode/ode_rkv65r.h"

class RxRkv65r : public ode::OdeRkv65r {
    t_dydt                  dydt_;
    int                    *full_neq_;
...
public:
    RxRkv65r(t_dydt dydt, int *full_neq, int neqOde,
            rx_solving_options_ind *ind, double *yp,
            long unsigned max_steps)
        : ode::OdeRkv65r((unsigned long)neqOde),
...
protected:
    ...
    void step_(double dt) override {
        ...
        for (unsigned long i = 0; i < neq_; i++)
            sol_[i] = sol_[i]+dt*(b1*k_[0][i]+b4*k_[3][i]+b6*k_[5][i]+b7*k_[6][i]+b8*k_[7][i]);
    ...
    void after_step(double) override {
        if (ind_->err != 0) throw std::runtime_error("rkv65r: ode error");
        if (nstep_ > max_steps_) {
            if (ind_->rc[0] == 0) ind_->rc[0] = -2019;
            ind_->err = 1;
            throw std::runtime_error("rkv65r: max steps exceeded");
        }
    }
};

#endif // ODE_RKV65R_BRIDGE_H_
