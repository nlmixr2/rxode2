...
static inline void rkssp54_do_steps(rx_solving_options_ind *ind, rx_solving_options *op,
  ...
  RxRkssp54 solver(c_dydt, neq, neqOde, ind, yp);
      ...
      if (ind->err) {
        ...
        if (handleExtraDose(neq, ind->BadDose, ind->InfusionRate, ind->dose, yp, xout,
          ...
          if (!localBadSolve && !isSameTime(ind->extraDoseNewXout, xp)) {
            ...
            if (neqOde > 0)
              rkssp54_do_steps(ind, op, c_dydt, neq, yp, xp, ind->extraDoseNewXout);
          ...
          if (!localBadSolve) {
            ...
            if (!isSameTime(xout, ind->extraDoseNewXout)) {
              ...
              if (neqOde > 0)
                rkssp54_do_steps(ind, op, c_dydt, neq, yp, ind->extraDoseNewXout, xout);
        ...
        if (!localBadSolve && !isSameTime(xout, xp)) {
          ...
          if (neqOde > 0)
            rkssp54_do_steps(ind, op, c_dydt, neq, yp, xp, xout);
...
extern "C" void rkssp54_solveWith1Pt(int *neq, double *yp, double *xp, double xout,
  ...
  if (neqOde > 0) {
    rkssp54_do_steps(ind, op, dydt, neq, yp, *xp, xout);
    if (ind->rc[0] < 0) {
      *istate = -1;
      return;
    }
