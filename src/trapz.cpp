...
extern "C" void ind_trapz_0(rx_solve *rx, rx_solving_options *op, int solveid, int *neq,
...
extern "C" void ind_trapz(rx_solve *rx, int solveid,
                            t_dydt c_dydt, t_update_inis u_inis) {
  rx_solving_options *op = rx->op;
  int neq[2];
  neq[0] = op->neq;
  neq[1] = 0;
  ind_trapz_0(rx, op, solveid, neq, c_dydt, u_inis);
}

extern "C" void par_trapz(rx_solve *rx) {
  rx_solving_options *op = rx->op;
#ifdef _OPENMP
  int cores = op->cores;
#else
...
#pragma omp atomic read
    ...
    if (localAbort == 0) {
      ...
      ind_trapz_0(rx, op, solveid, neq, dydt, update_inis);
...
extern "C" void trapz_solveWith1Pt(int *neq, double *yp, double *xp, double xout,
