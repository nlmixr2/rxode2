...
struct sb3a_system {
    ...
    sb3a_system(rx_solving_options_ind *ind, t_dydt c_dydt, int* neq)
    ...
    struct coor_func_t {
        const sb3a_system& parent_;
        coor_func_t(const sb3a_system& parent) : parent_(parent) {}
    ...
    struct momentum_func_t {
        const sb3a_system& parent_;
        momentum_func_t(const sb3a_system& parent) : parent_(parent) {}
...
static inline void sb3a_do_steps(rx_solving_options_ind *ind, rx_solving_options *op, sb3a_system& sys, std::pair<zero_copy_state, zero_copy_state>& state, double xp, double xout) {
  typedef boost::numeric::odeint::symplectic_rkn_sb3a_mclachlan<zero_copy_state> stepper_type;
...
extern "C" void ind_sb3a_0(rx_solve *rx, rx_solving_options *op, int solveid, int *neq,
  ...
  sb3a_system sys(ind, c_dydt, neq);
      ...
      if (ind->err){
        ...
        if (handleExtraDose(neq, ind->BadDose, ind->InfusionRate, ind->dose, yp, xout,
            ...
            if (!localBadSolve && !isSameTime(ind->extraDoseNewXout, xp)) {
              ...
              if (neqOde > 0) {
                  ...
                  sb3a_do_steps(ind, op, sys, state, xp, ind->extraDoseNewXout);
                  std::copy(state.first.begin(), state.first.end(), yp);
                  std::copy(state.second.begin(), state.second.begin() + (neqOde - N), yp + N);
              }

              copyLinCmt(neq, ind, op, yp);
              const char* err_msg = "sb3a failed";
              postSolve(neq, &istate, ind->rc, &i, yp, &err_msg, 16, true, ind, op, rx);
            ...
            if (!localBadSolve) {
              ...
              if (!isSameTime(xout, ind->extraDoseNewXout)) {
                ...
                if (neqOde > 0) {
                    ...
                    auto state = std::make_pair(q, p);
                    sb3a_do_steps(ind, op, sys, state, ind->extraDoseNewXout, xout);
                    std::copy(state.first.begin(), state.first.end(), yp);
                    std::copy(state.second.begin(), state.second.begin() + (neqOde - N), yp + N);
                }
                ...
                copyLinCmt(neq, ind, op, yp);
                const char* err_msg = "sb3a failed";
                postSolve(neq, &istate, ind->rc, &idx, yp, &err_msg, 16, false, ind, op, rx);
                if (*(ind->rc) < 0) localBadSolve = 1;
        ...
        if (!localBadSolve && !isSameTime(xout, xp)) {
          ...
          if (neqOde > 0) {
              ...
              auto state = std::make_pair(q, p);
              sb3a_do_steps(ind, op, sys, state, xp, xout);
              std::copy(state.first.begin(), state.first.end(), yp);
              std::copy(state.second.begin(), state.second.begin() + (neqOde - N), yp + N);
          }
...
extern "C" void ind_sb3a(rx_solve *rx, int solveid,
                          t_dydt c_dydt, t_update_inis u_inis){
  rx_solving_options *op = rx->op;
  int neq[2];
  neq[0] = op->neq;
  neq[1] = 0;
  ind_sb3a_0(rx, op, solveid, neq, c_dydt, u_inis);
}
...
#pragma omp atomic read
    ...
    if (localAbort == 0){
      ...
      ind_sb3a_0(rx, op, solveid, neq, dydt, update_inis);
...
extern "C" void sb3a_solveWith1Pt(int *neq, double *yp, double *xp, double xout,
