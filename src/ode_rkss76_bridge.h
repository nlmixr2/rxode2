#ifndef ODE_RKSS76_BRIDGE_H_
#define ODE_RKSS76_BRIDGE_H_

#include "ode/ode_rkss76.h"

class RxRkss76 : public ode::OdeRkss76 {
    t_dydt                  dydt_;
    int                    *full_neq_;
...
public:
    RxRkss76(t_dydt dydt, int *full_neq, int neqOde,
            rx_solving_options_ind *ind, double *yp,
            long unsigned max_steps)
        : ode::OdeRkss76((unsigned long)neqOde),
...
protected:
    ...
    void step_(double dt) override {
        ...
        // Stage 10
        for (unsigned long i = 0; i < neq_; i++)
            soltemp_[i] = sol_[i] + dt * (a101*k_[0][i]+a103*k_[2][i]+a104*k_[3][i]+a105*k_[4][i]+a106*k_[5][i]+a107*k_[6][i]+a108*k_[7][i]+a109*k_[8][i]);
        t_stage_ = t_ + dt;
        ode_fun_(soltemp_, k_[9]);
        // Stage 11
        for (unsigned long i = 0; i < neq_; i++)
            soltemp_[i] = sol_[i] + dt * (a111*k_[0][i]+a113*k_[2][i]+a114*k_[3][i]+a115*k_[4][i]+a116*k_[5][i]+a117*k_[6][i]+a118*k_[7][i]+a119*k_[8][i]);
        t_stage_ = t_ + dt;
        ode_fun_(soltemp_, k_[10]);
        // Primary (higher-order) solution -> sol_
        for (unsigned long i = 0; i < neq_; i++)
            sol_[i] = sol_[i]+dt*(b1*k_[0][i]+b5*k_[4][i]+b6*k_[5][i]+b7*k_[6][i]+b8*k_[7][i]+b9*k_[8][i]+b10*k_[9][i]);
        // Embedded (lower-order) solution -> solemb_
        for (unsigned long i = 0; i < neq_; i++)
            solemb_[i] = sol_[i] + dt * ((d1 - b1) * k_[0][i] + (d5 - b5) * k_[4][i] + (d6 - b6) * k_[5][i] + (d7 - b7) * k_[6][i] + (d8 - b8) * k_[7][i] + (d9 - b9) * k_[8][i] + (d10 - b10) * k_[9][i] + (d11 - b11) * k_[10][i]);
    }

    void after_step(double) override {
        if (ind_->err != 0) throw std::runtime_error("rkss76: ode error");
        if (nstep_ > max_steps_) {
            if (ind_->rc[0] == 0) ind_->rc[0] = -2019;
            ind_->err = 1;
            throw std::runtime_error("rkss76: max steps exceeded");
        }
    }
};

#endif // ODE_RKSS76_BRIDGE_H_
